...
Published in 2020 by He, Liu, Gao, Chen, **DeBERTa: Decoding-enhanced BERT with Disentangled Attention** belongs to the NLP part of the Paper2Code library. Disentangled attention (position and content separate) + enhanced mask decoder. Surpassed human performance on SuperGLUE with DeBERTa-xxlarge. Best performing BERT-style model. [1]
...
- **Disentangled attention:** identify its input, output, learned parameters, and the assumption it adds to the baseline.
- **separate position/content encodings:** identify its input, output, learned parameters, and the assumption it adds to the baseline.
- **enhanced mask decoder:** identify its input, output, learned parameters, and the assumption it adds to the baseline.

Its associated architecture or procedure is **DeBERTa**. The contribution should be explained as a change to a baseline computation: what new state is introduced, where information flows differently, and which objective trains the change. Disentangled attention (position and content separate) + enhanced mask decoder. Surpassed human performance on SuperGLUE with DeBERTa-xxlarge. Best performing BERT-style model.
...
The paper's main claim is summarized in the registry as: Disentangled attention (position and content separate) + enhanced mask decoder. Surpassed human performance on SuperGLUE with DeBERTa-xxlarge. Best performing BERT-style model. The key experiments should therefore isolate **causality**, not merely display a large final score.
...
The registry records the practical significance as: SuperGLUE SOTA; used as high-accuracy NLU backbone in production This statement explains *why the paper remained relevant*, but exact benchmark values must be taken from the primary tables in [1]. They are intentionally not invented or copied without experimental context here.
...
- [Architecture deep dive](/architectures/deberta) — inspect the corresponding model family and tensor flow.
...
[1] He, Liu, Gao, Chen. **DeBERTa: Decoding-enhanced BERT with Disentangled Attention**. 2020.
