...
Published in 2017 by He, Gkioxari, Dollár, Girshick, **Mask R-CNN** belongs to the COMPUTER VISION part of the Paper2Code library. Extended Faster R-CNN to instance segmentation by adding a mask head. RoIAlign fixed quantization artifacts. Defined the two-stage instance segmentation paradigm. [1]
...
- **Instance segmentation:** identify its input, output, learned parameters, and the assumption it adds to the baseline.
- **RoIAlign (vs RoI Pooling):** identify its input, output, learned parameters, and the assumption it adds to the baseline.
- **parallel mask and box prediction:** identify its input, output, learned parameters, and the assumption it adds to the baseline.

Its associated architecture or procedure is **Mask R-CNN, RoIAlign**. The contribution should be explained as a change to a baseline computation: what new state is introduced, where information flows differently, and which objective trains the change. Extended Faster R-CNN to instance segmentation by adding a mask head. RoIAlign fixed quantization artifacts. Defined the two-stage instance segmentation paradigm.
...
The paper's main claim is summarized in the registry as: Extended Faster R-CNN to instance segmentation by adding a mask head. RoIAlign fixed quantization artifacts. Defined the two-stage instance segmentation paradigm. The key experiments should therefore isolate **causality**, not merely display a large final score.
...
The registry records the practical significance as: Medical imaging, autonomous driving segmentation, Detectron2 default model This statement explains *why the paper remained relevant*, but exact benchmark values must be taken from the primary tables in [1]. They are intentionally not invented or copied without experimental context here.
...
- [Architecture deep dive](/architectures/mask-r-cnn) — inspect the corresponding model family and tensor flow.
...
[1] He, Gkioxari, Dollár, Girshick. **Mask R-CNN**. 2017.
