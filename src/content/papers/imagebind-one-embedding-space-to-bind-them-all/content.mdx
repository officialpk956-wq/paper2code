...
Published in 2023 by Girdhar, El-Nouby, Liu, Singh, Rambhatla, Farahat, Patel, Bisk, De Sa, Joulin, Malik, **ImageBind: One Embedding Space to Bind Them All** belongs to the MULTIMODAL part of the Paper2Code library. Single embedding space for 6 modalities — images, text, audio, depth, thermal, IMU. Audio-image and text-audio alignment without paired data, via binding through images. Any-to-any retrieval. [1]
...
- **Multi-modal binding:** identify its input, output, learned parameters, and the assumption it adds to the baseline.
- **emergent cross-modal alignment:** identify its input, output, learned parameters, and the assumption it adds to the baseline.
- **6-modality space:** identify its input, output, learned parameters, and the assumption it adds to the baseline.

Its associated architecture or procedure is **ImageBind**. The contribution should be explained as a change to a baseline computation: what new state is introduced, where information flows differently, and which objective trains the change. Single embedding space for 6 modalities — images, text, audio, depth, thermal, IMU. Audio-image and text-audio alignment without paired data, via binding through images. Any-to-any retrieval.
...
The paper's main claim is summarized in the registry as: Single embedding space for 6 modalities — images, text, audio, depth, thermal, IMU. Audio-image and text-audio alignment without paired data, via binding through images. Any-to-any retrieval. The key experiments should therefore isolate **causality**, not merely display a large final score.
...
Cross-modal search, any-modality-to-any retrieval To translate that impact into a production decision, measure the complete path rather than an isolated kernel or offline model. Include preprocessing, retrieval or batching, host-device transfer, post-processing, cache behavior, and tail latency.
...
- [Visual Instruction Tuning (LLaVA)](/papers/visual-instruction-tuning-llava) — adjacent reading from the same library section.
- [Robust Speech Recognition via Large-Scale Weak Supervision (Whisper)](/papers/robust-speech-recognition-via-large-scale-weak-supervision-whisper) — adjacent reading from the same library section.
...
[1] Girdhar, El-Nouby, Liu, Singh, Rambhatla, Farahat, Patel, Bisk, De Sa, Joulin, Malik. **ImageBind: One Embedding Space to Bind Them All**. 2023.
