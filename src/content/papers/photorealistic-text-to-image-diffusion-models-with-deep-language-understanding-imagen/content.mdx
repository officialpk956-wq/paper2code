...
Published in 2022 by Saharia, Chan, Saxena, Li, Whang, Denton, et al., **Photorealistic Text-to-Image Diffusion Models with Deep Language Understanding (Imagen)** belongs to the DIFFUSION MODELS part of the Paper2Code library. Large frozen language model (T5-XXL) as text encoder beats CLIP for text fidelity. Demonstrated that language understanding &gt; image-text alignment for high-fidelity generation. [1]
...
- **Frozen large LM for image generation:** identify its input, output, learned parameters, and the assumption it adds to the baseline.
- **cascaded diffusion:** identify its input, output, learned parameters, and the assumption it adds to the baseline.
- **classifier-free guidance scale:** identify its input, output, learned parameters, and the assumption it adds to the baseline.

Its associated architecture or procedure is **Imagen (cascaded diffusion + T5 conditioning)**. The contribution should be explained as a change to a baseline computation: what new state is introduced, where information flows differently, and which objective trains the change. Large frozen language model (T5-XXL) as text encoder beats CLIP for text fidelity. Demonstrated that language understanding &gt; image-text alignment for high-fidelity generation.
...
The paper's main claim is summarized in the registry as: Large frozen language model (T5-XXL) as text encoder beats CLIP for text fidelity. Demonstrated that language understanding &gt; image-text alignment for high-fidelity generation. The key experiments should therefore isolate **causality**, not merely display a large final score.
...
The registry records the practical significance as: Google's Imagen product; demonstrated T5 &gt; CLIP for text adherence This statement explains *why the paper remained relevant*, but exact benchmark values must be taken from the primary tables in [1]. They are intentionally not invented or copied without experimental context here.
...
- [Architecture deep dive](/architectures/imagen) — inspect the corresponding model family and tensor flow.
...
[1] Saharia, Chan, Saxena, Li, Whang, Denton, et al.. **Photorealistic Text-to-Image Diffusion Models with Deep Language Understanding (Imagen)**. 2022.
