...
Published in 2022 by Ramesh, Dhariwal, Nichol, Chu, Chen, **Hierarchical Text-Conditional Image Generation with CLIP Latents (DALL-E 2)** belongs to the DIFFUSION MODELS part of the Paper2Code library. CLIP image embeddings as prior between text and image. Demonstrated semantic image editing, interpolation, and text-image combination. Showed diffusion + CLIP = high-fidelity controllable generation. [1]
...
- **CLIP image embedding prior:** identify its input, output, learned parameters, and the assumption it adds to the baseline.
- **unCLIP:** identify its input, output, learned parameters, and the assumption it adds to the baseline.
- **hierarchical generation:** identify its input, output, learned parameters, and the assumption it adds to the baseline.
- **semantic image manipulation:** identify its input, output, learned parameters, and the assumption it adds to the baseline.

Its associated architecture or procedure is **DALL-E 2 (CLIP prior + diffusion decoder)**. The contribution should be explained as a change to a baseline computation: what new state is introduced, where information flows differently, and which objective trains the change. CLIP image embeddings as prior between text and image. Demonstrated semantic image editing, interpolation, and text-image combination. Showed diffusion + CLIP = high-fidelity controllable generation.
...
The registry records the practical significance as: OpenAI image generation; demonstrated text-image alignment at scale This statement explains *why the paper remained relevant*, but exact benchmark values must be taken from the primary tables in [1]. They are intentionally not invented or copied without experimental context here.
...
- [Architecture deep dive](/architectures/dall-e-2) — inspect the corresponding model family and tensor flow.
...
[1] Ramesh, Dhariwal, Nichol, Chu, Chen. **Hierarchical Text-Conditional Image Generation with CLIP Latents (DALL-E 2)**. 2022.
