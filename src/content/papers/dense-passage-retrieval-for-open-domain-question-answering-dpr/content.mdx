...
Published in 2020 by Karpukhin, Oğuz, Min, Lewis, Wu, Edunov, Chen, Yih, **Dense Passage Retrieval for Open-Domain Question Answering (DPR)** belongs to the RAG part of the Paper2Code library. Showed that dense bi-encoder retrieval with in-batch negative training outperforms TF-IDF/BM25 on open-domain QA. Defined the bi-encoder training recipe used in all dense retrievers. [1]
...
- **Bi-encoder retrieval:** identify its input, output, learned parameters, and the assumption it adds to the baseline.
- **in-batch negatives:** identify its input, output, learned parameters, and the assumption it adds to the baseline.
- **BERT-based encoder for retrieval:** identify its input, output, learned parameters, and the assumption it adds to the baseline.
- **FAISS integration:** identify its input, output, learned parameters, and the assumption it adds to the baseline.

Its associated architecture or procedure is **DPR (Dense Passage Retrieval)**. The contribution should be explained as a change to a baseline computation: what new state is introduced, where information flows differently, and which objective trains the change. Showed that dense bi-encoder retrieval with in-batch negative training outperforms TF-IDF/BM25 on open-domain QA. Defined the bi-encoder training recipe used in all dense retrievers.
...
The paper's main claim is summarized in the registry as: Showed that dense bi-encoder retrieval with in-batch negative training outperforms TF-IDF/BM25 on open-domain QA. Defined the bi-encoder training recipe used in all dense retrievers. The key experiments should therefore isolate **causality**, not merely display a large final score.
...
The registry records the practical significance as: Foundation of all dense retrieval systems; RAG retrieval component This statement explains *why the paper remained relevant*, but exact benchmark values must be taken from the primary tables in [1]. They are intentionally not invented or copied without experimental context here.
...
- [Retrieval-Augmented Generation for Knowledge-Intensive NLP Tasks](/papers/retrieval-augmented-generation-for-knowledge-intensive-nlp-tasks) — adjacent reading from the same library section.
...
[1] Karpukhin, Oğuz, Min, Lewis, Wu, Edunov, Chen, Yih. **Dense Passage Retrieval for Open-Domain Question Answering (DPR)**. 2020.
