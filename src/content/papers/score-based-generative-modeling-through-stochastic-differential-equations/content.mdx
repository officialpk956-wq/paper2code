...
Published in 2020 by Song, Sohl-Dickstein, Kingma, Kumar, Ermon, Poole, **Score-Based Generative Modeling through Stochastic Differential Equations** belongs to the DIFFUSION MODELS part of the Paper2Code library. Unified diffusion models and score matching as continuous-time SDEs. Theoretical foundation that enables new samplers, conditioned generation, and the connections to flow matching. [1]
...
- **Score function:** identify its input, output, learned parameters, and the assumption it adds to the baseline.
- **Stein's identity:** identify its input, output, learned parameters, and the assumption it adds to the baseline.
- **SDE framework:** identify its input, output, learned parameters, and the assumption it adds to the baseline.
- **probability flow ODE:** identify its input, output, learned parameters, and the assumption it adds to the baseline.

Its associated architecture or procedure is **None (theoretical framework)**. The contribution should be explained as a change to a baseline computation: what new state is introduced, where information flows differently, and which objective trains the change. Unified diffusion models and score matching as continuous-time SDEs. Theoretical foundation that enables new samplers, conditioned generation, and the connections to flow matching.
...
The paper's main claim is summarized in the registry as: Unified diffusion models and score matching as continuous-time SDEs. Theoretical foundation that enables new samplers, conditioned generation, and the connections to flow matching. The key experiments should therefore isolate **causality**, not merely display a large final score.
...
The registry records the practical significance as: Theoretical basis for improved samplers (DDIM, DPM-Solver) and flow matching This statement explains *why the paper remained relevant*, but exact benchmark values must be taken from the primary tables in [1]. They are intentionally not invented or copied without experimental context here.
...
- [High-Resolution Image Synthesis with Latent Diffusion Models (Stable Diffusion)](/papers/high-resolution-image-synthesis-with-latent-diffusion-models-stable-diffusion) — adjacent reading from the same library section.
- [Denoising Diffusion Implicit Models (DDIM)](/papers/denoising-diffusion-implicit-models-ddim) — adjacent reading from the same library section.
...
[1] Song, Sohl-Dickstein, Kingma, Kumar, Ermon, Poole. **Score-Based Generative Modeling through Stochastic Differential Equations**. 2020.
