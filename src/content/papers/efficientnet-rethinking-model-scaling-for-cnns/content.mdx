...
Published in 2019 by Tan, Le, **EfficientNet: Rethinking Model Scaling for CNNs** belongs to the FOUNDATIONS part of the Paper2Code library. Showed that jointly scaling width, depth, and resolution (compound scaling) yields better performance than scaling any dimension alone. Set ImageNet accuracy records while being smaller than competitors. [1]
...
- **Compound scaling:** identify its input, output, learned parameters, and the assumption it adds to the baseline.
- **neural architecture search for base model:** identify its input, output, learned parameters, and the assumption it adds to the baseline.
- **efficiency-accuracy Pareto frontier:** identify its input, output, learned parameters, and the assumption it adds to the baseline.

Its associated architecture or procedure is **EfficientNet-B0 through B7, compound scaling coefficient**. The contribution should be explained as a change to a baseline computation: what new state is introduced, where information flows differently, and which objective trains the change. Showed that jointly scaling width, depth, and resolution (compound scaling) yields better performance than scaling any dimension alone. Set ImageNet accuracy records while being smaller than competitors.
...
The paper's main claim is summarized in the registry as: Showed that jointly scaling width, depth, and resolution (compound scaling) yields better performance than scaling any dimension alone. Set ImageNet accuracy records while being smaller than competitors. The key experiments should therefore isolate **causality**, not merely display a large final score.
...
The registry records the practical significance as: Google Cloud Vision API, on-device image classification This statement explains *why the paper remained relevant*, but exact benchmark values must be taken from the primary tables in [1]. They are intentionally not invented or copied without experimental context here.
...
Google Cloud Vision API, on-device image classification To translate that impact into a production decision, measure the complete path rather than an isolated kernel or offline model. Include preprocessing, retrieval or batching, host-device transfer, post-processing, cache behavior, and tail latency.
...
- [Architecture deep dive](/architectures/efficientnet) — inspect the corresponding model family and tensor flow.
- [Layer Normalization](/papers/layer-normalization) — adjacent reading from the same library section.
- [Knowledge Distillation](/papers/knowledge-distillation) — adjacent reading from the same library section.
...
[1] Tan, Le. **EfficientNet: Rethinking Model Scaling for CNNs**. 2019.
