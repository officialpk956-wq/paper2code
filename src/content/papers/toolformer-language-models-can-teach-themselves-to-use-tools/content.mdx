...
Published in 2023 by Schick, Dwivedi-Yu, Dessì, Raileanu, Lomeli, Zettlemoyer, Cancedda, Scialom, **Toolformer: Language Models Can Teach Themselves to Use Tools** belongs to the AGENTS part of the Paper2Code library. Self-supervised learning of when and how to call APIs (calculator, search, calendar). No human annotation — the model decides when tools help. Defined tool-use as a learned capability. [1]
...
- **Self-supervised tool use:** identify its input, output, learned parameters, and the assumption it adds to the baseline.
- **API call generation:** identify its input, output, learned parameters, and the assumption it adds to the baseline.
- **tool integration as sequence modeling:** identify its input, output, learned parameters, and the assumption it adds to the baseline.

Its associated architecture or procedure is **Toolformer**. The contribution should be explained as a change to a baseline computation: what new state is introduced, where information flows differently, and which objective trains the change. Self-supervised learning of when and how to call APIs (calculator, search, calendar). No human annotation — the model decides when tools help. Defined tool-use as a learned capability.
...
The paper's main claim is summarized in the registry as: Self-supervised learning of when and how to call APIs (calculator, search, calendar). No human annotation — the model decides when tools help. Defined tool-use as a learned capability. The key experiments should therefore isolate **causality**, not merely display a large final score.
...
The registry records the practical significance as: Tool-calling LLMs; basis for OpenAI function calling, Claude tool use This statement explains *why the paper remained relevant*, but exact benchmark values must be taken from the primary tables in [1]. They are intentionally not invented or copied without experimental context here.
...
- [ReAct: Synergizing Reasoning and Acting in Language Models](/papers/react-synergizing-reasoning-and-acting-in-language-models) — adjacent reading from the same library section.
...
[1] Schick, Dwivedi-Yu, Dessì, Raileanu, Lomeli, Zettlemoyer, Cancedda, Scialom. **Toolformer: Language Models Can Teach Themselves to Use Tools**. 2023.
