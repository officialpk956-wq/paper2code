...
Published in 2022 by Peebles, Xie, **Scalable Diffusion Models with Transformers (DiT)** belongs to the DIFFUSION MODELS part of the Paper2Code library. Replaced U-Net backbone in diffusion with Vision Transformer. Scales better — larger DiT = better FID. Sora uses DiT-based architecture for video generation. Defined the next generation of diffusion models. [1]
...
- **Transformer as diffusion backbone:** identify its input, output, learned parameters, and the assumption it adds to the baseline.
- **adaptive layer norm conditioning:** identify its input, output, learned parameters, and the assumption it adds to the baseline.
- **linear class:** identify its input, output, learned parameters, and the assumption it adds to the baseline.

Its associated architecture or procedure is **DiT (Diffusion Transformer), adaLN-Zero conditioning**. The contribution should be explained as a change to a baseline computation: what new state is introduced, where information flows differently, and which objective trains the change. Replaced U-Net backbone in diffusion with Vision Transformer. Scales better — larger DiT = better FID. Sora uses DiT-based architecture for video generation. Defined the next generation of diffusion models.
...
The paper's main claim is summarized in the registry as: Replaced U-Net backbone in diffusion with Vision Transformer. Scales better — larger DiT = better FID. Sora uses DiT-based architecture for video generation. Defined the next generation of diffusion models. The key experiments should therefore isolate **causality**, not merely display a large final score.
...
The registry records the practical significance as: Sora's architecture basis; Stable Diffusion 3 uses DiT; future diffusion models will use DiT This statement explains *why the paper remained relevant*, but exact benchmark values must be taken from the primary tables in [1]. They are intentionally not invented or copied without experimental context here.
...
- [Hierarchical Text-Conditional Image Generation with CLIP Latents (DALL-E 2)](/papers/hierarchical-text-conditional-image-generation-with-clip-latents-dall-e-2) — adjacent reading from the same library section.
- [Adding Conditional Control to Text-to-Image Diffusion Models (ControlNet)](/papers/adding-conditional-control-to-text-to-image-diffusion-models-controlnet) — adjacent reading from the same library section.
...
[1] Peebles, Xie. **Scalable Diffusion Models with Transformers (DiT)**. 2022.
