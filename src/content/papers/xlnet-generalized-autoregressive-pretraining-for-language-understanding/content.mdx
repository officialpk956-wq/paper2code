...
Published in 2019 by Yang, Dai, Yang, Carbonell, Salakhutdinov, Le, **XLNet: Generalized Autoregressive Pretraining for Language Understanding** belongs to the NLP part of the Paper2Code library. Permutation language modeling — gets bidirectional context in an autoregressive model without masking artifacts. Showed MLM has a pre-train/fine-tune inconsistency BERT doesn't fix. [1]
...
- **Permutation language modeling:** identify its input, output, learned parameters, and the assumption it adds to the baseline.
- **two-stream attention:** identify its input, output, learned parameters, and the assumption it adds to the baseline.
- **relative positional encoding:** identify its input, output, learned parameters, and the assumption it adds to the baseline.

Its associated architecture or procedure is **XLNet**. The contribution should be explained as a change to a baseline computation: what new state is introduced, where information flows differently, and which objective trains the change. Permutation language modeling — gets bidirectional context in an autoregressive model without masking artifacts. Showed MLM has a pre-train/fine-tune inconsistency BERT doesn't fix.
...
The paper's main claim is summarized in the registry as: Permutation language modeling — gets bidirectional context in an autoregressive model without masking artifacts. Showed MLM has a pre-train/fine-tune inconsistency BERT doesn't fix. The key experiments should therefore isolate **causality**, not merely display a large final score.
...
The registry records the practical significance as: Beat BERT on 20 tasks; motivated improved training objectives This statement explains *why the paper remained relevant*, but exact benchmark values must be taken from the primary tables in [1]. They are intentionally not invented or copied without experimental context here.
...
Beat BERT on 20 tasks; motivated improved training objectives To translate that impact into a production decision, measure the complete path rather than an isolated kernel or offline model. Include preprocessing, retrieval or batching, host-device transfer, post-processing, cache behavior, and tail latency.
...
- [Architecture deep dive](/architectures/xlnet) — inspect the corresponding model family and tensor flow.
- [Sentence-BERT: Sentence Embeddings using Siamese BERT-Networks](/papers/sentence-bert-sentence-embeddings-using-siamese-bert-networks) — adjacent reading from the same library section.
- [ALBERT: A Lite BERT for Self-supervised Learning](/papers/albert-a-lite-bert-for-self-supervised-learning) — adjacent reading from the same library section.
...
[1] Yang, Dai, Yang, Carbonell, Salakhutdinov, Le. **XLNet: Generalized Autoregressive Pretraining for Language Understanding**. 2019.
