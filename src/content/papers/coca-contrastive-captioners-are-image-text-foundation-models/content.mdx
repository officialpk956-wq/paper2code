...
Published in 2022 by Yu, Wang, Vasudevan, Yeung, Sohl-Dickstein, Ghosh, **CoCa: Contrastive Captioners are Image-Text Foundation Models** belongs to the MULTIMODAL part of the Paper2Code library. Combined contrastive loss (like CLIP) and captioning loss (like generative models) in a single training. Best of both worlds — strong retrieval AND strong generation from one model. [1]
...
- **Contrastive + captioning loss:** identify its input, output, learned parameters, and the assumption it adds to the baseline.
- **attentional pooling:** identify its input, output, learned parameters, and the assumption it adds to the baseline.
- **unified image-text model:** identify its input, output, learned parameters, and the assumption it adds to the baseline.

Its associated architecture or procedure is **CoCa**. The contribution should be explained as a change to a baseline computation: what new state is introduced, where information flows differently, and which objective trains the change. Combined contrastive loss (like CLIP) and captioning loss (like generative models) in a single training. Best of both worlds — strong retrieval AND strong generation from one model.
...
The paper's main claim is summarized in the registry as: Combined contrastive loss (like CLIP) and captioning loss (like generative models) in a single training. Best of both worlds — strong retrieval AND strong generation from one model. The key experiments should therefore isolate **causality**, not merely display a large final score.
...
The registry records the practical significance as: Google's image understanding; showed combined objectives work better This statement explains *why the paper remained relevant*, but exact benchmark values must be taken from the primary tables in [1]. They are intentionally not invented or copied without experimental context here.
...
- [Architecture deep dive](/architectures/coca) — inspect the corresponding model family and tensor flow.
...
[1] Yu, Wang, Vasudevan, Yeung, Sohl-Dickstein, Ghosh. **CoCa: Contrastive Captioners are Image-Text Foundation Models**. 2022.
