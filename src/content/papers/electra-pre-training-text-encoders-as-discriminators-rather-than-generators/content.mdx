...
Published in 2020 by Clark, Luong, Le, Manning (Google Brain / Stanford), **ELECTRA: Pre-training Text Encoders as Discriminators Rather than Generators** belongs to the SYSTEMS part of the Paper2Code library. BERT masks ~15% of tokens and predicts only them — 85% of tokens contribute nothing to the loss. ELECTRA trains a discriminator to detect whether each token was replaced by a generator, learning from every token simultaneously. Achieves BERT performance with 4× less compute; ELECTRA-Small outperforms GPT on GLUE. [1]
...
- **Replaced token detection (RTD):** identify its input, output, learned parameters, and the assumption it adds to the baseline.
- **generator-discriminator pretraining:** identify its input, output, learned parameters, and the assumption it adds to the baseline.
- **compute-efficient pretraining:** identify its input, output, learned parameters, and the assumption it adds to the baseline.

Its associated architecture or procedure is **ELECTRA (replaced token detection)**. The contribution should be explained as a change to a baseline computation: what new state is introduced, where information flows differently, and which objective trains the change. BERT masks ~15% of tokens and predicts only them — 85% of tokens contribute nothing to the loss. ELECTRA trains a discriminator to detect whether each token was replaced by a generator, learning from every token simultaneously. Achieves BERT performance with 4× less compute; ELECTRA-Small outperforms GPT on GLUE.
...
The paper's main claim is summarized in the registry as: BERT masks ~15% of tokens and predicts only them — 85% of tokens contribute nothing to the loss. ELECTRA trains a discriminator to detect whether each token was replaced by a generator, learning from every token simultaneously. Achieves BERT performance with 4× less compute; ELECTRA-Small outperforms GPT on GLUE. The key experiments should therefore isolate **causality**, not merely display a large final score.
...
The registry records the practical significance as: Used in production NLP pipelines where inference cost matters. Influenced DeBERTa and more efficient encoder variants. Showed that pretraining objectives matter as much as architecture size. This statement explains *why the paper remained relevant*, but exact benchmark values must be taken from the primary tables in [1]. They are intentionally not invented or copied without experimental context here.
...
- [Architecture deep dive](/architectures/electra) — inspect the corresponding model family and tensor flow.
...
[1] Clark, Luong, Le, Manning (Google Brain / Stanford). **ELECTRA: Pre-training Text Encoders as Discriminators Rather than Generators**. 2020.
