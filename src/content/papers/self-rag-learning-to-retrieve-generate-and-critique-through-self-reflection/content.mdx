...
Published in 2023 by Asai, Wu, Wang, Sil, Hajishirzi, **Self-RAG: Learning to Retrieve, Generate, and Critique through Self-Reflection** belongs to the RAG part of the Paper2Code library. LLM learns when to retrieve (not always) and critiques its own outputs. Reflection tokens for retrieve/relevant/supported/useful decisions. Better accuracy and factuality than standard RAG. [1]
...
- **Adaptive retrieval:** identify its input, output, learned parameters, and the assumption it adds to the baseline.
- **reflection tokens:** identify its input, output, learned parameters, and the assumption it adds to the baseline.
- **self-critique for RAG:** identify its input, output, learned parameters, and the assumption it adds to the baseline.
- **citation grounding:** identify its input, output, learned parameters, and the assumption it adds to the baseline.

Its associated architecture or procedure is **Self-RAG**. The contribution should be explained as a change to a baseline computation: what new state is introduced, where information flows differently, and which objective trains the change. LLM learns when to retrieve (not always) and critiques its own outputs. Reflection tokens for retrieve/relevant/supported/useful decisions. Better accuracy and factuality than standard RAG.
...
The paper's main claim is summarized in the registry as: LLM learns when to retrieve (not always) and critiques its own outputs. Reflection tokens for retrieve/relevant/supported/useful decisions. Better accuracy and factuality than standard RAG. The key experiments should therefore isolate **causality**, not merely display a large final score.
...
The registry records the practical significance as: More accurate, efficient RAG — reduces unnecessary retrievals This statement explains *why the paper remained relevant*, but exact benchmark values must be taken from the primary tables in [1]. They are intentionally not invented or copied without experimental context here.
...
More accurate, efficient RAG — reduces unnecessary retrievals To translate that impact into a production decision, measure the complete path rather than an isolated kernel or offline model. Include preprocessing, retrieval or batching, host-device transfer, post-processing, cache behavior, and tail latency.
...
[1] Asai, Wu, Wang, Sil, Hajishirzi. **Self-RAG: Learning to Retrieve, Generate, and Critique through Self-Reflection**. 2023.
