...
Published in 2013 by Kingma, Welling, **Auto-Encoding Variational Bayes (VAE)** belongs to the FOUNDATIONS part of the Paper2Code library. Introduced principled probabilistic generative modeling. The ELBO objective and reparameterization trick enabled end-to-end training of generative models with latent variables. [1]
...
Earlier likelihood, adversarial, and autoregressive approaches traded off sample fidelity, mode coverage, stable optimization, and inference speed in different ways. By 2013, researchers had working baselines, but the remaining gap was large enough that simply adding more layers, data, or hardware did not answer the scientific question.
...
- **ELBO (evidence lower bound):** identify its input, output, learned parameters, and the assumption it adds to the baseline.
- **reparameterization trick:** identify its input, output, learned parameters, and the assumption it adds to the baseline.
- **latent variable models:** identify its input, output, learned parameters, and the assumption it adds to the baseline.
- **Gaussian latent space:** identify its input, output, learned parameters, and the assumption it adds to the baseline.

Its associated architecture or procedure is **Variational Autoencoder**. The contribution should be explained as a change to a baseline computation: what new state is introduced, where information flows differently, and which objective trains the change. Introduced principled probabilistic generative modeling. The ELBO objective and reparameterization trick enabled end-to-end training of generative models with latent variables.
...
The paper's main claim is summarized in the registry as: Introduced principled probabilistic generative modeling. The ELBO objective and reparameterization trick enabled end-to-end training of generative models with latent variables. The key experiments should therefore isolate **causality**, not merely display a large final score.
...
The registry records the practical significance as: Foundation for latent diffusion, stable diffusion's VQ-VAE stage, anomaly detection This statement explains *why the paper remained relevant*, but exact benchmark values must be taken from the primary tables in [1]. They are intentionally not invented or copied without experimental context here.
...
- [Architecture deep dive](/architectures/vae) — inspect the corresponding model family and tensor flow.
- [Generative Adversarial Nets](/papers/generative-adversarial-nets) — adjacent reading from the same library section.
- [Dropout: A Simple Way to Prevent Neural Networks from Overfitting](/papers/dropout-a-simple-way-to-prevent-neural-networks-from-overfitting) — adjacent reading from the same library section.
...
[1] Kingma, Welling. **Auto-Encoding Variational Bayes (VAE)**. 2013.
