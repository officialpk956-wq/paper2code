...
Published in 2023 by Shinn, Cassano, Labash, Gopinath, Narasimhan, Yao, **Reflexion: Language Agents with Verbal Reinforcement Learning** belongs to the AGENTS part of the Paper2Code library. Agents that learn from failure via verbal reflection — no gradient updates. Store failures in episodic memory, use them to improve on next trial. Self-improving agents without fine-tuning. [1]
...
- **Verbal reinforcement:** identify its input, output, learned parameters, and the assumption it adds to the baseline.
- **episodic memory:** identify its input, output, learned parameters, and the assumption it adds to the baseline.
- **self-reflection:** identify its input, output, learned parameters, and the assumption it adds to the baseline.
- **trial-and-error improvement:** identify its input, output, learned parameters, and the assumption it adds to the baseline.

Its associated architecture or procedure is **Reflexion**. The contribution should be explained as a change to a baseline computation: what new state is introduced, where information flows differently, and which objective trains the change. Agents that learn from failure via verbal reflection — no gradient updates. Store failures in episodic memory, use them to improve on next trial. Self-improving agents without fine-tuning.
...
The paper's main claim is summarized in the registry as: Agents that learn from failure via verbal reflection — no gradient updates. Store failures in episodic memory, use them to improve on next trial. Self-improving agents without fine-tuning. The key experiments should therefore isolate **causality**, not merely display a large final score.
...
The registry records the practical significance as: Self-improving coding agents, autonomous error correction This statement explains *why the paper remained relevant*, but exact benchmark values must be taken from the primary tables in [1]. They are intentionally not invented or copied without experimental context here.
...
Self-improving coding agents, autonomous error correction To translate that impact into a production decision, measure the complete path rather than an isolated kernel or offline model. Include preprocessing, retrieval or batching, host-device transfer, post-processing, cache behavior, and tail latency.
...
- [Gorilla: Large Language Model Connected with Massive APIs](/papers/gorilla-large-language-model-connected-with-massive-apis) — adjacent reading from the same library section.
...
[1] Shinn, Cassano, Labash, Gopinath, Narasimhan, Yao. **Reflexion: Language Agents with Verbal Reinforcement Learning**. 2023.
