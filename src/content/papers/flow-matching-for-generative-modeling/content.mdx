...
Published in 2022 by Lipman, Chen, Ben-Hamu, Nickel, Le, **Flow Matching for Generative Modeling** belongs to the DIFFUSION MODELS part of the Paper2Code library. Straight-line paths from noise to data vs. curved diffusion paths. Fewer NFE (function evaluations) needed. Conceptually simpler than score-based models. Basis for Stable Diffusion 3 and Flux. [1]
...
- **Conditional flow matching:** identify its input, output, learned parameters, and the assumption it adds to the baseline.
- **optimal transport paths:** identify its input, output, learned parameters, and the assumption it adds to the baseline.
- **rectified flows:** identify its input, output, learned parameters, and the assumption it adds to the baseline.
- **straight trajectories:** identify its input, output, learned parameters, and the assumption it adds to the baseline.

Its associated architecture or procedure is **Flow matching**. The contribution should be explained as a change to a baseline computation: what new state is introduced, where information flows differently, and which objective trains the change. Straight-line paths from noise to data vs. curved diffusion paths. Fewer NFE (function evaluations) needed. Conceptually simpler than score-based models. Basis for Stable Diffusion 3 and Flux.
...
The paper's main claim is summarized in the registry as: Straight-line paths from noise to data vs. curved diffusion paths. Fewer NFE (function evaluations) needed. Conceptually simpler than score-based models. Basis for Stable Diffusion 3 and Flux. The key experiments should therefore isolate **causality**, not merely display a large final score.
...
The registry records the practical significance as: Stable Diffusion 3, Flux, next-generation image generation models This statement explains *why the paper remained relevant*, but exact benchmark values must be taken from the primary tables in [1]. They are intentionally not invented or copied without experimental context here.
...
- [Consistency Models](/papers/consistency-models) — adjacent reading from the same library section.
- [Photorealistic Text-to-Image Diffusion Models with Deep Language Understanding (Imagen)](/papers/photorealistic-text-to-image-diffusion-models-with-deep-language-understanding-imagen) — adjacent reading from the same library section.
...
[1] Lipman, Chen, Ben-Hamu, Nickel, Le. **Flow Matching for Generative Modeling**. 2022.
