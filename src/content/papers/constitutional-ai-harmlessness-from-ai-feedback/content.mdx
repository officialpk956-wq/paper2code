...
Published in 2022 by Bai, Jones, Ndousse, Askell, Chen, DasSarma, Drain, Fort, et al., **Constitutional AI: Harmlessness from AI Feedback** belongs to the LARGE LANGUAGE MODELS part of the Paper2Code library. AI can critique and revise its own outputs using a constitution. Scalable oversight without human raters at every step. RLAIF as alternative to RLHF. Claude's alignment method. [1]
...
- **Constitutional AI:** identify its input, output, learned parameters, and the assumption it adds to the baseline.
- **RLAIF:** identify its input, output, learned parameters, and the assumption it adds to the baseline.
- **AI-generated preference labels:** identify its input, output, learned parameters, and the assumption it adds to the baseline.
- **self-critique:** identify its input, output, learned parameters, and the assumption it adds to the baseline.

Its associated architecture or procedure is **None**. The contribution should be explained as a change to a baseline computation: what new state is introduced, where information flows differently, and which objective trains the change. AI can critique and revise its own outputs using a constitution. Scalable oversight without human raters at every step. RLAIF as alternative to RLHF. Claude's alignment method.
...
The paper's main claim is summarized in the registry as: AI can critique and revise its own outputs using a constitution. Scalable oversight without human raters at every step. RLAIF as alternative to RLHF. Claude's alignment method. The key experiments should therefore isolate **causality**, not merely display a large final score.
...
The registry records the practical significance as: Anthropic's Claude alignment; scalable alignment at lower cost This statement explains *why the paper remained relevant*, but exact benchmark values must be taken from the primary tables in [1]. They are intentionally not invented or copied without experimental context here.
...
Anthropic's Claude alignment; scalable alignment at lower cost To translate that impact into a production decision, measure the complete path rather than an isolated kernel or offline model. Include preprocessing, retrieval or batching, host-device transfer, post-processing, cache behavior, and tail latency.
...
- [LLaMA: Open and Efficient Foundation Language Models](/papers/llama-open-and-efficient-foundation-language-models) — adjacent reading from the same library section.
- [Chain-of-Thought Prompting Elicits Reasoning in Large Language Models](/papers/chain-of-thought-prompting-elicits-reasoning-in-large-language-models) — adjacent reading from the same library section.
...
[1] Bai, Jones, Ndousse, Askell, Chen, DasSarma, Drain, Fort, et al.. **Constitutional AI: Harmlessness from AI Feedback**. 2022.
