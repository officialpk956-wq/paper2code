...
Published in 2015 by Ronneberger, Fischer, Brox, **U-Net: Convolutional Networks for Biomedical Image Segmentation** belongs to the COMPUTER VISION part of the Paper2Code library. Defined the encoder-decoder with skip connections architecture. Trained on very small datasets using data augmentation. Dominant in medical imaging for 10 years. [1]
...
- **Skip connections in encoder-decoder:** identify its input, output, learned parameters, and the assumption it adds to the baseline.
- **precise localization via concatenation:** identify its input, output, learned parameters, and the assumption it adds to the baseline.
- **small dataset training:** identify its input, output, learned parameters, and the assumption it adds to the baseline.

Its associated architecture or procedure is **U-Net**. The contribution should be explained as a change to a baseline computation: what new state is introduced, where information flows differently, and which objective trains the change. Defined the encoder-decoder with skip connections architecture. Trained on very small datasets using data augmentation. Dominant in medical imaging for 10 years.
...
The paper's main claim is summarized in the registry as: Defined the encoder-decoder with skip connections architecture. Trained on very small datasets using data augmentation. Dominant in medical imaging for 10 years. The key experiments should therefore isolate **causality**, not merely display a large final score.
...
The registry records the practical significance as: Standard in radiology AI, pathology; used as backbone in diffusion models This statement explains *why the paper remained relevant*, but exact benchmark values must be taken from the primary tables in [1]. They are intentionally not invented or copied without experimental context here.
...
- [Architecture deep dive](/architectures/u-net) — inspect the corresponding model family and tensor flow.
- [Focal Loss for Dense Object Detection (RetinaNet)](/papers/focal-loss-for-dense-object-detection-retinanet) — adjacent reading from the same library section.
...
[1] Ronneberger, Fischer, Brox. **U-Net: Convolutional Networks for Biomedical Image Segmentation**. 2015.
