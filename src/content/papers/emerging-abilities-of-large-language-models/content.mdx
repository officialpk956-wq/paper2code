# Emerging Abilities of Large Language Models
...
Published in 2022 by Wei, Tay, Bommasani, Raffel, Zoph, Borgeaud, Yogatama, Bosma, Zhou, et al., **Emerging Abilities of Large Language Models** belongs to the LARGE LANGUAGE MODELS part of the Paper2Code library. Documented capabilities that appear abruptly at scale thresholds: arithmetic, chain-of-thought, analogical reasoning. Showed there are qualitative phase transitions in LLM capability. [1]
...
Recurrent and convolutional sequence models compressed long contexts through sequential computation, limiting parallelism and making distant dependencies difficult to preserve. By 2022, researchers had working baselines, but the remaining gap was large enough that simply adding more layers, data, or hardware did not answer the scientific question.
...
- **Emergent abilities:** identify its input, output, learned parameters, and the assumption it adds to the baseline.
- **phase transitions in capabilities:** identify its input, output, learned parameters, and the assumption it adds to the baseline.
- **scale thresholds:** identify its input, output, learned parameters, and the assumption it adds to the baseline.

Its associated architecture or procedure is **None**. The contribution should be explained as a change to a baseline computation: what new state is introduced, where information flows differently, and which objective trains the change. Documented capabilities that appear abruptly at scale thresholds: arithmetic, chain-of-thought, analogical reasoning. Showed there are qualitative phase transitions in LLM capability.
...
The paper's main claim is summarized in the registry as: Documented capabilities that appear abruptly at scale thresholds: arithmetic, chain-of-thought, analogical reasoning. Showed there are qualitative phase transitions in LLM capability. The key experiments should therefore isolate **causality**, not merely display a large final score.
...
The registry records the practical significance as: Justified massive scale investments; shaped AI safety discourse This statement explains *why the paper remained relevant*, but exact benchmark values must be taken from the primary tables in [1]. They are intentionally not invented or copied without experimental context here.
...
- [Mixtral of Experts](/papers/mixtral-of-experts) — adjacent reading from the same library section.
- [RoFormer: Enhanced Transformer with Rotary Position Embedding (RoPE)](/papers/roformer-enhanced-transformer-with-rotary-position-embedding-rope) — adjacent reading from the same library section.
...
[1] Wei, Tay, Bommasani, Raffel, Zoph, Borgeaud, Yogatama, Bosma, Zhou, et al.. **Emerging Abilities of Large Language Models**. 2022.
