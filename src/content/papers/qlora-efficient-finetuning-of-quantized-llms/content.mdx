...
Published in 2023 by Dettmers, Pagnoni, Holtzman, Zettlemoyer, **QLoRA: Efficient Finetuning of Quantized LLMs** belongs to the LARGE LANGUAGE MODELS part of the Paper2Code library. Fine-tune 65B LLaMA on a single 48GB GPU via 4-bit quantization + LoRA + paged attention. Democratized LLM fine-tuning. Guanaco-65B competitive with ChatGPT. [1]
...
- **NF4 quantization:** identify its input, output, learned parameters, and the assumption it adds to the baseline.
- **double quantization:** identify its input, output, learned parameters, and the assumption it adds to the baseline.
- **paged optimizers:** identify its input, output, learned parameters, and the assumption it adds to the baseline.
- **QLoRA pipeline:** identify its input, output, learned parameters, and the assumption it adds to the baseline.

Its associated architecture or procedure is **None**. The contribution should be explained as a change to a baseline computation: what new state is introduced, where information flows differently, and which objective trains the change. Fine-tune 65B LLaMA on a single 48GB GPU via 4-bit quantization + LoRA + paged attention. Democratized LLM fine-tuning. Guanaco-65B competitive with ChatGPT.
...
The paper's main claim is summarized in the registry as: Fine-tune 65B LLaMA on a single 48GB GPU via 4-bit quantization + LoRA + paged attention. Democratized LLM fine-tuning. Guanaco-65B competitive with ChatGPT. The key experiments should therefore isolate **causality**, not merely display a large final score.
...
The registry records the practical significance as: Single-GPU fine-tuning of frontier models; widespread in research and startups This statement explains *why the paper remained relevant*, but exact benchmark values must be taken from the primary tables in [1]. They are intentionally not invented or copied without experimental context here.
...
- [FlashAttention: Fast and Memory-Efficient Exact Attention with IO-Awareness](/papers/flashattention-fast-and-memory-efficient-exact-attention-with-io-awareness) — adjacent reading from the same library section.
...
[1] Dettmers, Pagnoni, Holtzman, Zettlemoyer. **QLoRA: Efficient Finetuning of Quantized LLMs**. 2023.
