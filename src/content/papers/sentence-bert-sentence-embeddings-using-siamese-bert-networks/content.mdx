...
Published in 2019 by Reimers, Gurevych, **Sentence-BERT: Sentence Embeddings using Siamese BERT-Networks** belongs to the NLP part of the Paper2Code library. BERT is too slow for semantic similarity (O(n²) pairs). SBERT generates fixed-size sentence embeddings in a single forward pass enabling 50 million pairs per day vs. 65 hours with BERT. [1]
...
- **mean pooling:** identify its input, output, learned parameters, and the assumption it adds to the baseline.
- **cosine similarity for sentences:** identify its input, output, learned parameters, and the assumption it adds to the baseline.
- **NLI fine-tuning:** identify its input, output, learned parameters, and the assumption it adds to the baseline.

Its associated architecture or procedure is **Sentence-BERT, Siamese network for sentences**. The contribution should be explained as a change to a baseline computation: what new state is introduced, where information flows differently, and which objective trains the change. BERT is too slow for semantic similarity (O(n²) pairs). SBERT generates fixed-size sentence embeddings in a single forward pass enabling 50 million pairs per day vs. 65 hours with BERT.
...
The paper's main claim is summarized in the registry as: BERT is too slow for semantic similarity (O(n²) pairs). SBERT generates fixed-size sentence embeddings in a single forward pass enabling 50 million pairs per day vs. 65 hours with BERT. The key experiments should therefore isolate **causality**, not merely display a large final score.
...
The registry records the practical significance as: Semantic search, RAG retrieval, document clustering — every dense retrieval system This statement explains *why the paper remained relevant*, but exact benchmark values must be taken from the primary tables in [1]. They are intentionally not invented or copied without experimental context here.
...
- [Deep contextualized word representations (ELMo)](/papers/deep-contextualized-word-representations-elmo) — adjacent reading from the same library section.
- [XLNet: Generalized Autoregressive Pretraining for Language Understanding](/papers/xlnet-generalized-autoregressive-pretraining-for-language-understanding) — adjacent reading from the same library section.
...
[1] Reimers, Gurevych. **Sentence-BERT: Sentence Embeddings using Siamese BERT-Networks**. 2019.
