...
Published in 2017 by Hu, Shen, Albanie, Sun, Wu, **Squeeze-and-Excitation Networks (SENet)** belongs to the COMPUTER VISION part of the Paper2Code library. Channel attention via global average pooling → FC → scale. Won ILSVRC 2017. SE blocks add minimal parameters but consistent accuracy gains across architectures. [1]
...
Recurrent and convolutional sequence models compressed long contexts through sequential computation, limiting parallelism and making distant dependencies difficult to preserve. By 2017, researchers had working baselines, but the remaining gap was large enough that simply adding more layers, data, or hardware did not answer the scientific question.
...
- **Channel attention:** identify its input, output, learned parameters, and the assumption it adds to the baseline.
- **squeeze-and-excitation:** identify its input, output, learned parameters, and the assumption it adds to the baseline.
- **feature recalibration:** identify its input, output, learned parameters, and the assumption it adds to the baseline.

Its associated architecture or procedure is **SE block, SENet**. The contribution should be explained as a change to a baseline computation: what new state is introduced, where information flows differently, and which objective trains the change. Channel attention via global average pooling → FC → scale. Won ILSVRC 2017. SE blocks add minimal parameters but consistent accuracy gains across architectures.
...
The paper's main claim is summarized in the registry as: Channel attention via global average pooling → FC → scale. Won ILSVRC 2017. SE blocks add minimal parameters but consistent accuracy gains across architectures. The key experiments should therefore isolate **causality**, not merely display a large final score.
...
The registry records the practical significance as: SE blocks used in EfficientNet, MobileNetV3; channel attention standard in production This statement explains *why the paper remained relevant*, but exact benchmark values must be taken from the primary tables in [1]. They are intentionally not invented or copied without experimental context here.
...
- [Architecture deep dive](/architectures/senet) — inspect the corresponding model family and tensor flow.
- [Inception v1 (GoogLeNet)](/papers/inception-v1-googlenet) — adjacent reading from the same library section.
- [3D Gaussian Splatting for Real-Time Novel View Synthesis](/papers/3d-gaussian-splatting-for-real-time-novel-view-synthesis) — adjacent reading from the same library section.
...
[1] Hu, Shen, Albanie, Sun, Wu. **Squeeze-and-Excitation Networks (SENet)**. 2017.
