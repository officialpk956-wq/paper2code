...
Recurrent and convolutional sequence models compressed long contexts through sequential computation, limiting parallelism and making distant dependencies difficult to preserve. By 2023, researchers had working baselines, but the remaining gap was large enough that simply adding more layers, data, or hardware did not answer the scientific question.
...
- **Sequence parallelism:** identify its input, output, learned parameters, and the assumption it adds to the baseline.
- **ring communication for attention:** identify its input, output, learned parameters, and the assumption it adds to the baseline.
- **blockwise computation for arbitrarily long sequences:** identify its input, output, learned parameters, and the assumption it adds to the baseline.
...
The paper's main claim is summarized in the registry as: Enables training transformers with sequences of millions of tokens by distributing the attention computation across devices in a ring topology. Each device holds a slice of the sequence and computes local attention while simultaneously sending its KV blocks around the ring. Memory per device scales as O(N/k) where k is number of devices. The key experiments should therefore isolate **causality**, not merely display a large final score.
...
The registry records the practical significance as: Used for 1M-token context models. Enables LLM training on long documents without truncation. Google and Anthropic use sequence parallelism variants in production training. This statement explains *why the paper remained relevant*, but exact benchmark values must be taken from the primary tables in [1]. They are intentionally not invented or copied without experimental context here.
...
- [ONNX: Open Neural Network Exchange](/papers/onnx-open-neural-network-exchange) — adjacent reading from the same library section.
...
[1] Liu, Zaharia, Abbeel (Berkeley). **Ring Attention with Blockwise Transformers**. 2023.
