# Align Before You Fuse: A Unified Multimodal Understanding and Generation Model (BLIP)
...
Published in 2022 by Li, Li, Xiong, Hoi, **Align Before You Fuse: A Unified Multimodal Understanding and Generation Model (BLIP)** belongs to the MULTIMODAL part of the Paper2Code library. Introduced MED (Multimodal mixture of Encoder-Decoder) and CapFilt for noisy web data. Bootstrapping with captioner + filter improved training data quality. Defined the BLIP training paradigm. [1]
...
- **Unified understanding + generation:** identify its input, output, learned parameters, and the assumption it adds to the baseline.
- **bootstrapping noisy web data:** identify its input, output, learned parameters, and the assumption it adds to the baseline.
- **image-text contrastive + matching + generation:** identify its input, output, learned parameters, and the assumption it adds to the baseline.

Its associated architecture or procedure is **BLIP, MED (Multimodal Encoder-Decoder), CapFilt**. The contribution should be explained as a change to a baseline computation: what new state is introduced, where information flows differently, and which objective trains the change. Introduced MED (Multimodal mixture of Encoder-Decoder) and CapFilt for noisy web data. Bootstrapping with captioner + filter improved training data quality. Defined the BLIP training paradigm.
...
The paper's main claim is summarized in the registry as: Introduced MED (Multimodal mixture of Encoder-Decoder) and CapFilt for noisy web data. Bootstrapping with captioner + filter improved training data quality. Defined the BLIP training paradigm. The key experiments should therefore isolate **causality**, not merely display a large final score.
...
The registry records the practical significance as: Salesforce production multimodal systems This statement explains *why the paper remained relevant*, but exact benchmark values must be taken from the primary tables in [1]. They are intentionally not invented or copied without experimental context here.
...
Salesforce production multimodal systems To translate that impact into a production decision, measure the complete path rather than an isolated kernel or offline model. Include preprocessing, retrieval or batching, host-device transfer, post-processing, cache behavior, and tail latency.
...
- [Gemini: A Family of Highly Capable Multimodal Models](/papers/gemini-a-family-of-highly-capable-multimodal-models) — adjacent reading from the same library section.
...
[1] Li, Li, Xiong, Hoi. **Align Before You Fuse: A Unified Multimodal Understanding and Generation Model (BLIP)**. 2022.
