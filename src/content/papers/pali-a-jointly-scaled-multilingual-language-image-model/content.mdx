...
Published in 2022 by Chen, Parekh, Pang, Du, Tsai, Zhou, Humanistika, Goel, et al., **PaLI: A Jointly-Scaled Multilingual Language-Image Model** belongs to the MULTIMODAL part of the Paper2Code library. Jointly scale vision encoder and language model. 17B parameters across 100+ languages. Best multimodal accuracy of its time. Showed multilingual multimodal is achievable. [1]
...
- **Joint image-language scaling:** identify its input, output, learned parameters, and the assumption it adds to the baseline.
- **multilingual multimodal:** identify its input, output, learned parameters, and the assumption it adds to the baseline.
- **shared encoder:** identify its input, output, learned parameters, and the assumption it adds to the baseline.

Its associated architecture or procedure is **PaLI**. The contribution should be explained as a change to a baseline computation: what new state is introduced, where information flows differently, and which objective trains the change. Jointly scale vision encoder and language model. 17B parameters across 100+ languages. Best multimodal accuracy of its time. Showed multilingual multimodal is achievable.
...
The paper's main claim is summarized in the registry as: Jointly scale vision encoder and language model. 17B parameters across 100+ languages. Best multimodal accuracy of its time. Showed multilingual multimodal is achievable. The key experiments should therefore isolate **causality**, not merely display a large final score.
...
The registry records the practical significance as: Google Lens multilingual capabilities This statement explains *why the paper remained relevant*, but exact benchmark values must be taken from the primary tables in [1]. They are intentionally not invented or copied without experimental context here.
...
Google Lens multilingual capabilities To translate that impact into a production decision, measure the complete path rather than an isolated kernel or offline model. Include preprocessing, retrieval or batching, host-device transfer, post-processing, cache behavior, and tail latency.
...
[1] Chen, Parekh, Pang, Du, Tsai, Zhou, Humanistika, Goel, et al.. **PaLI: A Jointly-Scaled Multilingual Language-Image Model**. 2022.
