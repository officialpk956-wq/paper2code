...
Published in 1986 by Rumelhart, Hinton, Williams, **Learning Representations by Back-Propagating Errors** belongs to the FOUNDATIONS part of the Paper2Code library. Invented the algorithm that trains virtually every neural network ever built. Without backpropagation, deep learning as a field does not exist. [1]
...
- **Backpropagation:** identify its input, output, learned parameters, and the assumption it adds to the baseline.
- **chain rule for neural networks:** identify its input, output, learned parameters, and the assumption it adds to the baseline.
- **gradient-based weight updates:** identify its input, output, learned parameters, and the assumption it adds to the baseline.

Its associated architecture or procedure is **Multi-layer perceptron training procedure**. The contribution should be explained as a change to a baseline computation: what new state is introduced, where information flows differently, and which objective trains the change. Invented the algorithm that trains virtually every neural network ever built. Without backpropagation, deep learning as a field does not exist.
...
The paper's main claim is summarized in the registry as: Invented the algorithm that trains virtually every neural network ever built. Without backpropagation, deep learning as a field does not exist. The key experiments should therefore isolate **causality**, not merely display a large final score.
...
The registry records the practical significance as: Foundational to every deep learning framework — PyTorch, TensorFlow, JAX all implement this algorithm This statement explains *why the paper remained relevant*, but exact benchmark values must be taken from the primary tables in [1]. They are intentionally not invented or copied without experimental context here.
...
- [Attention Is All You Need](/papers/attention-is-all-you-need) — adjacent reading from the same library section.
