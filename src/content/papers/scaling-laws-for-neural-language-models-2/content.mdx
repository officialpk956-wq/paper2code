...
Published in 2020 by Kaplan, McCandlish, Henighan, Brown, Chess, Child, Gray, Radford, Wu, Amodei (OpenAI), **Scaling Laws for Neural Language Models** belongs to the SYSTEMS part of the Paper2Code library. Empirically establishes that language model loss follows a power law with respect to model parameters, dataset size, and compute — each independently. Shows that for a fixed compute budget, the optimal strategy is to scale parameters and data proportionally. This paper justified the investment in GPT-3 and every large model since. [1]
...
Tabular methods and unstable function approximation struggled with large observation spaces, off-policy data, and high-variance policy gradients. By 2020, researchers had working baselines, but the remaining gap was large enough that simply adding more layers, data, or hardware did not answer the scientific question.
...
- **Neural scaling laws:** identify its input, output, learned parameters, and the assumption it adds to the baseline.
- **compute-optimal frontier:** identify its input, output, learned parameters, and the assumption it adds to the baseline.
- **irreducible loss:** identify its input, output, learned parameters, and the assumption it adds to the baseline.
- **critical batch size formula:** identify its input, output, learned parameters, and the assumption it adds to the baseline.

Its associated architecture or procedure is **N/A — analytical result over transformer architecture space**. The contribution should be explained as a change to a baseline computation: what new state is introduced, where information flows differently, and which objective trains the change. Empirically establishes that language model loss follows a power law with respect to model parameters, dataset size, and compute — each independently. Shows that for a fixed compute budget, the optimal strategy is to scale parameters and data proportionally. This paper justified the investment in GPT-3 and every large model since.
...
The paper's main claim is summarized in the registry as: Empirically establishes that language model loss follows a power law with respect to model parameters, dataset size, and compute — each independently. Shows that for a fixed compute budget, the optimal strategy is to scale parameters and data proportionally. This paper justified the investment in GPT-3 and every large model since. The key experiments should therefore isolate **causality**, not merely display a large final score.
...
[1] Kaplan, McCandlish, Henighan, Brown, Chess, Child, Gray, Radford, Wu, Amodei (OpenAI). **Scaling Laws for Neural Language Models**. 2020.
