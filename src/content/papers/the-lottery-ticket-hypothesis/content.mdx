...
Published in 2019 by Frankle, Carlin, **The Lottery Ticket Hypothesis** belongs to the FOUNDATIONS part of the Paper2Code library. Showed that large networks contain small sparse subnetworks ("winning tickets") that can train to full accuracy. Foundational for network pruning, efficient neural networks, and mechanistic interpretability. [1]
...
- **Lottery ticket hypothesis:** identify its input, output, learned parameters, and the assumption it adds to the baseline.
- **winning tickets:** identify its input, output, learned parameters, and the assumption it adds to the baseline.
- **iterative magnitude pruning:** identify its input, output, learned parameters, and the assumption it adds to the baseline.
- **mask rewinding:** identify its input, output, learned parameters, and the assumption it adds to the baseline.

Its associated architecture or procedure is **None**. The contribution should be explained as a change to a baseline computation: what new state is introduced, where information flows differently, and which objective trains the change. Showed that large networks contain small sparse subnetworks ("winning tickets") that can train to full accuracy. Foundational for network pruning, efficient neural networks, and mechanistic interpretability.
...
The paper's main claim is summarized in the registry as: Showed that large networks contain small sparse subnetworks ("winning tickets") that can train to full accuracy. Foundational for network pruning, efficient neural networks, and mechanistic interpretability. The key experiments should therefore isolate **causality**, not merely display a large final score.
...
The registry records the practical significance as: Motivates neural network compression; used in on-device AI deployment This statement explains *why the paper remained relevant*, but exact benchmark values must be taken from the primary tables in [1]. They are intentionally not invented or copied without experimental context here.
...
[1] Frankle, Carlin. **The Lottery Ticket Hypothesis**. 2019.
