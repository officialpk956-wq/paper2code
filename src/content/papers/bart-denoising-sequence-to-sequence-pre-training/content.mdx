...
Published in 2019 by Lewis, Liu, Goyal, Ghazvininejad, Mohamed, Levy, Stoyanov, Zettlemoyer, **BART: Denoising Sequence-to-Sequence Pre-training** belongs to the NLP part of the Paper2Code library. Encoder-decoder trained with denoising (text infilling, sentence permutation). Best model for generation tasks at release — summarization, translation, QA. Combines BERT's understanding with GPT's generation. [1]
...
- **Denoising pre-training:** identify its input, output, learned parameters, and the assumption it adds to the baseline.
- **text infilling:** identify its input, output, learned parameters, and the assumption it adds to the baseline.
- **multiple noise functions:** identify its input, output, learned parameters, and the assumption it adds to the baseline.
- **seq2seq fine-tuning:** identify its input, output, learned parameters, and the assumption it adds to the baseline.

Its associated architecture or procedure is **BART**. The contribution should be explained as a change to a baseline computation: what new state is introduced, where information flows differently, and which objective trains the change. Encoder-decoder trained with denoising (text infilling, sentence permutation). Best model for generation tasks at release — summarization, translation, QA. Combines BERT's understanding with GPT's generation.
...
The paper's main claim is summarized in the registry as: Encoder-decoder trained with denoising (text infilling, sentence permutation). Best model for generation tasks at release — summarization, translation, QA. Combines BERT's understanding with GPT's generation. The key experiments should therefore isolate **causality**, not merely display a large final score.
...
The registry records the practical significance as: News summarization, abstractive QA systems This statement explains *why the paper remained relevant*, but exact benchmark values must be taken from the primary tables in [1]. They are intentionally not invented or copied without experimental context here.
...
News summarization, abstractive QA systems To translate that impact into a production decision, measure the complete path rather than an isolated kernel or offline model. Include preprocessing, retrieval or batching, host-device transfer, post-processing, cache behavior, and tail latency.
...
- [Architecture deep dive](/architectures/bart) — inspect the corresponding model family and tensor flow.
- [Exploring the Limits of Transfer Learning with T5](/papers/exploring-the-limits-of-transfer-learning-with-t5) — adjacent reading from the same library section.
- [GloVe: Global Vectors for Word Representation](/papers/glove-global-vectors-for-word-representation) — adjacent reading from the same library section.
...
[1] Lewis, Liu, Goyal, Ghazvininejad, Mohamed, Levy, Stoyanov, Zettlemoyer. **BART: Denoising Sequence-to-Sequence Pre-training**. 2019.
