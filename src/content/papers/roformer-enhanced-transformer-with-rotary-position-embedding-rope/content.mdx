...
Published in 2021 by Su, Lu, Pan, Murtadha, Wen, Liu, **RoFormer: Enhanced Transformer with Rotary Position Embedding (RoPE)** belongs to the LARGE LANGUAGE MODELS part of the Paper2Code library. Rotary positional embeddings encode absolute position via rotation while enabling relative position computation in attention. Naturally extends to longer contexts. Now standard in LLaMA, PaLM, GPT-Neo. [1]
...
- **Rotary position embedding:** identify its input, output, learned parameters, and the assumption it adds to the baseline.
- **relative position in dot product:** identify its input, output, learned parameters, and the assumption it adds to the baseline.
- **extrapolation:** identify its input, output, learned parameters, and the assumption it adds to the baseline.

Its associated architecture or procedure is **RoFormer**. The contribution should be explained as a change to a baseline computation: what new state is introduced, where information flows differently, and which objective trains the change. Rotary positional embeddings encode absolute position via rotation while enabling relative position computation in attention. Naturally extends to longer contexts. Now standard in LLaMA, PaLM, GPT-Neo.
...
The paper's main claim is summarized in the registry as: Rotary positional embeddings encode absolute position via rotation while enabling relative position computation in attention. Naturally extends to longer contexts. Now standard in LLaMA, PaLM, GPT-Neo. The key experiments should therefore isolate **causality**, not merely display a large final score.
...
The registry records the practical significance as: Default positional encoding in LLaMA, GPT-NeoX, PaLM 2, most modern LLMs This statement explains *why the paper remained relevant*, but exact benchmark values must be taken from the primary tables in [1]. They are intentionally not invented or copied without experimental context here.
...
- [Emerging Abilities of Large Language Models](/papers/emerging-abilities-of-large-language-models) — adjacent reading from the same library section.
- [ZeRO: Memory Optimizations Toward Training Trillion Parameter Models](/papers/zero-memory-optimizations-toward-training-trillion-parameter-models) — adjacent reading from the same library section.
...
[1] Su, Lu, Pan, Murtadha, Wen, Liu. **RoFormer: Enhanced Transformer with Rotary Position Embedding (RoPE)**. 2021.
