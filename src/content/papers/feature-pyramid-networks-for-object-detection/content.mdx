...
Published in 2016 by Lin, Dollár, Girshick, He, Hariharan, Belongie, **Feature Pyramid Networks for Object Detection** belongs to the COMPUTER VISION part of the Paper2Code library. Solved multi-scale object detection by building a feature pyramid with lateral connections. Small objects detected from high-resolution low-level features; large objects from low-resolution high-level features. [1]
...
- **Top-down feature hierarchy:** identify its input, output, learned parameters, and the assumption it adds to the baseline.
- **lateral connections:** identify its input, output, learned parameters, and the assumption it adds to the baseline.
- **multi-scale feature maps:** identify its input, output, learned parameters, and the assumption it adds to the baseline.

Its associated architecture or procedure is **Feature Pyramid Network (FPN)**. The contribution should be explained as a change to a baseline computation: what new state is introduced, where information flows differently, and which objective trains the change. Solved multi-scale object detection by building a feature pyramid with lateral connections. Small objects detected from high-resolution low-level features; large objects from low-resolution high-level features.
...
The paper's main claim is summarized in the registry as: Solved multi-scale object detection by building a feature pyramid with lateral connections. Small objects detected from high-resolution low-level features; large objects from low-resolution high-level features. The key experiments should therefore isolate **causality**, not merely display a large final score.
...
The registry records the practical significance as: Default backbone for object detection in Detectron2; in every production detector This statement explains *why the paper remained relevant*, but exact benchmark values must be taken from the primary tables in [1]. They are intentionally not invented or copied without experimental context here.
...
- [Architecture deep dive](/architectures/fpn) — inspect the corresponding model family and tensor flow.
...
[1] Lin, Dollár, Girshick, He, Hariharan, Belongie. **Feature Pyramid Networks for Object Detection**. 2016.
