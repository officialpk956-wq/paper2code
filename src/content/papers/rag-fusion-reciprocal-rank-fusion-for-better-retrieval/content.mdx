# RAG-Fusion: Reciprocal Rank Fusion for Better Retrieval
...
Published in 2023 by Shi, Min, Yasunaga, Seo, James, Lewis, Zettlemoyer, Yih (various), **RAG-Fusion: Reciprocal Rank Fusion for Better Retrieval** belongs to the SYSTEMS part of the Paper2Code library. A single query to a retriever captures one perspective of information need. RAG-Fusion generates multiple query variations using the LLM, retrieves documents for each, and uses Reciprocal Rank Fusion to merge the result sets into a single ranked list. Captures more facets of information need without changing the retriever. [1]
...
- **Query fan-out:** identify its input, output, learned parameters, and the assumption it adds to the baseline.
- **reciprocal rank fusion (RRF):** identify its input, output, learned parameters, and the assumption it adds to the baseline.
- **multi-query retrieval:** identify its input, output, learned parameters, and the assumption it adds to the baseline.

Its associated architecture or procedure is **RAG-Fusion pipeline**. The contribution should be explained as a change to a baseline computation: what new state is introduced, where information flows differently, and which objective trains the change. A single query to a retriever captures one perspective of information need. RAG-Fusion generates multiple query variations using the LLM, retrieves documents for each, and uses Reciprocal Rank Fusion to merge the result sets into a single ranked list. Captures more facets of information need without changing the retriever.
...
The paper's main claim is summarized in the registry as: A single query to a retriever captures one perspective of information need. RAG-Fusion generates multiple query variations using the LLM, retrieves documents for each, and uses Reciprocal Rank Fusion to merge the result sets into a single ranked list. Captures more facets of information need without changing the retriever. The key experiments should therefore isolate **causality**, not merely display a large final score.
...
The registry records the practical significance as: Widely used in production RAG pipelines as a no-training improvement. LangChain and LlamaIndex implement multi-query retrieval with RRF natively. This statement explains *why the paper remained relevant*, but exact benchmark values must be taken from the primary tables in [1]. They are intentionally not invented or copied without experimental context here.
...
[1] Shi, Min, Yasunaga, Seo, James, Lewis, Zettlemoyer, Yih (various). **RAG-Fusion: Reciprocal Rank Fusion for Better Retrieval**. 2023.
