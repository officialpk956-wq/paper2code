...
Published in 2017 by Lin, Goyal, Girshick, He, Dollár, **Focal Loss for Dense Object Detection (RetinaNet)** belongs to the COMPUTER VISION part of the Paper2Code library. Solved class imbalance in single-stage detectors with Focal Loss — down-weights easy negatives. Made single-stage detectors competitive with two-stage at accuracy. [1]
...
- **Focal loss:** identify its input, output, learned parameters, and the assumption it adds to the baseline.
- **class imbalance in detection:** identify its input, output, learned parameters, and the assumption it adds to the baseline.
- **hard example mining alternative:** identify its input, output, learned parameters, and the assumption it adds to the baseline.

Its associated architecture or procedure is **RetinaNet**. The contribution should be explained as a change to a baseline computation: what new state is introduced, where information flows differently, and which objective trains the change. Solved class imbalance in single-stage detectors with Focal Loss — down-weights easy negatives. Made single-stage detectors competitive with two-stage at accuracy.
...
The paper's main claim is summarized in the registry as: Solved class imbalance in single-stage detectors with Focal Loss — down-weights easy negatives. Made single-stage detectors competitive with two-stage at accuracy. The key experiments should therefore isolate **causality**, not merely display a large final score.
...
The registry records the practical significance as: RetinaNet widely used in production; Focal Loss used in NLP and other domains This statement explains *why the paper remained relevant*, but exact benchmark values must be taken from the primary tables in [1]. They are intentionally not invented or copied without experimental context here.
...
- [Architecture deep dive](/architectures/retinanet) — inspect the corresponding model family and tensor flow.
- [Feature Pyramid Networks for Object Detection](/papers/feature-pyramid-networks-for-object-detection) — adjacent reading from the same library section.
...
[1] Lin, Goyal, Girshick, He, Dollár. **Focal Loss for Dense Object Detection (RetinaNet)**. 2017.
