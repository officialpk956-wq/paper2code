...
Published in 2020 by Rasley, Rajbhandari, Ruwase, He, **DeepSpeed: System Optimizations Enable Training Deep Learning Models with Over 100 Billion Parameters** belongs to the SYSTEMS part of the Paper2Code library. ZeRO optimizer + mixed precision + gradient checkpointing in one system. Enabled training of models that couldn't fit in memory otherwise. Microsoft's production training framework. [1]
...
- **ZeRO optimization:** identify its input, output, learned parameters, and the assumption it adds to the baseline.
- **model compression:** identify its input, output, learned parameters, and the assumption it adds to the baseline.
- **progressive layer dropping:** identify its input, output, learned parameters, and the assumption it adds to the baseline.
- **pipeline parallelism:** identify its input, output, learned parameters, and the assumption it adds to the baseline.

Its associated architecture or procedure is **DeepSpeed system**. The contribution should be explained as a change to a baseline computation: what new state is introduced, where information flows differently, and which objective trains the change. ZeRO optimizer + mixed precision + gradient checkpointing in one system. Enabled training of models that couldn't fit in memory otherwise. Microsoft's production training framework.
...
The paper's main claim is summarized in the registry as: ZeRO optimizer + mixed precision + gradient checkpointing in one system. Enabled training of models that couldn't fit in memory otherwise. Microsoft's production training framework. The key experiments should therefore isolate **causality**, not merely display a large final score.
...
The registry records the practical significance as: Used for GPT-3 training scale; powers large-scale training at Microsoft, NVIDIA This statement explains *why the paper remained relevant*, but exact benchmark values must be taken from the primary tables in [1]. They are intentionally not invented or copied without experimental context here.
...
- [PyTorch: An Imperative Style, High-Performance Deep Learning Library](/papers/pytorch-an-imperative-style-high-performance-deep-learning-library) — adjacent reading from the same library section.
...
[1] Rasley, Rajbhandari, Ruwase, He. **DeepSpeed: System Optimizations Enable Training Deep Learning Models with Over 100 Billion Parameters**. 2020.
