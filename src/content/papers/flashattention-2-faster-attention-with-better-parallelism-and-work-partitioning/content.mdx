...
Published in 2023 by Dao (Princeton), **FlashAttention-2: Faster Attention with Better Parallelism and Work Partitioning** belongs to the SYSTEMS part of the Paper2Code library. Revised the tiling strategy from FlashAttention-1, reducing non-matmul FLOPs and improving thread block utilization. Achieves 50–73% of theoretical A100 MFU for attention — 2× faster than FlashAttention-1. Non-causal and multi-head attention coverage added. [1]
...
- **Work partitioning:** identify its input, output, learned parameters, and the assumption it adds to the baseline.
- **warp-level specialization:** identify its input, output, learned parameters, and the assumption it adds to the baseline.
- **non-causal masking in tiled kernels:** identify its input, output, learned parameters, and the assumption it adds to the baseline.

Its associated architecture or procedure is **A task-specific realization of the paper's central mechanism**. The contribution should be explained as a change to a baseline computation: what new state is introduced, where information flows differently, and which objective trains the change. Revised the tiling strategy from FlashAttention-1, reducing non-matmul FLOPs and improving thread block utilization. Achieves 50–73% of theoretical A100 MFU for attention — 2× faster than FlashAttention-1. Non-causal and multi-head attention coverage added.
...
The paper's main claim is summarized in the registry as: Revised the tiling strategy from FlashAttention-1, reducing non-matmul FLOPs and improving thread block utilization. Achieves 50–73% of theoretical A100 MFU for attention — 2× faster than FlashAttention-1. Non-causal and multi-head attention coverage added. The key experiments should therefore isolate **causality**, not merely display a large final score.
...
The registry records the practical significance as: Dropped directly into transformers as a replacement for FlashAttention-1. Enabled 128K context models at reasonable training cost. Standard in vLLM, TGI, SGLang. This statement explains *why the paper remained relevant*, but exact benchmark values must be taken from the primary tables in [1]. They are intentionally not invented or copied without experimental context here.
...
[1] Dao (Princeton). **FlashAttention-2: Faster Attention with Better Parallelism and Work Partitioning**. 2023.
