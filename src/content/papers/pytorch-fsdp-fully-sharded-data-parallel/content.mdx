...
Published in 2023 by Zhao, Gu, Varma, Luo, Huang, Xu, Wright, Shojanazeri, Ott, Shleifer, Rayner, Ma, Nguyen, Chauhan, Hao, Mathews, Chintala (Meta), **PyTorch FSDP: Fully Sharded Data Parallel** belongs to the SYSTEMS part of the Paper2Code library. FSDP is PyTorch-native ZeRO-3: it shards model parameters, gradients, and optimizer states across data-parallel workers, and all-gathers them just-in-time during forward/backward passes. Unlike DeepSpeed ZeRO, it is fully integrated into PyTorch's autograd and module system. Used to train LLaMA 1/2/3, OPT, and other Meta models. [1]
...
- **Lazy all-gather:** identify its input, output, learned parameters, and the assumption it adds to the baseline.
- **backward prefetching:** identify its input, output, learned parameters, and the assumption it adds to the baseline.
- **parameter CPU offloading:** identify its input, output, learned parameters, and the assumption it adds to the baseline.
- **activation checkpointing with sharding:** identify its input, output, learned parameters, and the assumption it adds to the baseline.

Its associated architecture or procedure is **A task-specific realization of the paper's central mechanism**. The contribution should be explained as a change to a baseline computation: what new state is introduced, where information flows differently, and which objective trains the change. FSDP is PyTorch-native ZeRO-3: it shards model parameters, gradients, and optimizer states across data-parallel workers, and all-gathers them just-in-time during forward/backward passes. Unlike DeepSpeed ZeRO, it is fully integrated into PyTorch's autograd and module system. Used to train LLaMA 1/2/3, OPT, and other Meta models.
...
The paper's main claim is summarized in the registry as: FSDP is PyTorch-native ZeRO-3: it shards model parameters, gradients, and optimizer states across data-parallel workers, and all-gathers them just-in-time during forward/backward passes. Unlike DeepSpeed ZeRO, it is fully integrated into PyTorch's autograd and module system. Used to train LLaMA 1/2/3, OPT, and other Meta models. The key experiments should therefore isolate **causality**, not merely display a large final score.
...
The registry records the practical significance as: Standard for distributed training at Meta. LLaMA 3 training used FSDP. Natively supported in Hugging Face Accelerate and TorchTitan. This statement explains *why the paper remained relevant*, but exact benchmark values must be taken from the primary tables in [1]. They are intentionally not invented or copied without experimental context here.
...
- [Ring Attention with Blockwise Transformers](/papers/ring-attention-with-blockwise-transformers) — adjacent reading from the same library section.
...
[1] Zhao, Gu, Varma, Luo, Huang, Xu, Wright, Shojanazeri, Ott, Shleifer, Rayner, Ma, Nguyen, Chauhan, Hao, Mathews, Chintala (Meta). **PyTorch FSDP: Fully Sharded Data Parallel**. 2023.
