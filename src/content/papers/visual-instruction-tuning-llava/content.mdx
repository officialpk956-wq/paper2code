...
Published in 2023 by Liu, Li, Wu, Lee, **Visual Instruction Tuning (LLaVA)** belongs to the MULTIMODAL part of the Paper2Code library. GPT-4 generates visual instruction data → train LLaMA with simple linear projection from CLIP. First fully open-source visual instruction tuning. LLaVA-1.5 matches GPT-4V on several benchmarks. [1]
...
- **GPT-4-generated multimodal data:** identify its input, output, learned parameters, and the assumption it adds to the baseline.
- **linear projector:** identify its input, output, learned parameters, and the assumption it adds to the baseline.

Its associated architecture or procedure is **LLaVA**. The contribution should be explained as a change to a baseline computation: what new state is introduced, where information flows differently, and which objective trains the change. GPT-4 generates visual instruction data → train LLaMA with simple linear projection from CLIP. First fully open-source visual instruction tuning. LLaVA-1.5 matches GPT-4V on several benchmarks.
...
The paper's main claim is summarized in the registry as: GPT-4 generates visual instruction data → train LLaMA with simple linear projection from CLIP. First fully open-source visual instruction tuning. LLaVA-1.5 matches GPT-4V on several benchmarks. The key experiments should therefore isolate **causality**, not merely display a large final score.
...
The registry records the practical significance as: Open-source multimodal LLM ecosystem; most fine-tuned multimodal model This statement explains *why the paper remained relevant*, but exact benchmark values must be taken from the primary tables in [1]. They are intentionally not invented or copied without experimental context here.
...
- [Architecture deep dive](/architectures/llava) — inspect the corresponding model family and tensor flow.
...
[1] Liu, Li, Wu, Lee. **Visual Instruction Tuning (LLaVA)**. 2023.
