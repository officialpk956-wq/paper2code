...
Published in 2023 by Wu, Bansal, Zhang, Wu, Zhang, Zhu, Li, Jiang, Zhang, Wang, **AutoGen: Enabling Next-Gen LLM Applications via Multi-Agent Conversation** belongs to the AGENTS part of the Paper2Code library. Framework for LLMs to converse and collaborate as agents. Human proxies, automated code execution, flexible conversation patterns. Became the most-used multi-agent framework. [1]
...
- **Conversable agents:** identify its input, output, learned parameters, and the assumption it adds to the baseline.
- **human-in-the-loop agents:** identify its input, output, learned parameters, and the assumption it adds to the baseline.
- **automated code execution loop:** identify its input, output, learned parameters, and the assumption it adds to the baseline.

Its associated architecture or procedure is **AutoGen framework**. The contribution should be explained as a change to a baseline computation: what new state is introduced, where information flows differently, and which objective trains the change. Framework for LLMs to converse and collaborate as agents. Human proxies, automated code execution, flexible conversation patterns. Became the most-used multi-agent framework.
...
The paper's main claim is summarized in the registry as: Framework for LLMs to converse and collaborate as agents. Human proxies, automated code execution, flexible conversation patterns. Became the most-used multi-agent framework. The key experiments should therefore isolate **causality**, not merely display a large final score.
...
The registry records the practical significance as: Microsoft product research; widely used for multi-agent AI systems This statement explains *why the paper remained relevant*, but exact benchmark values must be taken from the primary tables in [1]. They are intentionally not invented or copied without experimental context here.
...
[1] Wu, Bansal, Zhang, Wu, Zhang, Zhu, Li, Jiang, Zhang, Wang. **AutoGen: Enabling Next-Gen LLM Applications via Multi-Agent Conversation**. 2023.
