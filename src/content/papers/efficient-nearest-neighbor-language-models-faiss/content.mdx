# Efficient Nearest Neighbor Language Models (FAISS)
...
Published in 2019 by Johnson, Douze, Jégou, **Efficient Nearest Neighbor Language Models (FAISS)** belongs to the RAG part of the Paper2Code library. Library for efficient similarity search over billions of vectors. IVF, HNSW, Product Quantization implementations at industrial scale. The infrastructure layer of all vector retrieval. [1]
...
- **IVF index:** identify its input, output, learned parameters, and the assumption it adds to the baseline.
- **HNSW:** identify its input, output, learned parameters, and the assumption it adds to the baseline.
- **product quantization:** identify its input, output, learned parameters, and the assumption it adds to the baseline.
- **GPU-accelerated ANN:** identify its input, output, learned parameters, and the assumption it adds to the baseline.

Its associated architecture or procedure is **FAISS library**. The contribution should be explained as a change to a baseline computation: what new state is introduced, where information flows differently, and which objective trains the change. Library for efficient similarity search over billions of vectors. IVF, HNSW, Product Quantization implementations at industrial scale. The infrastructure layer of all vector retrieval.
...
The paper's main claim is summarized in the registry as: Library for efficient similarity search over billions of vectors. IVF, HNSW, Product Quantization implementations at industrial scale. The infrastructure layer of all vector retrieval. The key experiments should therefore isolate **causality**, not merely display a large final score.
...
The registry records the practical significance as: Used by Meta, Google, OpenAI; every production vector search system This statement explains *why the paper remained relevant*, but exact benchmark values must be taken from the primary tables in [1]. They are intentionally not invented or copied without experimental context here.
...
- [Leveraging Passage Retrieval with Generative Models for Open Domain QA (FiD)](/papers/leveraging-passage-retrieval-with-generative-models-for-open-domain-qa-fid) — adjacent reading from the same library section.
- [Efficient and Robust Approximate Nearest Neighbor Search Using HNSW](/papers/efficient-and-robust-approximate-nearest-neighbor-search-using-hnsw) — adjacent reading from the same library section.
...
[1] Johnson, Douze, Jégou. **Efficient Nearest Neighbor Language Models (FAISS)**. 2019.
