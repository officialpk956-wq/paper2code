...
Published in 2022 by Wei, Wang, Schuurmans, Bosma, Ichter, Xia, Chi, Le, Zhou, **Chain-of-Thought Prompting Elicits Reasoning in Large Language Models** belongs to the LARGE LANGUAGE MODELS part of the Paper2Code library. Adding "Let's think step by step" dramatically improves LLM reasoning on math, logic, and complex tasks. Emergent only at sufficient scale. Sparked the entire prompting engineering field. [1]
...
Single-turn language models could describe actions but lacked a grounded loop for executing them, observing consequences, and recovering from errors. By 2022, researchers had working baselines, but the remaining gap was large enough that simply adding more layers, data, or hardware did not answer the scientific question.
...
- **Chain-of-thought prompting:** identify its input, output, learned parameters, and the assumption it adds to the baseline.
- **reasoning traces:** identify its input, output, learned parameters, and the assumption it adds to the baseline.
- **few-shot CoT:** identify its input, output, learned parameters, and the assumption it adds to the baseline.
- **emergent reasoning:** identify its input, output, learned parameters, and the assumption it adds to the baseline.

Its associated architecture or procedure is **None**. The contribution should be explained as a change to a baseline computation: what new state is introduced, where information flows differently, and which objective trains the change. Adding "Let's think step by step" dramatically improves LLM reasoning on math, logic, and complex tasks. Emergent only at sufficient scale. Sparked the entire prompting engineering field.
...
The paper's main claim is summarized in the registry as: Adding "Let's think step by step" dramatically improves LLM reasoning on math, logic, and complex tasks. Emergent only at sufficient scale. Sparked the entire prompting engineering field. The key experiments should therefore isolate **causality**, not merely display a large final score.
...
The registry records the practical significance as: CoT is standard in every production LLM prompt for reasoning tasks This statement explains *why the paper remained relevant*, but exact benchmark values must be taken from the primary tables in [1]. They are intentionally not invented or copied without experimental context here.
...
- [Constitutional AI: Harmlessness from AI Feedback](/papers/constitutional-ai-harmlessness-from-ai-feedback) — adjacent reading from the same library section.
- [LoRA: Low-Rank Adaptation of Large Language Models](/papers/lora-low-rank-adaptation-of-large-language-models) — adjacent reading from the same library section.
...
[1] Wei, Wang, Schuurmans, Bosma, Ichter, Xia, Chi, Le, Zhou. **Chain-of-Thought Prompting Elicits Reasoning in Large Language Models**. 2022.
