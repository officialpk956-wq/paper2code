...
Published in 2016 by Silver, Huang, Maddison, Guez, Sifre, van den Driessche, Schrittwieser, et al., **Mastering the Game of Go with Deep Neural Networks and Tree Search (AlphaGo)** belongs to the REINFORCEMENT LEARNING part of the Paper2Code library. Beat world champion Go player. First superhuman performance in a game considered too complex for AI. Policy network + value network + MCTS = the AlphaGo paradigm. [1]
...
Tabular methods and unstable function approximation struggled with large observation spaces, off-policy data, and high-variance policy gradients. By 2016, researchers had working baselines, but the remaining gap was large enough that simply adding more layers, data, or hardware did not answer the scientific question.
...
- **Policy network:** identify its input, output, learned parameters, and the assumption it adds to the baseline.
- **value network:** identify its input, output, learned parameters, and the assumption it adds to the baseline.
- **MCTS with neural evaluation:** identify its input, output, learned parameters, and the assumption it adds to the baseline.
- **self-play:** identify its input, output, learned parameters, and the assumption it adds to the baseline.

Its associated architecture or procedure is **AlphaGo (policy + value network + MCTS)**. The contribution should be explained as a change to a baseline computation: what new state is introduced, where information flows differently, and which objective trains the change. Beat world champion Go player. First superhuman performance in a game considered too complex for AI. Policy network + value network + MCTS = the AlphaGo paradigm.
...
The paper's main claim is summarized in the registry as: Beat world champion Go player. First superhuman performance in a game considered too complex for AI. Policy network + value network + MCTS = the AlphaGo paradigm. The key experiments should therefore isolate **causality**, not merely display a large final score.
...
The registry records the practical significance as: Landmark in AI capability; demonstrated game-playing AI could reach superhuman level This statement explains *why the paper remained relevant*, but exact benchmark values must be taken from the primary tables in [1]. They are intentionally not invented or copied without experimental context here.
...
- [Architecture deep dive](/architectures/alphago) — inspect the corresponding model family and tensor flow.
- [Playing Atari with Deep Reinforcement Learning (DQN)](/papers/playing-atari-with-deep-reinforcement-learning-dqn) — adjacent reading from the same library section.
- [Mastering Chess and Shogi by Self-Play with AlphaZero](/papers/mastering-chess-and-shogi-by-self-play-with-alphazero) — adjacent reading from the same library section.
...
[1] Silver, Huang, Maddison, Guez, Sifre, van den Driessche, Schrittwieser, et al.. **Mastering the Game of Go with Deep Neural Networks and Tree Search (AlphaGo)**. 2016.
