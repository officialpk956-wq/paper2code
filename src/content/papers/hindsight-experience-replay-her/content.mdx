...
Published in 2017 by Andrychowicz, Wolski, Ray, Schneider, Fong, Welinder, McGrew, Tobin, Abbeel, Zaremba, **Hindsight Experience Replay (HER)** belongs to the REINFORCEMENT LEARNING part of the Paper2Code library. Solved sparse reward problem by relabeling failed trajectories with achieved goals. Enables learning goal-conditioned policies even when every episode fails. Critical for robotics. [1]
...
Tabular methods and unstable function approximation struggled with large observation spaces, off-policy data, and high-variance policy gradients. By 2017, researchers had working baselines, but the remaining gap was large enough that simply adding more layers, data, or hardware did not answer the scientific question.
...
- **Goal relabeling:** identify its input, output, learned parameters, and the assumption it adds to the baseline.
- **hindsight goals:** identify its input, output, learned parameters, and the assumption it adds to the baseline.
- **sparse reward solving:** identify its input, output, learned parameters, and the assumption it adds to the baseline.
- **goal-conditioned RL:** identify its input, output, learned parameters, and the assumption it adds to the baseline.

Its associated architecture or procedure is **HER**. The contribution should be explained as a change to a baseline computation: what new state is introduced, where information flows differently, and which objective trains the change. Solved sparse reward problem by relabeling failed trajectories with achieved goals. Enables learning goal-conditioned policies even when every episode fails. Critical for robotics.
...
The paper's main claim is summarized in the registry as: Solved sparse reward problem by relabeling failed trajectories with achieved goals. Enables learning goal-conditioned policies even when every episode fails. Critical for robotics. The key experiments should therefore isolate **causality**, not merely display a large final score.
...
The registry records the practical significance as: Robotic manipulation with sparse rewards; OpenAI Dactyl This statement explains *why the paper remained relevant*, but exact benchmark values must be taken from the primary tables in [1]. They are intentionally not invented or copied without experimental context here.
...
Robotic manipulation with sparse rewards; OpenAI Dactyl To translate that impact into a production decision, measure the complete path rather than an isolated kernel or offline model. Include preprocessing, retrieval or batching, host-device transfer, post-processing, cache behavior, and tail latency.
...
- [Addressing Function Approximation Error in Actor-Critic Methods (TD3)](/papers/addressing-function-approximation-error-in-actor-critic-methods-td3) — adjacent reading from the same library section.
- [Decision Transformer: Reinforcement Learning via Sequence Modeling](/papers/decision-transformer-reinforcement-learning-via-sequence-modeling) — adjacent reading from the same library section.
...
[1] Andrychowicz, Wolski, Ray, Schneider, Fong, Welinder, McGrew, Tobin, Abbeel, Zaremba. **Hindsight Experience Replay (HER)**. 2017.
