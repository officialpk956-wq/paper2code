# Rethinking Atrous Convolution for Semantic Segmentation (DeepLab v3)
...
Published in 2017 by Chen, Papandreou, Schroff, Adam, **Rethinking Atrous Convolution for Semantic Segmentation (DeepLab v3)** belongs to the COMPUTER VISION part of the Paper2Code library. ASPP (Atrous Spatial Pyramid Pooling) captures multi-scale context without reducing resolution. Encoder-decoder variant (v3+) set segmentation SOTA on multiple benchmarks. [1]
...
- **Atrous (dilated) convolutions:** identify its input, output, learned parameters, and the assumption it adds to the baseline.
- **multi-scale context aggregation:** identify its input, output, learned parameters, and the assumption it adds to the baseline.
- **dense prediction:** identify its input, output, learned parameters, and the assumption it adds to the baseline.

Its associated architecture or procedure is **DeepLab v3, DeepLab v3+, ASPP**. The contribution should be explained as a change to a baseline computation: what new state is introduced, where information flows differently, and which objective trains the change. ASPP (Atrous Spatial Pyramid Pooling) captures multi-scale context without reducing resolution. Encoder-decoder variant (v3+) set segmentation SOTA on multiple benchmarks.
...
The paper's main claim is summarized in the registry as: ASPP (Atrous Spatial Pyramid Pooling) captures multi-scale context without reducing resolution. Encoder-decoder variant (v3+) set segmentation SOTA on multiple benchmarks. The key experiments should therefore isolate **causality**, not merely display a large final score.
...
The registry records the practical significance as: Autonomous driving segmentation, medical imaging; Google's production segmentation This statement explains *why the paper remained relevant*, but exact benchmark values must be taken from the primary tables in [1]. They are intentionally not invented or copied without experimental context here.
...
- [Architecture deep dive](/architectures/deeplab-v3-2) — inspect the corresponding model family and tensor flow.
- [MobileNets: Efficient CNNs for Mobile Vision](/papers/mobilenets-efficient-cnns-for-mobile-vision) — adjacent reading from the same library section.
...
[1] Chen, Papandreou, Schroff, Adam. **Rethinking Atrous Convolution for Semantic Segmentation (DeepLab v3)**. 2017.
