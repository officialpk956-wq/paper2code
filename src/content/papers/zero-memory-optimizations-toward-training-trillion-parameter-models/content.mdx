...
Published in 2020 by Rajbhandari, Rasley, Ruwase, He, **ZeRO: Memory Optimizations Toward Training Trillion Parameter Models** belongs to the LARGE LANGUAGE MODELS part of the Paper2Code library. Partitions optimizer state, gradients, and parameters across GPUs to eliminate redundancy. Enables training models 8× larger than previously possible with data parallelism alone. [1]
...
- **ZeRO stage 1/2/3:** identify its input, output, learned parameters, and the assumption it adds to the baseline.
- **optimizer state partitioning:** identify its input, output, learned parameters, and the assumption it adds to the baseline.
- **memory efficiency in distributed training:** identify its input, output, learned parameters, and the assumption it adds to the baseline.

Its associated architecture or procedure is **None (optimization)**. The contribution should be explained as a change to a baseline computation: what new state is introduced, where information flows differently, and which objective trains the change. Partitions optimizer state, gradients, and parameters across GPUs to eliminate redundancy. Enables training models 8× larger than previously possible with data parallelism alone.
...
The paper's main claim is summarized in the registry as: Partitions optimizer state, gradients, and parameters across GPUs to eliminate redundancy. Enables training models 8× larger than previously possible with data parallelism alone. The key experiments should therefore isolate **causality**, not merely display a large final score.
...
The registry records the practical significance as: DeepSpeed ZeRO is the standard for training large models; powers most frontier model training This statement explains *why the paper remained relevant*, but exact benchmark values must be taken from the primary tables in [1]. They are intentionally not invented or copied without experimental context here.
...
[1] Rajbhandari, Rasley, Ruwase, He. **ZeRO: Memory Optimizations Toward Training Trillion Parameter Models**. 2020.
