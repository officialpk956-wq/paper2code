...
Published in 2021 by Liu, Lin, Cao, Hu, Wei, Zhang, Lin, Guo, **Swin Transformer: Hierarchical Vision Transformer using Shifted Windows** belongs to the COMPUTER VISION part of the Paper2Code library. Made ViT practical for dense prediction tasks. Shifted window attention achieves O(n) complexity vs O(n²) for full attention. Hierarchical features enable FPN integration. [1]
...
- **Window-based self-attention:** identify its input, output, learned parameters, and the assumption it adds to the baseline.
- **shifted windows for cross-window connections:** identify its input, output, learned parameters, and the assumption it adds to the baseline.
- **patch merging:** identify its input, output, learned parameters, and the assumption it adds to the baseline.

Its associated architecture or procedure is **Swin Transformer, shifted window attention**. The contribution should be explained as a change to a baseline computation: what new state is introduced, where information flows differently, and which objective trains the change. Made ViT practical for dense prediction tasks. Shifted window attention achieves O(n) complexity vs O(n²) for full attention. Hierarchical features enable FPN integration.
...
The registry records the practical significance as: Backbone for detection/segmentation in production vision systems This statement explains *why the paper remained relevant*, but exact benchmark values must be taken from the primary tables in [1]. They are intentionally not invented or copied without experimental context here.
...
- [Architecture deep dive](/architectures/swin-transformer) — inspect the corresponding model family and tensor flow.
- [End-to-End Object Detection with Transformers (DETR)](/papers/end-to-end-object-detection-with-transformers-detr) — adjacent reading from the same library section.
- [Masked Autoencoders Are Scalable Vision Learners (MAE)](/papers/masked-autoencoders-are-scalable-vision-learners-mae) — adjacent reading from the same library section.
...
[1] Liu, Lin, Cao, Hu, Wei, Zhang, Lin, Guo. **Swin Transformer: Hierarchical Vision Transformer using Shifted Windows**. 2021.
