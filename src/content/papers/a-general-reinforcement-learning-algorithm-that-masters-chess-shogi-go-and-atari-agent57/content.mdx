# A General Reinforcement Learning Algorithm that Masters Chess, Shogi, Go and Atari (Agent57)
...
Published in 2020 by Badia, Piot, Kapturowski, Sprechmann, Vitvitskyi, Guo, Blundell, **A General Reinforcement Learning Algorithm that Masters Chess, Shogi, Go and Atari (Agent57)** belongs to the REINFORCEMENT LEARNING part of the Paper2Code library. First RL agent to outperform human players on ALL 57 Atari games. Adaptive meta-controller balancing exploration. Showed RL can achieve broad generalization. [1]
...
- **Meta-controller for exploration:** identify its input, output, learned parameters, and the assumption it adds to the baseline.
- **universal value function approximator:** identify its input, output, learned parameters, and the assumption it adds to the baseline.
- **Never Give Up:** identify its input, output, learned parameters, and the assumption it adds to the baseline.

Its associated architecture or procedure is **Agent57**. The contribution should be explained as a change to a baseline computation: what new state is introduced, where information flows differently, and which objective trains the change. First RL agent to outperform human players on ALL 57 Atari games. Adaptive meta-controller balancing exploration. Showed RL can achieve broad generalization.
...
The paper's main claim is summarized in the registry as: First RL agent to outperform human players on ALL 57 Atari games. Adaptive meta-controller balancing exploration. Showed RL can achieve broad generalization. The key experiments should therefore isolate **causality**, not merely display a large final score.
...
The registry records the practical significance as: Research milestone; defined the Atari 57 benchmark completion This statement explains *why the paper remained relevant*, but exact benchmark values must be taken from the primary tables in [1]. They are intentionally not invented or copied without experimental context here.
...
Research milestone; defined the Atari 57 benchmark completion To translate that impact into a production decision, measure the complete path rather than an isolated kernel or offline model. Include preprocessing, retrieval or batching, host-device transfer, post-processing, cache behavior, and tail latency.
...
- [Architecture deep dive](/architectures/t5) — inspect the corresponding model family and tensor flow.
...
[1] Badia, Piot, Kapturowski, Sprechmann, Vitvitskyi, Guo, Blundell. **A General Reinforcement Learning Algorithm that Masters Chess, Shogi, Go and Atari (Agent57)**. 2020.
