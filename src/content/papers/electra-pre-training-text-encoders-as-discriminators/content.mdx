...
Published in 2020 by Clark, Luong, Le, Manning, **ELECTRA: Pre-training Text Encoders as Discriminators** belongs to the NLP part of the Paper2Code library. Replaced token detection vs. masked language modeling — every token contributes to training signal vs. 15% in BERT. Same accuracy at 1/4 the compute. More sample-efficient pre-training. [1]
...
- **Replaced token detection:** identify its input, output, learned parameters, and the assumption it adds to the baseline.
- **generator-discriminator pre-training:** identify its input, output, learned parameters, and the assumption it adds to the baseline.
- **all-token learning signal:** identify its input, output, learned parameters, and the assumption it adds to the baseline.

Its associated architecture or procedure is **ELECTRA**. The contribution should be explained as a change to a baseline computation: what new state is introduced, where information flows differently, and which objective trains the change. Replaced token detection vs. masked language modeling — every token contributes to training signal vs. 15% in BERT. Same accuracy at 1/4 the compute. More sample-efficient pre-training.
...
The paper's main claim is summarized in the registry as: Replaced token detection vs. masked language modeling — every token contributes to training signal vs. 15% in BERT. Same accuracy at 1/4 the compute. More sample-efficient pre-training. The key experiments should therefore isolate **causality**, not merely display a large final score.
...
The registry records the practical significance as: More efficient language model pre-training; used in resource-constrained settings This statement explains *why the paper remained relevant*, but exact benchmark values must be taken from the primary tables in [1]. They are intentionally not invented or copied without experimental context here.
...
- [DeBERTa: Decoding-enhanced BERT with Disentangled Attention](/papers/deberta-decoding-enhanced-bert-with-disentangled-attention) — adjacent reading from the same library section.
- [Longformer: The Long-Document Transformer](/papers/longformer-the-long-document-transformer) — adjacent reading from the same library section.
...
[1] Clark, Luong, Le, Manning. **ELECTRA: Pre-training Text Encoders as Discriminators**. 2020.
