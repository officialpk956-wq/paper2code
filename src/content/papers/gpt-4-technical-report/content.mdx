...
Published in 2023 by OpenAI, **GPT-4 Technical Report** belongs to the LARGE LANGUAGE MODELS part of the Paper2Code library. Multimodal, dramatically more capable than GPT-3.5. First frontier model technical report describing evaluation methodology, alignment techniques, and capability/safety evaluations at scale. [1]
...
- **Multimodal LLM at scale:** identify its input, output, learned parameters, and the assumption it adds to the baseline.
- **predictable capability from scaling:** identify its input, output, learned parameters, and the assumption it adds to the baseline.
- **red-teaming methodology:** identify its input, output, learned parameters, and the assumption it adds to the baseline.

Its associated architecture or procedure is **GPT-4 (details undisclosed)**. The contribution should be explained as a change to a baseline computation: what new state is introduced, where information flows differently, and which objective trains the change. Multimodal, dramatically more capable than GPT-3.5. First frontier model technical report describing evaluation methodology, alignment techniques, and capability/safety evaluations at scale.
...
The paper's main claim is summarized in the registry as: Multimodal, dramatically more capable than GPT-3.5. First frontier model technical report describing evaluation methodology, alignment techniques, and capability/safety evaluations at scale. The key experiments should therefore isolate **causality**, not merely display a large final score.
...
The registry records the practical significance as: Triggered AI industry transformation; GPT-4 became the quality bar all systems aim for This statement explains *why the paper remained relevant*, but exact benchmark values must be taken from the primary tables in [1]. They are intentionally not invented or copied without experimental context here.
...
- [Architecture deep dive](/architectures/gpt-4) — inspect the corresponding model family and tensor flow.
