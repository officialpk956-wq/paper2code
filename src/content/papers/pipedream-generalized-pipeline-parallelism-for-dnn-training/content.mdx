...
Published in 2019 by Narayanan, Harlap, Phanishayee, Seshadri, Devanur, Ganger, Gibbons, Zaharia (Microsoft / CMU), **PipeDream: Generalized Pipeline Parallelism for DNN Training** belongs to the SYSTEMS part of the Paper2Code library. Identified that GPipe's synchronous pipeline creates a bubble — all workers stall between forward and backward passes. PipeDream introduced 1F1B (one-forward-one-backward) scheduling to eliminate the bubble, interleaving forward and backward passes to keep all stages active. Introduced weight stashing to maintain consistent gradient computation. [1]
...
Earlier implementations were bounded by one dominant resource such as accelerator memory, network bandwidth, data movement, synchronization, or tail latency. By 2019, researchers had working baselines, but the remaining gap was large enough that simply adding more layers, data, or hardware did not answer the scientific question.
...
- **Pipeline bubble analysis:** identify its input, output, learned parameters, and the assumption it adds to the baseline.
- **asynchronous pipeline training:** identify its input, output, learned parameters, and the assumption it adds to the baseline.
- **weight version consistency:** identify its input, output, learned parameters, and the assumption it adds to the baseline.

Its associated architecture or procedure is **A task-specific realization of the paper's central mechanism**. The contribution should be explained as a change to a baseline computation: what new state is introduced, where information flows differently, and which objective trains the change. Identified that GPipe's synchronous pipeline creates a bubble — all workers stall between forward and backward passes. PipeDream introduced 1F1B (one-forward-one-backward) scheduling to eliminate the bubble, interleaving forward and backward passes to keep all stages active. Introduced weight stashing to maintain consistent gradient computation.
...
The paper's main claim is summarized in the registry as: Identified that GPipe's synchronous pipeline creates a bubble — all workers stall between forward and backward passes. PipeDream introduced 1F1B (one-forward-one-backward) scheduling to eliminate the bubble, interleaving forward and backward passes to keep all stages active. Introduced weight stashing to maintain consistent gradient computation. The key experiments should therefore isolate **causality**, not merely display a large final score.
...
The registry records the practical significance as: 1F1B scheduling adopted by Megatron-LM and is now the standard pipeline schedule for large model training. Reduced pipeline bubble from O(p) to O(1) where p is pipeline stages. This statement explains *why the paper remained relevant*, but exact benchmark values must be taken from the primary tables in [1]. They are intentionally not invented or copied without experimental context here.
...
[1] Narayanan, Harlap, Phanishayee, Seshadri, Devanur, Ganger, Gibbons, Zaharia (Microsoft / CMU). **PipeDream: Generalized Pipeline Parallelism for DNN Training**. 2019.
