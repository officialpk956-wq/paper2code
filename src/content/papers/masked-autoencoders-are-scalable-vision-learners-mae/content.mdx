...
Published in 2021 by He, Chen, Xie, Li, Dollár, Girshick, **Masked Autoencoders Are Scalable Vision Learners (MAE)** belongs to the COMPUTER VISION part of the Paper2Code library. Showed that masking 75% of image patches and reconstructing them produces powerful ViT representations. 3× faster training than contrastive methods. Simple yet effective self-supervised learning. [1]
...
- **Masked image modeling:** identify its input, output, learned parameters, and the assumption it adds to the baseline.
- **asymmetric encoder-decoder:** identify its input, output, learned parameters, and the assumption it adds to the baseline.
- **75% masking ratio:** identify its input, output, learned parameters, and the assumption it adds to the baseline.

Its associated architecture or procedure is **MAE**. The contribution should be explained as a change to a baseline computation: what new state is introduced, where information flows differently, and which objective trains the change. Showed that masking 75% of image patches and reconstructing them produces powerful ViT representations. 3× faster training than contrastive methods. Simple yet effective self-supervised learning.
...
The paper's main claim is summarized in the registry as: Showed that masking 75% of image patches and reconstructing them produces powerful ViT representations. 3× faster training than contrastive methods. Simple yet effective self-supervised learning. The key experiments should therefore isolate **causality**, not merely display a large final score.
...
The registry records the practical significance as: Self-supervised vision pre-training; used in medical imaging, robotics This statement explains *why the paper remained relevant*, but exact benchmark values must be taken from the primary tables in [1]. They are intentionally not invented or copied without experimental context here.
...
- [Architecture deep dive](/architectures/ae) — inspect the corresponding model family and tensor flow.
...
- [Emerging Properties in Self-Supervised Vision Transformers (DINO)](/papers/emerging-properties-in-self-supervised-vision-transformers-dino) — adjacent reading from the same library section.
...
[1] He, Chen, Xie, Li, Dollár, Girshick. **Masked Autoencoders Are Scalable Vision Learners (MAE)**. 2021.
