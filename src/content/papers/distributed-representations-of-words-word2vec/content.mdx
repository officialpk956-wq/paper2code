...
Published in 2013 by Mikolov, Chen, Corrado, Dean, **Distributed Representations of Words (Word2Vec)** belongs to the FOUNDATIONS part of the Paper2Code library. Showed that words could be represented as dense vectors capturing semantic relationships. "King - Man + Woman = Queen." Launched the embedding paradigm. [1]
...
- **Word embeddings:** identify its input, output, learned parameters, and the assumption it adds to the baseline.
- **distributional semantics:** identify its input, output, learned parameters, and the assumption it adds to the baseline.
- **negative sampling:** identify its input, output, learned parameters, and the assumption it adds to the baseline.
- **analogy tasks:** identify its input, output, learned parameters, and the assumption it adds to the baseline.

Its associated architecture or procedure is **Skip-gram, CBOW**. The contribution should be explained as a change to a baseline computation: what new state is introduced, where information flows differently, and which objective trains the change. Showed that words could be represented as dense vectors capturing semantic relationships. "King - Man + Woman = Queen." Launched the embedding paradigm.
...
The paper's main claim is summarized in the registry as: Showed that words could be represented as dense vectors capturing semantic relationships. "King - Man + Woman = Queen." Launched the embedding paradigm. The key experiments should therefore isolate **causality**, not merely display a large final score.
...
- [Architecture deep dive](/architectures/word2vec) — inspect the corresponding model family and tensor flow.
- [Long Short-Term Memory](/papers/long-short-term-memory) — adjacent reading from the same library section.
- [Scaling Laws for Neural Language Models](/papers/scaling-laws-for-neural-language-models) — adjacent reading from the same library section.
...
[1] Mikolov, Chen, Corrado, Dean. **Distributed Representations of Words (Word2Vec)**. 2013.
