...
Published in 2015 by Hinton, Vinyals, Dean, **Knowledge Distillation** belongs to the FOUNDATIONS part of the Paper2Code library. Showed that a small network can learn from a large network's soft outputs, achieving near-large-model accuracy. Core technique for model compression and the basis for DistilBERT, TinyBERT. [1]
...
- **Soft targets:** identify its input, output, learned parameters, and the assumption it adds to the baseline.
- **temperature scaling:** identify its input, output, learned parameters, and the assumption it adds to the baseline.
- **teacher-student framework:** identify its input, output, learned parameters, and the assumption it adds to the baseline.
- **dark knowledge:** identify its input, output, learned parameters, and the assumption it adds to the baseline.

Its associated architecture or procedure is **None**. The contribution should be explained as a change to a baseline computation: what new state is introduced, where information flows differently, and which objective trains the change. Showed that a small network can learn from a large network's soft outputs, achieving near-large-model accuracy. Core technique for model compression and the basis for DistilBERT, TinyBERT.
...
The paper's main claim is summarized in the registry as: Showed that a small network can learn from a large network's soft outputs, achieving near-large-model accuracy. Core technique for model compression and the basis for DistilBERT, TinyBERT. The key experiments should therefore isolate **causality**, not merely display a large final score.
...
The registry records the practical significance as: On-device AI, model compression; every edge deployment uses some form of distillation This statement explains *why the paper remained relevant*, but exact benchmark values must be taken from the primary tables in [1]. They are intentionally not invented or copied without experimental context here.
...
- [EfficientNet: Rethinking Model Scaling for CNNs](/papers/efficientnet-rethinking-model-scaling-for-cnns) — adjacent reading from the same library section.
- [Deep Double Descent](/papers/deep-double-descent) — adjacent reading from the same library section.
...
[1] Hinton, Vinyals, Dean. **Knowledge Distillation**. 2015.
