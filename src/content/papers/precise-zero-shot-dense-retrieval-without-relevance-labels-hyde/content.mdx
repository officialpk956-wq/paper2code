...
Published in 2022 by Gao, Ma, Lin, Callan, **Precise Zero-Shot Dense Retrieval without Relevance Labels (HyDE)** belongs to the RAG part of the Paper2Code library. Generate a hypothetical document from the query, embed it, retrieve real documents similar to the hypothetical. Dramatically improves zero-shot retrieval by bridging query-document distribution gap. [1]
...
- **Hypothetical document embedding:** identify its input, output, learned parameters, and the assumption it adds to the baseline.
- **query expansion via generation:** identify its input, output, learned parameters, and the assumption it adds to the baseline.
- **zero-shot retrieval:** identify its input, output, learned parameters, and the assumption it adds to the baseline.

Its associated architecture or procedure is **HyDE**. The contribution should be explained as a change to a baseline computation: what new state is introduced, where information flows differently, and which objective trains the change. Generate a hypothetical document from the query, embed it, retrieve real documents similar to the hypothetical. Dramatically improves zero-shot retrieval by bridging query-document distribution gap.
...
The paper's main claim is summarized in the registry as: Generate a hypothetical document from the query, embed it, retrieve real documents similar to the hypothetical. Dramatically improves zero-shot retrieval by bridging query-document distribution gap. The key experiments should therefore isolate **causality**, not merely display a large final score.
...
Used in production RAG pipelines to improve retrieval quality To translate that impact into a production decision, measure the complete path rather than an isolated kernel or offline model. Include preprocessing, retrieval or batching, host-device transfer, post-processing, cache behavior, and tail latency.
...
[1] Gao, Ma, Lin, Callan. **Precise Zero-Shot Dense Retrieval without Relevance Labels (HyDE)**. 2022.
