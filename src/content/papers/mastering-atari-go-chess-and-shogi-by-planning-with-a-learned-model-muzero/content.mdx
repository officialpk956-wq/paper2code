...
Published in 2019 by Schrittwieser, Antonoglou, Hubert, Lanctot, Sifre, Schmitt, et al., **Mastering Atari, Go, Chess and Shogi by Planning with a Learned Model (MuZero)** belongs to the REINFORCEMENT LEARNING part of the Paper2Code library. Learns a model of the environment's dynamics without access to game rules. MuZero combines model learning + MCTS. Surpasses AlphaZero while working in unknown environments. [1]
...
- **latent state planning:** identify its input, output, learned parameters, and the assumption it adds to the baseline.
- **model-based MCTS without rules:** identify its input, output, learned parameters, and the assumption it adds to the baseline.

Its associated architecture or procedure is **MuZero**. The contribution should be explained as a change to a baseline computation: what new state is introduced, where information flows differently, and which objective trains the change. Learns a model of the environment's dynamics without access to game rules. MuZero combines model learning + MCTS. Surpasses AlphaZero while working in unknown environments.
...
The paper's main claim is summarized in the registry as: Learns a model of the environment's dynamics without access to game rules. MuZero combines model learning + MCTS. Surpasses AlphaZero while working in unknown environments. The key experiments should therefore isolate **causality**, not merely display a large final score.
...
The registry records the practical significance as: AlphaFold 2 uses similar ideas; video compression (DeepMind); general model-based RL This statement explains *why the paper remained relevant*, but exact benchmark values must be taken from the primary tables in [1]. They are intentionally not invented or copied without experimental context here.
...
- [Soft Actor-Critic: Off-Policy Maximum Entropy Deep RL](/papers/soft-actor-critic-off-policy-maximum-entropy-deep-rl) — adjacent reading from the same library section.
- [Dream to Control: Learning Behaviors by Latent Imagination (Dreamer)](/papers/dream-to-control-learning-behaviors-by-latent-imagination-dreamer) — adjacent reading from the same library section.
...
[1] Schrittwieser, Antonoglou, Hubert, Lanctot, Sifre, Schmitt, et al.. **Mastering Atari, Go, Chess and Shogi by Planning with a Learned Model (MuZero)**. 2019.
