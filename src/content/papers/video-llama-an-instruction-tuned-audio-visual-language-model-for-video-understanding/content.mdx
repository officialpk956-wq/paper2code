...
Published in 2023 by Zhang, Zhang, Li, Qiao, Gao, Xu, Shi, Shan, Qing, Zhu (Alibaba DAMO / NTU), **Video-LLaMA: An Instruction-Tuned Audio-Visual Language Model for Video Understanding** belongs to the SYSTEMS part of the Paper2Code library. Extends LLaVA's visual instruction tuning to video and audio. Adds a Video Q-Former (sampling frames from video) and an Audio Q-Former (encoding audio from IMAGEBIND) into the Llama language model. Enables video QA, audio QA, and audio-visual QA with a single model. First open-source audio-visual LLM. [1]
...
- **Temporal visual encoding with Q-Former:** identify its input, output, learned parameters, and the assumption it adds to the baseline.
- **audio branch from ImageBind:** identify its input, output, learned parameters, and the assumption it adds to the baseline.
- **joint audio-visual-text generation:** identify its input, output, learned parameters, and the assumption it adds to the baseline.

Its associated architecture or procedure is **Video-LLaMA (video + audio Q-Former + LLM)**. The contribution should be explained as a change to a baseline computation: what new state is introduced, where information flows differently, and which objective trains the change. Extends LLaVA's visual instruction tuning to video and audio. Adds a Video Q-Former (sampling frames from video) and an Audio Q-Former (encoding audio from IMAGEBIND) into the Llama language model. Enables video QA, audio QA, and audio-visual QA with a single model. First open-source audio-visual LLM.
...
The paper's main claim is summarized in the registry as: Extends LLaVA's visual instruction tuning to video and audio. Adds a Video Q-Former (sampling frames from video) and an Audio Q-Former (encoding audio from IMAGEBIND) into the Llama language model. Enables video QA, audio QA, and audio-visual QA with a single model. First open-source audio-visual LLM. The key experiments should therefore isolate **causality**, not merely display a large final score.
...
The registry records the practical significance as: Foundation for subsequent video understanding models (Video-LLaVA, MiniGPT4-Video, InternVL-Video). Enabled QA over meeting recordings, lecture videos, and instructional content. This statement explains *why the paper remained relevant*, but exact benchmark values must be taken from the primary tables in [1]. They are intentionally not invented or copied without experimental context here.
...
- [CogAgent: A Visual Language Model for GUI Agents](/papers/cogagent-a-visual-language-model-for-gui-agents) — adjacent reading from the same library section.
...
[1] Zhang, Zhang, Li, Qiao, Gao, Xu, Shi, Shan, Qing, Zhu (Alibaba DAMO / NTU). **Video-LLaMA: An Instruction-Tuned Audio-Visual Language Model for Video Understanding**. 2023.
