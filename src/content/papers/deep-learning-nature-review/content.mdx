...
Published in 2015 by LeCun, Bengio, Hinton, **Deep Learning (Nature Review)** belongs to the FOUNDATIONS part of the Paper2Code library. The canonical review of deep learning by its three founding fathers. Explains the full stack — representations, convolutional nets, recurrent nets, optimization — in one paper. [1]
...
- **Representation learning:** identify its input, output, learned parameters, and the assumption it adds to the baseline.
- **hierarchical features:** identify its input, output, learned parameters, and the assumption it adds to the baseline.
- **the case for depth:** identify its input, output, learned parameters, and the assumption it adds to the baseline.

Its associated architecture or procedure is **None (review)**. The contribution should be explained as a change to a baseline computation: what new state is introduced, where information flows differently, and which objective trains the change. The canonical review of deep learning by its three founding fathers. Explains the full stack — representations, convolutional nets, recurrent nets, optimization — in one paper.
...
The paper's main claim is summarized in the registry as: The canonical review of deep learning by its three founding fathers. Explains the full stack — representations, convolutional nets, recurrent nets, optimization — in one paper. The key experiments should therefore isolate **causality**, not merely display a large final score.
...
The registry records the practical significance as: Introduced thousands of engineers and researchers to the field This statement explains *why the paper remained relevant*, but exact benchmark values must be taken from the primary tables in [1]. They are intentionally not invented or copied without experimental context here.
...
- [Neural Architecture Search with Reinforcement Learning (NASNet)](/papers/neural-architecture-search-with-reinforcement-learning-nasnet) — adjacent reading from the same library section.
...
[1] LeCun, Bengio, Hinton. **Deep Learning (Nature Review)**. 2015.
