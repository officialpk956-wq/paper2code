...
Published in 1997 by Hochreiter, Schmidhuber, **Long Short-Term Memory** belongs to the FOUNDATIONS part of the Paper2Code library. Solved the vanishing gradient problem for recurrent networks. Enabled learning of long-range dependencies in sequences. Dominated NLP for 20 years until Transformers. [1]
...
Earlier likelihood, adversarial, and autoregressive approaches traded off sample fidelity, mode coverage, stable optimization, and inference speed in different ways. By 1997, researchers had working baselines, but the remaining gap was large enough that simply adding more layers, data, or hardware did not answer the scientific question.
...
- **Gated recurrent units:** identify its input, output, learned parameters, and the assumption it adds to the baseline.
- **cell state:** identify its input, output, learned parameters, and the assumption it adds to the baseline.
- **long-range gradient flow:** identify its input, output, learned parameters, and the assumption it adds to the baseline.
- **memory cells:** identify its input, output, learned parameters, and the assumption it adds to the baseline.

Its associated architecture or procedure is **LSTM cell (forget gate, input gate, output gate, cell state)**. The contribution should be explained as a change to a baseline computation: what new state is introduced, where information flows differently, and which objective trains the change. Solved the vanishing gradient problem for recurrent networks. Enabled learning of long-range dependencies in sequences. Dominated NLP for 20 years until Transformers.
...
The paper's main claim is summarized in the registry as: Solved the vanishing gradient problem for recurrent networks. Enabled learning of long-range dependencies in sequences. Dominated NLP for 20 years until Transformers. The key experiments should therefore isolate **causality**, not merely display a large final score.
...
The registry records the practical significance as: Powered Siri, Google voice search, neural machine translation pre-Transformer This statement explains *why the paper remained relevant*, but exact benchmark values must be taken from the primary tables in [1]. They are intentionally not invented or copied without experimental context here.
...
- [Architecture deep dive](/architectures/lstm) — inspect the corresponding model family and tensor flow.
- [Batch Normalization: Accelerating Deep Network Training](/papers/batch-normalization-accelerating-deep-network-training) — adjacent reading from the same library section.
