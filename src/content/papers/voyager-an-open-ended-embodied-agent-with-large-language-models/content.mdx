...
Published in 2023 by Wang, Cai, Liu, Ma, Liang, Abbeel, **Voyager: An Open-Ended Embodied Agent with Large Language Models** belongs to the AGENTS part of the Paper2Code library. LLM agent in Minecraft that continuously explores, learns, and acquires skills without human intervention. Skill library grows without forgetting. Demonstrated curriculum learning via LLM. [1]
...
- **Automatic curriculum:** identify its input, output, learned parameters, and the assumption it adds to the baseline.
- **skill library:** identify its input, output, learned parameters, and the assumption it adds to the baseline.
- **iterative prompting:** identify its input, output, learned parameters, and the assumption it adds to the baseline.
- **lifelong learning agents:** identify its input, output, learned parameters, and the assumption it adds to the baseline.

Its associated architecture or procedure is **Voyager**. The contribution should be explained as a change to a baseline computation: what new state is introduced, where information flows differently, and which objective trains the change. LLM agent in Minecraft that continuously explores, learns, and acquires skills without human intervention. Skill library grows without forgetting. Demonstrated curriculum learning via LLM.
...
The paper's main claim is summarized in the registry as: LLM agent in Minecraft that continuously explores, learns, and acquires skills without human intervention. Skill library grows without forgetting. Demonstrated curriculum learning via LLM. The key experiments should therefore isolate **causality**, not merely display a large final score.
...
The registry records the practical significance as: Open-ended game AI, autonomous skill acquisition This statement explains *why the paper remained relevant*, but exact benchmark values must be taken from the primary tables in [1]. They are intentionally not invented or copied without experimental context here.
...
Open-ended game AI, autonomous skill acquisition To translate that impact into a production decision, measure the complete path rather than an isolated kernel or offline model. Include preprocessing, retrieval or batching, host-device transfer, post-processing, cache behavior, and tail latency.
...
- [Generative Agents: Interactive Simulacra of Human Behavior](/papers/generative-agents-interactive-simulacra-of-human-behavior) — adjacent reading from the same library section.
...
[1] Wang, Cai, Liu, Ma, Liang, Abbeel. **Voyager: An Open-Ended Embodied Agent with Large Language Models**. 2023.
