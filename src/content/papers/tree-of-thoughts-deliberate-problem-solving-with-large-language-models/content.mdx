...
Published in 2023 by Yao, Yu, Zhao, Shafran, Griffiths, Cao, Narasimhan, **Tree of Thoughts: Deliberate Problem Solving with Large Language Models** belongs to the LARGE LANGUAGE MODELS part of the Paper2Code library. Extended CoT to tree-structured exploration with backtracking. BFS/DFS over reasoning steps. Solved game of 24 where CoT + self-consistency failed. Defined deliberate reasoning paradigm. [1]
...
- **Deliberate search over thoughts:** identify its input, output, learned parameters, and the assumption it adds to the baseline.
- **evaluation of intermediate steps:** identify its input, output, learned parameters, and the assumption it adds to the baseline.
- **lookahead reasoning:** identify its input, output, learned parameters, and the assumption it adds to the baseline.

Its associated architecture or procedure is **Tree of Thoughts framework**. The contribution should be explained as a change to a baseline computation: what new state is introduced, where information flows differently, and which objective trains the change. Extended CoT to tree-structured exploration with backtracking. BFS/DFS over reasoning steps. Solved game of 24 where CoT + self-consistency failed. Defined deliberate reasoning paradigm.
...
The paper's main claim is summarized in the registry as: Extended CoT to tree-structured exploration with backtracking. BFS/DFS over reasoning steps. Solved game of 24 where CoT + self-consistency failed. Defined deliberate reasoning paradigm. The key experiments should therefore isolate **causality**, not merely display a large final score.
...
The registry records the practical significance as: Advanced reasoning in LLM products; influenced reasoning models (o1-style) This statement explains *why the paper remained relevant*, but exact benchmark values must be taken from the primary tables in [1]. They are intentionally not invented or copied without experimental context here.
...
- [Megatron-LM: Training Multi-Billion Parameter Language Models Using GPU Model Parallelism](/papers/megatron-lm-training-multi-billion-parameter-language-models-using-gpu-model-parallelism) — adjacent reading from the same library section.
- [Self-Consistency Improves Chain of Thought Reasoning in Language Models](/papers/self-consistency-improves-chain-of-thought-reasoning-in-language-models) — adjacent reading from the same library section.
...
[1] Yao, Yu, Zhao, Shafran, Griffiths, Cao, Narasimhan. **Tree of Thoughts: Deliberate Problem Solving with Large Language Models**. 2023.
