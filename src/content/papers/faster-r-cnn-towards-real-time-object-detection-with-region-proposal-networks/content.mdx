...
Published in 2015 by Ren, He, Girshick, Sun, **Faster R-CNN: Towards Real-Time Object Detection with Region Proposal Networks** belongs to the COMPUTER VISION part of the Paper2Code library. Unified region proposal and detection in one network. The definitive two-stage detector. Region Proposal Network + RoI Pooling became the standard pipeline for accurate detection. [1]
...
- **Anchor boxes:** identify its input, output, learned parameters, and the assumption it adds to the baseline.
- **RPN sharing features with detector:** identify its input, output, learned parameters, and the assumption it adds to the baseline.
- **end-to-end two-stage detection:** identify its input, output, learned parameters, and the assumption it adds to the baseline.

Its associated architecture or procedure is **Faster R-CNN, Region Proposal Network (RPN)**. The contribution should be explained as a change to a baseline computation: what new state is introduced, where information flows differently, and which objective trains the change. Unified region proposal and detection in one network. The definitive two-stage detector. Region Proposal Network + RoI Pooling became the standard pipeline for accurate detection.
...
The paper's main claim is summarized in the registry as: Unified region proposal and detection in one network. The definitive two-stage detector. Region Proposal Network + RoI Pooling became the standard pipeline for accurate detection. The key experiments should therefore isolate **causality**, not merely display a large final score.
...
The registry records the practical significance as: Benchmark standard for accurate detection; used in medical imaging, satellite imagery This statement explains *why the paper remained relevant*, but exact benchmark values must be taken from the primary tables in [1]. They are intentionally not invented or copied without experimental context here.
...
- [Architecture deep dive](/architectures/faster-r-cnn) — inspect the corresponding model family and tensor flow.
- [You Only Look Once: Unified, Real-Time Object Detection (YOLO)](/papers/you-only-look-once-unified-real-time-object-detection-yolo) — adjacent reading from the same library section.
- [Mask R-CNN](/papers/mask-r-cnn) — adjacent reading from the same library section.
...
[1] Ren, He, Girshick, Sun. **Faster R-CNN: Towards Real-Time Object Detection with Region Proposal Networks**. 2015.
