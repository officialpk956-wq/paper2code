...
Published in 2023 by Patil, Zhang, Wang, Gonzalez, **Gorilla: Large Language Model Connected with Massive APIs** belongs to the AGENTS part of the Paper2Code library. Fine-tuned LLaMA on 1,645 APIs across HuggingFace, TorchHub, TensorFlow Hub. Reduced hallucination in API calls via retrieval-aware training. Practical tool-use at scale. [1]
...
- **API-aware generation:** identify its input, output, learned parameters, and the assumption it adds to the baseline.
- **retrieval-augmented tool use:** identify its input, output, learned parameters, and the assumption it adds to the baseline.
- **reducing hallucinated API calls:** identify its input, output, learned parameters, and the assumption it adds to the baseline.

Its associated architecture or procedure is **Gorilla (retrieval-augmented tool use)**. The contribution should be explained as a change to a baseline computation: what new state is introduced, where information flows differently, and which objective trains the change. Fine-tuned LLaMA on 1,645 APIs across HuggingFace, TorchHub, TensorFlow Hub. Reduced hallucination in API calls via retrieval-aware training. Practical tool-use at scale.
...
The paper's main claim is summarized in the registry as: Fine-tuned LLaMA on 1,645 APIs across HuggingFace, TorchHub, TensorFlow Hub. Reduced hallucination in API calls via retrieval-aware training. Practical tool-use at scale. The key experiments should therefore isolate **causality**, not merely display a large final score.
...
The registry records the practical significance as: LLM tool integration, code generation for real APIs This statement explains *why the paper remained relevant*, but exact benchmark values must be taken from the primary tables in [1]. They are intentionally not invented or copied without experimental context here.
...
LLM tool integration, code generation for real APIs To translate that impact into a production decision, measure the complete path rather than an isolated kernel or offline model. Include preprocessing, retrieval or batching, host-device transfer, post-processing, cache behavior, and tail latency.
...
- [AutoGen: Enabling Next-Gen LLM Applications via Multi-Agent Conversation](/papers/autogen-enabling-next-gen-llm-applications-via-multi-agent-conversation) — adjacent reading from the same library section.
...
[1] Patil, Zhang, Wang, Gonzalez. **Gorilla: Large Language Model Connected with Massive APIs**. 2023.
