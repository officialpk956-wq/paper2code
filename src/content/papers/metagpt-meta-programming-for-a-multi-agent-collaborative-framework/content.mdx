...
Published in 2023 by Hong, Zheng, Chen, Cheng, Zhang, Wang, Yau, Lin, Zhou, Ran, Xiao, Wu, **MetaGPT: Meta Programming for a Multi-Agent Collaborative Framework** belongs to the AGENTS part of the Paper2Code library. Multi-agent system where LLMs take roles (PM, architect, engineer, QA). Produces a complete software project from a one-line requirement. Defined structured multi-agent collaboration. [1]
...
- **Role-based multi-agent:** identify its input, output, learned parameters, and the assumption it adds to the baseline.
- **structured communication:** identify its input, output, learned parameters, and the assumption it adds to the baseline.
- **software development pipeline:** identify its input, output, learned parameters, and the assumption it adds to the baseline.

Its associated architecture or procedure is **MetaGPT framework**. The contribution should be explained as a change to a baseline computation: what new state is introduced, where information flows differently, and which objective trains the change. Multi-agent system where LLMs take roles (PM, architect, engineer, QA). Produces a complete software project from a one-line requirement. Defined structured multi-agent collaboration.
...
The paper's main claim is summarized in the registry as: Multi-agent system where LLMs take roles (PM, architect, engineer, QA). Produces a complete software project from a one-line requirement. Defined structured multi-agent collaboration. The key experiments should therefore isolate **causality**, not merely display a large final score.
...
The registry records the practical significance as: Automated software development research; AI developer tools This statement explains *why the paper remained relevant*, but exact benchmark values must be taken from the primary tables in [1]. They are intentionally not invented or copied without experimental context here.
...
Automated software development research; AI developer tools To translate that impact into a production decision, measure the complete path rather than an isolated kernel or offline model. Include preprocessing, retrieval or batching, host-device transfer, post-processing, cache behavior, and tail latency.
...
- [SWE-bench: Can Language Models Resolve Real-World GitHub Issues?](/papers/swe-bench-can-language-models-resolve-real-world-github-issues) — adjacent reading from the same library section.
- [WebGPT: Browser-Assisted Question-Answering with Human Feedback](/papers/webgpt-browser-assisted-question-answering-with-human-feedback) — adjacent reading from the same library section.
...
[1] Hong, Zheng, Chen, Cheng, Zhang, Wang, Yau, Lin, Zhou, Ran, Xiao, Wu. **MetaGPT: Meta Programming for a Multi-Agent Collaborative Framework**. 2023.
