...
- **Retrieval quality scoring:** identify its input, output, learned parameters, and the assumption it adds to the baseline.
- **knowledge refinement:** identify its input, output, learned parameters, and the assumption it adds to the baseline.
- **fallback to web search:** identify its input, output, learned parameters, and the assumption it adds to the baseline.
- **fine-grained knowledge strips:** identify its input, output, learned parameters, and the assumption it adds to the baseline.

Its associated architecture or procedure is **CRAG pipeline with retrieval evaluator**. The contribution should be explained as a change to a baseline computation: what new state is introduced, where information flows differently, and which objective trains the change. CRAG adds a retrieval evaluator that scores the relevance of retrieved documents. If documents are irrelevant, CRAG triggers web search as a fallback. If documents are ambiguous, it decomposes them into fine-grained chunks and filters knowledge strips. Prevents generation hallucinations from bad retrievals.
...
The registry records the practical significance as: Used in enterprise RAG deployments where retrieval quality varies. Influenced Self-RAG evaluation. Part of the "agentic RAG" paradigm. This statement explains *why the paper remained relevant*, but exact benchmark values must be taken from the primary tables in [1]. They are intentionally not invented or copied without experimental context here.
...
[1] Yan, Gu, Yu, Feng (various). **Corrective RAG (CRAG): Corrective Retrieval Augmented Generation**. 2024.
