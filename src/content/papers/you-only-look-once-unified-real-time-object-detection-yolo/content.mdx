...
Published in 2015 by Redmon, Divvala, Girshick, Farhadi, **You Only Look Once: Unified, Real-Time Object Detection (YOLO)** belongs to the COMPUTER VISION part of the Paper2Code library. Reframed object detection as a single regression problem. 45 FPS real-time detection vs. 7 FPS for Faster R-CNN. Spawned a family of models that dominate edge deployment today. [1]
...
- **Single-pass detection:** identify its input, output, learned parameters, and the assumption it adds to the baseline.
- **objectness score:** identify its input, output, learned parameters, and the assumption it adds to the baseline.
- **grid cell regression:** identify its input, output, learned parameters, and the assumption it adds to the baseline.
- **real-time detection tradeoffs:** identify its input, output, learned parameters, and the assumption it adds to the baseline.

Its associated architecture or procedure is **YOLO (grid cell prediction)**. The contribution should be explained as a change to a baseline computation: what new state is introduced, where information flows differently, and which objective trains the change. Reframed object detection as a single regression problem. 45 FPS real-time detection vs. 7 FPS for Faster R-CNN. Spawned a family of models that dominate edge deployment today.
...
The paper's main claim is summarized in the registry as: Reframed object detection as a single regression problem. 45 FPS real-time detection vs. 7 FPS for Faster R-CNN. Spawned a family of models that dominate edge deployment today. The key experiments should therefore isolate **causality**, not merely display a large final score.
...
The registry records the practical significance as: Autonomous vehicles, security cameras, industrial inspection — any real-time detection This statement explains *why the paper remained relevant*, but exact benchmark values must be taken from the primary tables in [1]. They are intentionally not invented or copied without experimental context here.
...
- [Very Deep Convolutional Networks for Large-Scale Image Recognition (VGGNet)](/papers/very-deep-convolutional-networks-for-large-scale-image-recognition-vggnet) — adjacent reading from the same library section.
- [Faster R-CNN: Towards Real-Time Object Detection with Region Proposal Networks](/papers/faster-r-cnn-towards-real-time-object-detection-with-region-proposal-networks) — adjacent reading from the same library section.
...
[1] Redmon, Divvala, Girshick, Farhadi. **You Only Look Once: Unified, Real-Time Object Detection (YOLO)**. 2015.
