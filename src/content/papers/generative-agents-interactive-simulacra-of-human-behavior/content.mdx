...
Published in 2023 by Park, O'Brien, Cai, Morris, Liang, Bernstein, **Generative Agents: Interactive Simulacra of Human Behavior** belongs to the AGENTS part of the Paper2Code library. Demonstrated LLM agents with persistent memory, reflection, and planning can simulate believable human behavior. 25 agents in a Sims-like world with emergent social behaviors. [1]
...
- **Memory stream:** identify its input, output, learned parameters, and the assumption it adds to the baseline.
- **reflection:** identify its input, output, learned parameters, and the assumption it adds to the baseline.
- **planning:** identify its input, output, learned parameters, and the assumption it adds to the baseline.
- **believable agents:** identify its input, output, learned parameters, and the assumption it adds to the baseline.
- **social simulation:** identify its input, output, learned parameters, and the assumption it adds to the baseline.

Its associated architecture or procedure is **Generative Agent architecture**. The contribution should be explained as a change to a baseline computation: what new state is introduced, where information flows differently, and which objective trains the change. Demonstrated LLM agents with persistent memory, reflection, and planning can simulate believable human behavior. 25 agents in a Sims-like world with emergent social behaviors.
...
The paper's main claim is summarized in the registry as: Demonstrated LLM agents with persistent memory, reflection, and planning can simulate believable human behavior. 25 agents in a Sims-like world with emergent social behaviors. The key experiments should therefore isolate **causality**, not merely display a large final score.
...
The registry records the practical significance as: Game AI, social simulation, AI companions This statement explains *why the paper remained relevant*, but exact benchmark values must be taken from the primary tables in [1]. They are intentionally not invented or copied without experimental context here.
...
Game AI, social simulation, AI companions To translate that impact into a production decision, measure the complete path rather than an isolated kernel or offline model. Include preprocessing, retrieval or batching, host-device transfer, post-processing, cache behavior, and tail latency.
...
- [Toolformer: Language Models Can Teach Themselves to Use Tools](/papers/toolformer-language-models-can-teach-themselves-to-use-tools) — adjacent reading from the same library section.
...
[1] Park, O'Brien, Cai, Morris, Liang, Bernstein. **Generative Agents: Interactive Simulacra of Human Behavior**. 2023.
