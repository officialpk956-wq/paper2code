# Human-Level Control through Deep Reinforcement Learning (Rainbow)
...
Published in 2017 by Hessel, Modayil, van Hasselt, Schaul, Ostrovski, et al., **Human-Level Control through Deep Reinforcement Learning (Rainbow)** belongs to the REINFORCEMENT LEARNING part of the Paper2Code library. Systematic combination of 6 DQN improvements (double, dueling, prioritized replay, n-step, distributional, noisy). Each improvement synergizes. Defined modern value-based RL. [1]
...
- **Distributional RL:** identify its input, output, learned parameters, and the assumption it adds to the baseline.
- **n-step returns:** identify its input, output, learned parameters, and the assumption it adds to the baseline.
- **noisy networks:** identify its input, output, learned parameters, and the assumption it adds to the baseline.
- **combined improvements:** identify its input, output, learned parameters, and the assumption it adds to the baseline.

Its associated architecture or procedure is **Rainbow**. The contribution should be explained as a change to a baseline computation: what new state is introduced, where information flows differently, and which objective trains the change. Systematic combination of 6 DQN improvements (double, dueling, prioritized replay, n-step, distributional, noisy). Each improvement synergizes. Defined modern value-based RL.
...
The paper's main claim is summarized in the registry as: Systematic combination of 6 DQN improvements (double, dueling, prioritized replay, n-step, distributional, noisy). Each improvement synergizes. Defined modern value-based RL. The key experiments should therefore isolate **causality**, not merely display a large final score.
...
The registry records the practical significance as: Best Atari benchmark; defined best practices for value-based RL This statement explains *why the paper remained relevant*, but exact benchmark values must be taken from the primary tables in [1]. They are intentionally not invented or copied without experimental context here.
...
- [Architecture deep dive](/architectures/rainbow) — inspect the corresponding model family and tensor flow.
...
[1] Hessel, Modayil, van Hasselt, Schaul, Ostrovski, et al.. **Human-Level Control through Deep Reinforcement Learning (Rainbow)**. 2017.
