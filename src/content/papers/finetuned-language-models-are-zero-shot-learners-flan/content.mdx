...
Published in 2021 by Wei, Bosma, Zhao, Yu, Chi, Short, Rashid, Huang, Du, Le, Zhou, **Finetuned Language Models Are Zero-Shot Learners (FLAN)** belongs to the LARGE LANGUAGE MODELS part of the Paper2Code library. Fine-tuning on diverse instruction templates enables strong zero-shot generalization. More diverse tasks → better generalization. Started the instruction tuning paradigm. [1]
...
Recurrent and convolutional sequence models compressed long contexts through sequential computation, limiting parallelism and making distant dependencies difficult to preserve. By 2021, researchers had working baselines, but the remaining gap was large enough that simply adding more layers, data, or hardware did not answer the scientific question.
...
- **Instruction tuning:** identify its input, output, learned parameters, and the assumption it adds to the baseline.
- **template diversity:** identify its input, output, learned parameters, and the assumption it adds to the baseline.
- **zero-shot generalization from supervised data:** identify its input, output, learned parameters, and the assumption it adds to the baseline.

Its associated architecture or procedure is **None**. The contribution should be explained as a change to a baseline computation: what new state is introduced, where information flows differently, and which objective trains the change. Fine-tuning on diverse instruction templates enables strong zero-shot generalization. More diverse tasks → better generalization. Started the instruction tuning paradigm.
...
The paper's main claim is summarized in the registry as: Fine-tuning on diverse instruction templates enables strong zero-shot generalization. More diverse tasks → better generalization. Started the instruction tuning paradigm. The key experiments should therefore isolate **causality**, not merely display a large final score.
...
- [QLoRA: Efficient Finetuning of Quantized LLMs](/papers/qlora-efficient-finetuning-of-quantized-llms) — adjacent reading from the same library section.
- [Self-Instruct: Aligning Language Models with Self-Generated Instructions](/papers/self-instruct-aligning-language-models-with-self-generated-instructions) — adjacent reading from the same library section.
...
[1] Wei, Bosma, Zhao, Yu, Chi, Short, Rashid, Huang, Du, Le, Zhou. **Finetuned Language Models Are Zero-Shot Learners (FLAN)**. 2021.
