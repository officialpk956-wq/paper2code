...
Published in 2017 by Bai et al. (Microsoft, Facebook/Meta), **ONNX: Open Neural Network Exchange** belongs to the SYSTEMS part of the Paper2Code library. ONNX defines a common intermediate representation for neural network models, enabling models trained in one framework (PyTorch, TensorFlow, scikit-learn) to be deployed in any other runtime (TensorRT, OpenVINO, CoreML, ONNX Runtime). Solves the train-in-one-place, deploy-everywhere problem. [1]
...
- **Computational graph serialization:** identify its input, output, learned parameters, and the assumption it adds to the baseline.
- **operator standardization:** identify its input, output, learned parameters, and the assumption it adds to the baseline.
- **opset versioning:** identify its input, output, learned parameters, and the assumption it adds to the baseline.
- **runtime optimization passes:** identify its input, output, learned parameters, and the assumption it adds to the baseline.
...
The registry records the practical significance as: Industry standard for model portability. ONNX Runtime is used in Microsoft Azure ML and Windows ML. Enables deployment on edge devices without retraining. Over 150 operators in the ONNX opset. This statement explains *why the paper remained relevant*, but exact benchmark values must be taken from the primary tables in [1]. They are intentionally not invented or copied without experimental context here.
...
[1] Bai et al. (Microsoft, Facebook/Meta). **ONNX: Open Neural Network Exchange**. 2017.
