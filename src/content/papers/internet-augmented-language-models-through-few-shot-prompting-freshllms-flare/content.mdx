# Internet-Augmented Language Models through Few-Shot Prompting (FreshLLMs/FLARE)
...
Published in 2023 by Jiang, Xu, Gao, Sun, Liu, Dwivedi-Yu, Yang, Callan, Neubig, **Internet-Augmented Language Models through Few-Shot Prompting (FreshLLMs/FLARE)** belongs to the RAG part of the Paper2Code library. Active retrieval — predict next sentence, retrieve if low confidence. Interleaves retrieval with generation rather than retrieve-once. More accurate for long-form generation. [1]
...
- **Active retrieval:** identify its input, output, learned parameters, and the assumption it adds to the baseline.
- **confidence-based retrieval trigger:** identify its input, output, learned parameters, and the assumption it adds to the baseline.
- **iterative RAG:** identify its input, output, learned parameters, and the assumption it adds to the baseline.

Its associated architecture or procedure is **FLARE**. The contribution should be explained as a change to a baseline computation: what new state is introduced, where information flows differently, and which objective trains the change. Active retrieval — predict next sentence, retrieve if low confidence. Interleaves retrieval with generation rather than retrieve-once. More accurate for long-form generation.
...
The paper's main claim is summarized in the registry as: Active retrieval — predict next sentence, retrieve if low confidence. Interleaves retrieval with generation rather than retrieve-once. More accurate for long-form generation. The key experiments should therefore isolate **causality**, not merely display a large final score.
...
The registry records the practical significance as: Long-form question answering systems, research synthesis tools This statement explains *why the paper remained relevant*, but exact benchmark values must be taken from the primary tables in [1]. They are intentionally not invented or copied without experimental context here.
...
Long-form question answering systems, research synthesis tools To translate that impact into a production decision, measure the complete path rather than an isolated kernel or offline model. Include preprocessing, retrieval or batching, host-device transfer, post-processing, cache behavior, and tail latency.
...
[1] Jiang, Xu, Gao, Sun, Liu, Dwivedi-Yu, Yang, Callan, Neubig. **Internet-Augmented Language Models through Few-Shot Prompting (FreshLLMs/FLARE)**. 2023.
