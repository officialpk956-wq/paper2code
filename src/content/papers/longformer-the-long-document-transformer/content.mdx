...
Published in 2020 by Beltagy, Peters, Cohan, **Longformer: The Long-Document Transformer** belongs to the NLP part of the Paper2Code library. Extended BERT to documents 4096+ tokens via sliding window attention + global attention. Made Transformer-based NLP practical for long documents like legal contracts and scientific papers. [1]
...
Recurrent and convolutional sequence models compressed long contexts through sequential computation, limiting parallelism and making distant dependencies difficult to preserve. By 2020, researchers had working baselines, but the remaining gap was large enough that simply adding more layers, data, or hardware did not answer the scientific question.
...
- **global attention tokens:** identify its input, output, learned parameters, and the assumption it adds to the baseline.
- **O(n) attention for long documents:** identify its input, output, learned parameters, and the assumption it adds to the baseline.

Its associated architecture or procedure is **Longformer**. The contribution should be explained as a change to a baseline computation: what new state is introduced, where information flows differently, and which objective trains the change. Extended BERT to documents 4096+ tokens via sliding window attention + global attention. Made Transformer-based NLP practical for long documents like legal contracts and scientific papers.
...
The paper's main claim is summarized in the registry as: Extended BERT to documents 4096+ tokens via sliding window attention + global attention. Made Transformer-based NLP practical for long documents like legal contracts and scientific papers. The key experiments should therefore isolate **causality**, not merely display a large final score.
...
The registry records the practical significance as: Legal AI, scientific document processing, long-document QA This statement explains *why the paper remained relevant*, but exact benchmark values must be taken from the primary tables in [1]. They are intentionally not invented or copied without experimental context here.
...
Legal AI, scientific document processing, long-document QA To translate that impact into a production decision, measure the complete path rather than an isolated kernel or offline model. Include preprocessing, retrieval or batching, host-device transfer, post-processing, cache behavior, and tail latency.
...
- [ELECTRA: Pre-training Text Encoders as Discriminators](/papers/electra-pre-training-text-encoders-as-discriminators) — adjacent reading from the same library section.
- [ColBERT: Efficient and Effective Passage Search via Contextualized Late Interaction](/papers/colbert-efficient-and-effective-passage-search-via-contextualized-late-interaction) — adjacent reading from the same library section.
...
[1] Beltagy, Peters, Cohan. **Longformer: The Long-Document Transformer**. 2020.
