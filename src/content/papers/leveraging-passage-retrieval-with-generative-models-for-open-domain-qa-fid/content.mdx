# Leveraging Passage Retrieval with Generative Models for Open Domain QA (FiD)
...
Published in 2020 by Izacard, Grave, **Leveraging Passage Retrieval with Generative Models for Open Domain QA (FiD)** belongs to the RAG part of the Paper2Code library. Fuse-in-Decoder: encode top-k retrieved passages independently with BERT, fuse in T5 decoder. Simple but dramatically outperforms single-passage RAG. Most effective RAG architecture at the time. [1]
...
- **decoder-level fusion:** identify its input, output, learned parameters, and the assumption it adds to the baseline.
- **multi-passage reading:** identify its input, output, learned parameters, and the assumption it adds to the baseline.

Its associated architecture or procedure is **FiD (Fusion-in-Decoder)**. The contribution should be explained as a change to a baseline computation: what new state is introduced, where information flows differently, and which objective trains the change. Fuse-in-Decoder: encode top-k retrieved passages independently with BERT, fuse in T5 decoder. Simple but dramatically outperforms single-passage RAG. Most effective RAG architecture at the time.
...
The paper's main claim is summarized in the registry as: Fuse-in-Decoder: encode top-k retrieved passages independently with BERT, fuse in T5 decoder. Simple but dramatically outperforms single-passage RAG. Most effective RAG architecture at the time. The key experiments should therefore isolate **causality**, not merely display a large final score.
...
The registry records the practical significance as: High-accuracy open-domain QA systems; clinical QA, research assistants This statement explains *why the paper remained relevant*, but exact benchmark values must be taken from the primary tables in [1]. They are intentionally not invented or copied without experimental context here.
...
- [Dense Passage Retrieval for Open-Domain Question Answering (DPR)](/papers/dense-passage-retrieval-for-open-domain-question-answering-dpr) — adjacent reading from the same library section.
...
[1] Izacard, Grave. **Leveraging Passage Retrieval with Generative Models for Open Domain QA (FiD)**. 2020.
