...
Published in 2023 by Oquab, Darcet, Moutakanni, Vo, Szafraniec, et al., **DINOv2: Learning Robust Visual Features without Supervision** belongs to the COMPUTER VISION part of the Paper2Code library. ViT features trained at scale on curated data match or exceed specialized supervised models on dense prediction tasks. Universal visual feature extractor without fine-tuning. [1]
...
- **Curated self-supervised data:** identify its input, output, learned parameters, and the assumption it adds to the baseline.
- **KoLeo regularization:** identify its input, output, learned parameters, and the assumption it adds to the baseline.
- **frozen features for dense tasks:** identify its input, output, learned parameters, and the assumption it adds to the baseline.

Its associated architecture or procedure is **DINOv2 (ViT with improved self-distillation)**. The contribution should be explained as a change to a baseline computation: what new state is introduced, where information flows differently, and which objective trains the change. ViT features trained at scale on curated data match or exceed specialized supervised models on dense prediction tasks. Universal visual feature extractor without fine-tuning.
...
The paper's main claim is summarized in the registry as: ViT features trained at scale on curated data match or exceed specialized supervised models on dense prediction tasks. Universal visual feature extractor without fine-tuning. The key experiments should therefore isolate **causality**, not merely display a large final score.
...
- [Architecture deep dive](/architectures/dino) — inspect the corresponding model family and tensor flow.
...
[1] Oquab, Darcet, Moutakanni, Vo, Szafraniec, et al.. **DINOv2: Learning Robust Visual Features without Supervision**. 2023.
