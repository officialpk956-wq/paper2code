...
Published in 2014 by Srivastava, Hinton, Krizhevsky, Sutskever, Salakhutdinov, **Dropout: A Simple Way to Prevent Neural Networks from Overfitting** belongs to the FOUNDATIONS part of the Paper2Code library. The canonical regularization technique for neural networks. Simple to implement, effective across architectures. Reframed regularization as approximate model ensembling. [1]
...
- **Dropout:** identify its input, output, learned parameters, and the assumption it adds to the baseline.
- **test-time averaging:** identify its input, output, learned parameters, and the assumption it adds to the baseline.
- **model ensembling interpretation:** identify its input, output, learned parameters, and the assumption it adds to the baseline.
- **inverted dropout:** identify its input, output, learned parameters, and the assumption it adds to the baseline.

Its associated architecture or procedure is **None**. The contribution should be explained as a change to a baseline computation: what new state is introduced, where information flows differently, and which objective trains the change. The canonical regularization technique for neural networks. Simple to implement, effective across architectures. Reframed regularization as approximate model ensembling.
...
The paper's main claim is summarized in the registry as: The canonical regularization technique for neural networks. Simple to implement, effective across architectures. Reframed regularization as approximate model ensembling. The key experiments should therefore isolate **causality**, not merely display a large final score.
...
The registry records the practical significance as: Standard in virtually every neural network trained before 2020; still widely used This statement explains *why the paper remained relevant*, but exact benchmark values must be taken from the primary tables in [1]. They are intentionally not invented or copied without experimental context here.
...
- [Auto-Encoding Variational Bayes (VAE)](/papers/auto-encoding-variational-bayes-vae) — adjacent reading from the same library section.
...
[1] Srivastava, Hinton, Krizhevsky, Sutskever, Salakhutdinov. **Dropout: A Simple Way to Prevent Neural Networks from Overfitting**. 2014.
