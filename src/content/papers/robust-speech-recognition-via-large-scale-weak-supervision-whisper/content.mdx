...
Published in 2022 by Radford, Kim, Xu, Brockman, McLeavey, Sutskever, **Robust Speech Recognition via Large-Scale Weak Supervision (Whisper)** belongs to the MULTIMODAL part of the Paper2Code library. 680,000 hours of web audio for weakly supervised ASR. Multitask (transcription, translation, language ID, timestamps). Robust zero-shot generalization — doesn't need fine-tuning for most languages. [1]
...
- **Weakly supervised ASR at scale:** identify its input, output, learned parameters, and the assumption it adds to the baseline.
- **multitask audio:** identify its input, output, learned parameters, and the assumption it adds to the baseline.
- **log-Mel spectrogram input:** identify its input, output, learned parameters, and the assumption it adds to the baseline.

Its associated architecture or procedure is **Whisper (encoder-decoder Transformer for audio)**. The contribution should be explained as a change to a baseline computation: what new state is introduced, where information flows differently, and which objective trains the change. 680,000 hours of web audio for weakly supervised ASR. Multitask (transcription, translation, language ID, timestamps). Robust zero-shot generalization — doesn't need fine-tuning for most languages.
...
The paper's main claim is summarized in the registry as: 680,000 hours of web audio for weakly supervised ASR. Multitask (transcription, translation, language ID, timestamps). Robust zero-shot generalization — doesn't need fine-tuning for most languages. The key experiments should therefore isolate **causality**, not merely display a large final score.
...
The registry records the practical significance as: OpenAI Whisper API, speech-to-text in virtually all AI voice products This statement explains *why the paper remained relevant*, but exact benchmark values must be taken from the primary tables in [1]. They are intentionally not invented or copied without experimental context here.
...
- [ImageBind: One Embedding Space to Bind Them All](/papers/imagebind-one-embedding-space-to-bind-them-all) — adjacent reading from the same library section.
- [CoCa: Contrastive Captioners are Image-Text Foundation Models](/papers/coca-contrastive-captioners-are-image-text-foundation-models) — adjacent reading from the same library section.
...
[1] Radford, Kim, Xu, Brockman, McLeavey, Sutskever. **Robust Speech Recognition via Large-Scale Weak Supervision (Whisper)**. 2022.
