...
Published in 2023 by Zhang, Rao, Agrawala, **Adding Conditional Control to Text-to-Image Diffusion Models (ControlNet)** belongs to the DIFFUSION MODELS part of the Paper2Code library. Trainable copy of diffusion encoder + zero-convolution enables precise spatial control from edges, depth maps, poses, and sketches. Made controllable image generation practical. [1]
...
- **Zero convolution:** identify its input, output, learned parameters, and the assumption it adds to the baseline.
- **trainable encoder copy:** identify its input, output, learned parameters, and the assumption it adds to the baseline.
- **conditioning images:** identify its input, output, learned parameters, and the assumption it adds to the baseline.
- **spatial control:** identify its input, output, learned parameters, and the assumption it adds to the baseline.

Its associated architecture or procedure is **ControlNet**. The contribution should be explained as a change to a baseline computation: what new state is introduced, where information flows differently, and which objective trains the change. Trainable copy of diffusion encoder + zero-convolution enables precise spatial control from edges, depth maps, poses, and sketches. Made controllable image generation practical.
...
The paper's main claim is summarized in the registry as: Trainable copy of diffusion encoder + zero-convolution enables precise spatial control from edges, depth maps, poses, and sketches. Made controllable image generation practical. The key experiments should therefore isolate **causality**, not merely display a large final score.
...
- [Scalable Diffusion Models with Transformers (DiT)](/papers/scalable-diffusion-models-with-transformers-dit) — adjacent reading from the same library section.
- [DreamBooth: Fine Tuning Text-to-Image Diffusion Models for Subject-Driven Generation](/papers/dreambooth-fine-tuning-text-to-image-diffusion-models-for-subject-driven-generation) — adjacent reading from the same library section.
...
[1] Zhang, Rao, Agrawala. **Adding Conditional Control to Text-to-Image Diffusion Models (ControlNet)**. 2023.
