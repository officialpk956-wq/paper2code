...
Published in 2020 by Guu, Lee, Tung, Pasupat, Chang (Google), **REALM: Retrieval-Augmented Language Model Pre-Training** belongs to the SYSTEMS part of the Paper2Code library. REALM integrates retrieval into pretraining itself — for each masked token prediction, the model retrieves relevant documents from a knowledge corpus before predicting. The retriever and language model are trained jointly end-to-end. Knowledge is stored in an external index, not in model parameters. [1]
...
- **End-to-end retrieval learning:** identify its input, output, learned parameters, and the assumption it adds to the baseline.
- **retriever as latent variable:** identify its input, output, learned parameters, and the assumption it adds to the baseline.
- **asynchronous index refresh during training:** identify its input, output, learned parameters, and the assumption it adds to the baseline.

Its associated architecture or procedure is **REALM (retrieval-augmented pretraining)**. The contribution should be explained as a change to a baseline computation: what new state is introduced, where information flows differently, and which objective trains the change. REALM integrates retrieval into pretraining itself — for each masked token prediction, the model retrieves relevant documents from a knowledge corpus before predicting. The retriever and language model are trained jointly end-to-end. Knowledge is stored in an external index, not in model parameters.
...
The paper's main claim is summarized in the registry as: REALM integrates retrieval into pretraining itself — for each masked token prediction, the model retrieves relevant documents from a knowledge corpus before predicting. The retriever and language model are trained jointly end-to-end. Knowledge is stored in an external index, not in model parameters. The key experiments should therefore isolate **causality**, not merely display a large final score.
...
The registry records the practical significance as: Foundational for retrieval-augmented language models. Predates but directly influenced RAG (2020). Showed that parametric and non-parametric knowledge can be jointly optimized. This statement explains *why the paper remained relevant*, but exact benchmark values must be taken from the primary tables in [1]. They are intentionally not invented or copied without experimental context here.
...
[1] Guu, Lee, Tung, Pasupat, Chang (Google). **REALM: Retrieval-Augmented Language Model Pre-Training**. 2020.
