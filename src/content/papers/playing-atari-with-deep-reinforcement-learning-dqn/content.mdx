...
Published in 2013 by Mnih, Kavukcuoglu, Silver, Graves, Antonoglou, Wierstra, Riedmiller, **Playing Atari with Deep Reinforcement Learning (DQN)** belongs to the REINFORCEMENT LEARNING part of the Paper2Code library. First demonstration that deep neural networks could learn to play games directly from raw pixels better than humans. Experience replay + target network = stable deep Q-learning. Started deep RL. [1]
...
Tabular methods and unstable function approximation struggled with large observation spaces, off-policy data, and high-variance policy gradients. By 2013, researchers had working baselines, but the remaining gap was large enough that simply adding more layers, data, or hardware did not answer the scientific question.
...
- **Experience replay:** identify its input, output, learned parameters, and the assumption it adds to the baseline.
- **target network:** identify its input, output, learned parameters, and the assumption it adds to the baseline.
- **ε-greedy exploration:** identify its input, output, learned parameters, and the assumption it adds to the baseline.
- **Atari benchmark:** identify its input, output, learned parameters, and the assumption it adds to the baseline.

Its associated architecture or procedure is **DQN**. The contribution should be explained as a change to a baseline computation: what new state is introduced, where information flows differently, and which objective trains the change. First demonstration that deep neural networks could learn to play games directly from raw pixels better than humans. Experience replay + target network = stable deep Q-learning. Started deep RL.
...
The paper's main claim is summarized in the registry as: First demonstration that deep neural networks could learn to play games directly from raw pixels better than humans. Experience replay + target network = stable deep Q-learning. Started deep RL. The key experiments should therefore isolate **causality**, not merely display a large final score.
...
The registry records the practical significance as: Foundation for all game-playing AI; influenced robotics, recommendation systems This statement explains *why the paper remained relevant*, but exact benchmark values must be taken from the primary tables in [1]. They are intentionally not invented or copied without experimental context here.
...
- [Architecture deep dive](/architectures/dqn) — inspect the corresponding model family and tensor flow.
- [Mastering the Game of Go with Deep Neural Networks and Tree Search (AlphaGo)](/papers/mastering-the-game-of-go-with-deep-neural-networks-and-tree-search-alphago) — adjacent reading from the same library section.
...
[1] Mnih, Kavukcuoglu, Silver, Graves, Antonoglou, Wierstra, Riedmiller. **Playing Atari with Deep Reinforcement Learning (DQN)**. 2013.
