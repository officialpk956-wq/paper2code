...
Published in 2017 by Howard, Zhu, Chen, Kalenichenko, Wang, Weyand, Andreetto, Adam, **MobileNets: Efficient CNNs for Mobile Vision** belongs to the COMPUTER VISION part of the Paper2Code library. Depthwise separable convolutions reduce compute 8–9× vs. standard convolutions with minimal accuracy loss. Defined the mobile inference paradigm. [1]
...
- **Depthwise convolution:** identify its input, output, learned parameters, and the assumption it adds to the baseline.
- **pointwise convolution:** identify its input, output, learned parameters, and the assumption it adds to the baseline.
- **width multiplier:** identify its input, output, learned parameters, and the assumption it adds to the baseline.
- **resolution multiplier:** identify its input, output, learned parameters, and the assumption it adds to the baseline.

Its associated architecture or procedure is **MobileNet, depthwise separable convolution**. The contribution should be explained as a change to a baseline computation: what new state is introduced, where information flows differently, and which objective trains the change. Depthwise separable convolutions reduce compute 8–9× vs. standard convolutions with minimal accuracy loss. Defined the mobile inference paradigm.
...
The paper's main claim is summarized in the registry as: Depthwise separable convolutions reduce compute 8–9× vs. standard convolutions with minimal accuracy loss. Defined the mobile inference paradigm. The key experiments should therefore isolate **causality**, not merely display a large final score.
...
The registry records the practical significance as: Google Lens, AR filters, on-device inference; powers billions of mobile AI features This statement explains *why the paper remained relevant*, but exact benchmark values must be taken from the primary tables in [1]. They are intentionally not invented or copied without experimental context here.
...
- [Densely Connected Convolutional Networks (DenseNet)](/papers/densely-connected-convolutional-networks-densenet) — adjacent reading from the same library section.
- [Rethinking Atrous Convolution for Semantic Segmentation (DeepLab v3)](/papers/rethinking-atrous-convolution-for-semantic-segmentation-deeplab-v3) — adjacent reading from the same library section.
...
[1] Howard, Zhu, Chen, Kalenichenko, Wang, Weyand, Andreetto, Adam. **MobileNets: Efficient CNNs for Mobile Vision**. 2017.
