...
Published in 2019 by Hafner, Lillicrap, Ba, Norouzi, **Dream to Control: Learning Behaviors by Latent Imagination (Dreamer)** belongs to the REINFORCEMENT LEARNING part of the Paper2Code library. World model in latent space. Learn actor-critic entirely in imagination — no real environment rollouts during policy optimization. 26× more sample-efficient than model-free on visual control. [1]
...
Tabular methods and unstable function approximation struggled with large observation spaces, off-policy data, and high-variance policy gradients. By 2019, researchers had working baselines, but the remaining gap was large enough that simply adding more layers, data, or hardware did not answer the scientific question.
...
- **Recurrent state space model:** identify its input, output, learned parameters, and the assumption it adds to the baseline.
- **imagination-based training:** identify its input, output, learned parameters, and the assumption it adds to the baseline.
- **latent world model:** identify its input, output, learned parameters, and the assumption it adds to the baseline.

Its associated architecture or procedure is **Dreamer (RSSM + actor-critic)**. The contribution should be explained as a change to a baseline computation: what new state is introduced, where information flows differently, and which objective trains the change. World model in latent space. Learn actor-critic entirely in imagination — no real environment rollouts during policy optimization. 26× more sample-efficient than model-free on visual control.
...
The paper's main claim is summarized in the registry as: World model in latent space. Learn actor-critic entirely in imagination — no real environment rollouts during policy optimization. 26× more sample-efficient than model-free on visual control. The key experiments should therefore isolate **causality**, not merely display a large final score.
...
- [Architecture deep dive](/architectures/dreamer) — inspect the corresponding model family and tensor flow.
- [Mastering Atari, Go, Chess and Shogi by Planning with a Learned Model (MuZero)](/papers/mastering-atari-go-chess-and-shogi-by-planning-with-a-learned-model-muzero) — adjacent reading from the same library section.
- [Human-Level Control through Deep Reinforcement Learning (Rainbow)](/papers/human-level-control-through-deep-reinforcement-learning-rainbow) — adjacent reading from the same library section.
...
[1] Hafner, Lillicrap, Ba, Norouzi. **Dream to Control: Learning Behaviors by Latent Imagination (Dreamer)**. 2019.
