...
Published in 2022 by Yao, Zhao, Yu, Du, Shafran, Narasimhan, Cao, **ReAct: Synergizing Reasoning and Acting in Language Models** belongs to the AGENTS part of the Paper2Code library. Interleaved reasoning traces and actions — "Thought, Action, Observation" loop. Dramatically improved performance on QA, fact verification, decision-making benchmarks. Defined the LLM agent paradigm. [1]
...
- **Thought-Action-Observation loop:** identify its input, output, learned parameters, and the assumption it adds to the baseline.
- **reasoning traces:** identify its input, output, learned parameters, and the assumption it adds to the baseline.
- **tool use:** identify its input, output, learned parameters, and the assumption it adds to the baseline.
- **grounded reasoning:** identify its input, output, learned parameters, and the assumption it adds to the baseline.

Its associated architecture or procedure is **ReAct (Reasoning + Acting)**. The contribution should be explained as a change to a baseline computation: what new state is introduced, where information flows differently, and which objective trains the change. Interleaved reasoning traces and actions — "Thought, Action, Observation" loop. Dramatically improved performance on QA, fact verification, decision-making benchmarks. Defined the LLM agent paradigm.
...
The paper's main claim is summarized in the registry as: Interleaved reasoning traces and actions — "Thought, Action, Observation" loop. Dramatically improved performance on QA, fact verification, decision-making benchmarks. Defined the LLM agent paradigm. The key experiments should therefore isolate **causality**, not merely display a large final score.
...
The registry records the practical significance as: Foundation of LangChain agents, AutoGPT, all LLM agent frameworks This statement explains *why the paper remained relevant*, but exact benchmark values must be taken from the primary tables in [1]. They are intentionally not invented or copied without experimental context here.
...
[1] Yao, Zhao, Yu, Du, Shafran, Narasimhan, Cao. **ReAct: Synergizing Reasoning and Acting in Language Models**. 2022.
