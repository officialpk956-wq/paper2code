...
Published in 2015 by Kingma, Ba, **Adam: A Method for Stochastic Optimization** belongs to the FOUNDATIONS part of the Paper2Code library. The default optimizer for deep learning. Adaptive learning rates per parameter + momentum = reliable convergence across architectures. Still the first choice 10 years later. [1]
...
- **Adaptive learning rates:** identify its input, output, learned parameters, and the assumption it adds to the baseline.
- **first/second moment estimation:** identify its input, output, learned parameters, and the assumption it adds to the baseline.
- **bias correction:** identify its input, output, learned parameters, and the assumption it adds to the baseline.
- **AdaGrad/RMSProp unification:** identify its input, output, learned parameters, and the assumption it adds to the baseline.

Its associated architecture or procedure is **None**. The contribution should be explained as a change to a baseline computation: what new state is introduced, where information flows differently, and which objective trains the change. The default optimizer for deep learning. Adaptive learning rates per parameter + momentum = reliable convergence across architectures. Still the first choice 10 years later.
...
The paper's main claim is summarized in the registry as: The default optimizer for deep learning. Adaptive learning rates per parameter + momentum = reliable convergence across architectures. Still the first choice 10 years later. The key experiments should therefore isolate **causality**, not merely display a large final score.
...
The registry records the practical significance as: Used as default optimizer in PyTorch, TensorFlow; every major model trained with Adam or derivative This statement explains *why the paper remained relevant*, but exact benchmark values must be taken from the primary tables in [1]. They are intentionally not invented or copied without experimental context here.
...
- [ImageNet Classification with Deep Convolutional Neural Networks (AlexNet)](/papers/imagenet-classification-with-deep-convolutional-neural-networks-alexnet) — adjacent reading from the same library section.
...
[1] Kingma, Ba. **Adam: A Method for Stochastic Optimization**. 2015.
