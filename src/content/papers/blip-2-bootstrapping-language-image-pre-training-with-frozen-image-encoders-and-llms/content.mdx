...
Published in 2023 by Li, Li, Savarese, Hoi, **BLIP-2: Bootstrapping Language-Image Pre-training with Frozen Image Encoders and LLMs** belongs to the MULTIMODAL part of the Paper2Code library. Q-Former bridges frozen image encoder and frozen LLM — only Q-Former trained. 54× fewer trainable parameters than Flamingo. State-of-art zero-shot VQA efficiently. [1]
...
- **Querying transformer:** identify its input, output, learned parameters, and the assumption it adds to the baseline.
- **bootstrapped alignment:** identify its input, output, learned parameters, and the assumption it adds to the baseline.
- **stage-wise pre-training:** identify its input, output, learned parameters, and the assumption it adds to the baseline.

Its associated architecture or procedure is **BLIP-2, Q-Former**. The contribution should be explained as a change to a baseline computation: what new state is introduced, where information flows differently, and which objective trains the change. Q-Former bridges frozen image encoder and frozen LLM — only Q-Former trained. 54× fewer trainable parameters than Flamingo. State-of-art zero-shot VQA efficiently.
...
The paper's main claim is summarized in the registry as: Q-Former bridges frozen image encoder and frozen LLM — only Q-Former trained. 54× fewer trainable parameters than Flamingo. State-of-art zero-shot VQA efficiently. The key experiments should therefore isolate **causality**, not merely display a large final score.
...
The registry records the practical significance as: Efficient vision-language alignment; basis for InstructBLIP This statement explains *why the paper remained relevant*, but exact benchmark values must be taken from the primary tables in [1]. They are intentionally not invented or copied without experimental context here.
...
Efficient vision-language alignment; basis for InstructBLIP To translate that impact into a production decision, measure the complete path rather than an isolated kernel or offline model. Include preprocessing, retrieval or batching, host-device transfer, post-processing, cache behavior, and tail latency.
...
- [Architecture deep dive](/architectures/blip-2) — inspect the corresponding model family and tensor flow.
- [Flamingo: a Visual Language Model for Few-Shot Learning](/papers/flamingo-a-visual-language-model-for-few-shot-learning) — adjacent reading from the same library section.
...
[1] Li, Li, Savarese, Hoi. **BLIP-2: Bootstrapping Language-Image Pre-training with Frozen Image Encoders and LLMs**. 2023.
