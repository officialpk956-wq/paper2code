...
- **GUI grounding:** identify its input, output, learned parameters, and the assumption it adds to the baseline.
- **dual-resolution visual encoding for UI text:** identify its input, output, learned parameters, and the assumption it adds to the baseline.
- **screenshot-based action prediction:** identify its input, output, learned parameters, and the assumption it adds to the baseline.
- **web navigation from pixels:** identify its input, output, learned parameters, and the assumption it adds to the baseline.

Its associated architecture or procedure is **CogAgent (dual-resolution GUI VLM)**. The contribution should be explained as a change to a baseline computation: what new state is introduced, where information flows differently, and which objective trains the change. Demonstrates a VLM trained specifically to interact with computer GUIs — clicking, typing, navigating — by predicting actions from screenshots. CogAgent introduces a high-resolution image encoder (1120×1120) for reading fine-grained UI text, combined with a low-resolution context encoder. Achieves SOTA on GUI-grounding and web agent benchmarks.
...
The paper's main claim is summarized in the registry as: Demonstrates a VLM trained specifically to interact with computer GUIs — clicking, typing, navigating — by predicting actions from screenshots. CogAgent introduces a high-resolution image encoder (1120×1120) for reading fine-grained UI text, combined with a low-resolution context encoder. Achieves SOTA on GUI-grounding and web agent benchmarks. The key experiments should therefore isolate **causality**, not merely display a large final score.
...
The registry records the practical significance as: One of the first capable GUI agents from screenshots. Directly influenced Claude Computer Use, GPT-4o's screen-understanding capabilities, and every agentic desktop automation system. This statement explains *why the paper remained relevant*, but exact benchmark values must be taken from the primary tables in [1]. They are intentionally not invented or copied without experimental context here.
...
- [Video-LLaMA: An Instruction-Tuned Audio-Visual Language Model for Video Understanding](/papers/video-llama-an-instruction-tuned-audio-visual-language-model-for-video-understanding) — adjacent reading from the same library section.
...
[1] Hong, Wang, Chen, Huang, Ma, Li, Lin, Tang, Zhang, Wang, Dong, Ji, et al. (Zhipu AI). **CogAgent: A Visual Language Model for GUI Agents**. 2023.
