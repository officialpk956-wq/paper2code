...
Published in 2023 by Song, Dhariwal, Chen, Sutskever, **Consistency Models** belongs to the DIFFUSION MODELS part of the Paper2Code library. Single-step generation via self-consistency — any point on the ODE trajectory maps to the same origin. 100× faster than DDPM with competitive quality. Enabled real-time image generation. [1]
...
- **Self-consistency property:** identify its input, output, learned parameters, and the assumption it adds to the baseline.
- **consistency distillation:** identify its input, output, learned parameters, and the assumption it adds to the baseline.
- **single-step generation:** identify its input, output, learned parameters, and the assumption it adds to the baseline.

Its associated architecture or procedure is **Consistency model, consistency distillation**. The contribution should be explained as a change to a baseline computation: what new state is introduced, where information flows differently, and which objective trains the change. Single-step generation via self-consistency — any point on the ODE trajectory maps to the same origin. 100× faster than DDPM with competitive quality. Enabled real-time image generation.
...
The paper's main claim is summarized in the registry as: Single-step generation via self-consistency — any point on the ODE trajectory maps to the same origin. 100× faster than DDPM with competitive quality. Enabled real-time image generation. The key experiments should therefore isolate **causality**, not merely display a large final score.
...
The registry records the practical significance as: Real-time image synthesis; enables on-device generation This statement explains *why the paper remained relevant*, but exact benchmark values must be taken from the primary tables in [1]. They are intentionally not invented or copied without experimental context here.
...
Real-time image synthesis; enables on-device generation To translate that impact into a production decision, measure the complete path rather than an isolated kernel or offline model. Include preprocessing, retrieval or batching, host-device transfer, post-processing, cache behavior, and tail latency.
...
- [Flow Matching for Generative Modeling](/papers/flow-matching-for-generative-modeling) — adjacent reading from the same library section.
...
[1] Song, Dhariwal, Chen, Sutskever. **Consistency Models**. 2023.
