...
Published in 2020 by Kaplan, McCandlish, Henighan, Brown, Chess, Child, Gray, Radford, Wu, Amodei, **Scaling Laws for Neural Language Models** belongs to the FOUNDATIONS part of the Paper2Code library. Discovered that LLM performance follows predictable power laws with compute, data, and parameters. Changed how AI labs plan training runs. Directly motivated GPT-3, Chinchilla, and all frontier model training. [1]
...
- **compute-optimal training:** identify its input, output, learned parameters, and the assumption it adds to the baseline.
- **power law exponents:** identify its input, output, learned parameters, and the assumption it adds to the baseline.
- **loss prediction from scale:** identify its input, output, learned parameters, and the assumption it adds to the baseline.

Its associated architecture or procedure is **None**. The contribution should be explained as a change to a baseline computation: what new state is introduced, where information flows differently, and which objective trains the change. Discovered that LLM performance follows predictable power laws with compute, data, and parameters. Changed how AI labs plan training runs. Directly motivated GPT-3, Chinchilla, and all frontier model training.
...
The paper's main claim is summarized in the registry as: Discovered that LLM performance follows predictable power laws with compute, data, and parameters. Changed how AI labs plan training runs. Directly motivated GPT-3, Chinchilla, and all frontier model training. The key experiments should therefore isolate **causality**, not merely display a large final score.
...
The registry records the practical significance as: Every major LLM training decision at OpenAI, Google, Meta, Anthropic uses scaling law analysis This statement explains *why the paper remained relevant*, but exact benchmark values must be taken from the primary tables in [1]. They are intentionally not invented or copied without experimental context here.
...
- [Distributed Representations of Words (Word2Vec)](/papers/distributed-representations-of-words-word2vec) — adjacent reading from the same library section.
- [An Image is Worth 16x16 Words: Transformers for Image Recognition at Scale (ViT)](/papers/an-image-is-worth-16x16-words-transformers-for-image-recognition-at-scale-vit) — adjacent reading from the same library section.
...
[1] Kaplan, McCandlish, Henighan, Brown, Chess, Child, Gray, Radford, Wu, Amodei. **Scaling Laws for Neural Language Models**. 2020.
