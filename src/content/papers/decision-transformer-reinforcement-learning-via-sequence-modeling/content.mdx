...
Published in 2021 by Chen, Lu, Rajeswaran, Lee, Grover, Laskin, Abbeel, Srinivas, Mordatch, **Decision Transformer: Reinforcement Learning via Sequence Modeling** belongs to the REINFORCEMENT LEARNING part of the Paper2Code library. Offline RL as conditional sequence modeling. Return-conditioned autoregressive policy using GPT. No Bellman equations, no bootstrapping — just sequence modeling. Bridged RL and LLMs. [1]
...
- **Return-conditioned generation:** identify its input, output, learned parameters, and the assumption it adds to the baseline.
- **offline RL as sequence modeling:** identify its input, output, learned parameters, and the assumption it adds to the baseline.
- **transformer policy:** identify its input, output, learned parameters, and the assumption it adds to the baseline.

Its associated architecture or procedure is **Decision Transformer**. The contribution should be explained as a change to a baseline computation: what new state is introduced, where information flows differently, and which objective trains the change. Offline RL as conditional sequence modeling. Return-conditioned autoregressive policy using GPT. No Bellman equations, no bootstrapping — just sequence modeling. Bridged RL and LLMs.
...
The paper's main claim is summarized in the registry as: Offline RL as conditional sequence modeling. Return-conditioned autoregressive policy using GPT. No Bellman equations, no bootstrapping — just sequence modeling. Bridged RL and LLMs. The key experiments should therefore isolate **causality**, not merely display a large final score.
...
The registry records the practical significance as: Influenced Gato, LLM-as-policy research This statement explains *why the paper remained relevant*, but exact benchmark values must be taken from the primary tables in [1]. They are intentionally not invented or copied without experimental context here.
...
Influenced Gato, LLM-as-policy research To translate that impact into a production decision, measure the complete path rather than an isolated kernel or offline model. Include preprocessing, retrieval or batching, host-device transfer, post-processing, cache behavior, and tail latency.
...
- [Hindsight Experience Replay (HER)](/papers/hindsight-experience-replay-her) — adjacent reading from the same library section.
- [A General Reinforcement Learning Algorithm that Masters Chess, Shogi, Go and Atari (Agent57)](/papers/a-general-reinforcement-learning-algorithm-that-masters-chess-shogi-go-and-atari-agent57) — adjacent reading from the same library section.
...
[1] Chen, Lu, Rajeswaran, Lee, Grover, Laskin, Abbeel, Srinivas, Mordatch. **Decision Transformer: Reinforcement Learning via Sequence Modeling**. 2021.
