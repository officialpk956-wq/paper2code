# Neural Architecture Search with Reinforcement Learning (NASNet)
...
Published in 2017 by Zoph, Le, **Neural Architecture Search with Reinforcement Learning (NASNet)** belongs to the FOUNDATIONS part of the Paper2Code library. Showed architectures could be designed by algorithms, not just humans. Started the AutoML field. Led to EfficientNet which dominated image classification benchmarks for years. [1]
...
- **Neural architecture search:** identify its input, output, learned parameters, and the assumption it adds to the baseline.
- **RL for architecture design:** identify its input, output, learned parameters, and the assumption it adds to the baseline.
- **cell-based search space:** identify its input, output, learned parameters, and the assumption it adds to the baseline.

Its associated architecture or procedure is **NASNet architecture, transferable cells**. The contribution should be explained as a change to a baseline computation: what new state is introduced, where information flows differently, and which objective trains the change. Showed architectures could be designed by algorithms, not just humans. Started the AutoML field. Led to EfficientNet which dominated image classification benchmarks for years.
...
The paper's main claim is summarized in the registry as: Showed architectures could be designed by algorithms, not just humans. Started the AutoML field. Led to EfficientNet which dominated image classification benchmarks for years. The key experiments should therefore isolate **causality**, not merely display a large final score.
...
The registry records the practical significance as: EfficientNet (derived from NAS) powers Google's image infrastructure This statement explains *why the paper remained relevant*, but exact benchmark values must be taken from the primary tables in [1]. They are intentionally not invented or copied without experimental context here.
...
- [Architecture deep dive](/architectures/nasnet) — inspect the corresponding model family and tensor flow.
- [Deep Learning (Nature Review)](/papers/deep-learning-nature-review) — adjacent reading from the same library section.
- [The Lottery Ticket Hypothesis](/papers/the-lottery-ticket-hypothesis) — adjacent reading from the same library section.
...
[1] Zoph, Le. **Neural Architecture Search with Reinforcement Learning (NASNet)**. 2017.
