...
Published in 2024 by Jiang, Sablayrolles, Roux, Mensch, Savary, Bamford, et al., **Mixtral of Experts** belongs to the LARGE LANGUAGE MODELS part of the Paper2Code library. Sparse Mixture of Experts with 8 experts, 2 active per token. 46.7B parameters, 12.9B active. GPT-3.5 quality at 6× lower inference cost. Validated MoE as practical architecture. [1]
...
- **Top-k expert routing:** identify its input, output, learned parameters, and the assumption it adds to the baseline.
- **sparse activation:** identify its input, output, learned parameters, and the assumption it adds to the baseline.
- **SMoE:** identify its input, output, learned parameters, and the assumption it adds to the baseline.
- **expert capacity factors:** identify its input, output, learned parameters, and the assumption it adds to the baseline.

Its associated architecture or procedure is **Mixtral 8×7B**. The contribution should be explained as a change to a baseline computation: what new state is introduced, where information flows differently, and which objective trains the change. Sparse Mixture of Experts with 8 experts, 2 active per token. 46.7B parameters, 12.9B active. GPT-3.5 quality at 6× lower inference cost. Validated MoE as practical architecture.
...
The paper's main claim is summarized in the registry as: Sparse Mixture of Experts with 8 experts, 2 active per token. 46.7B parameters, 12.9B active. GPT-3.5 quality at 6× lower inference cost. Validated MoE as practical architecture. The key experiments should therefore isolate **causality**, not merely display a large final score.
...
The registry records the practical significance as: MoE now standard for frontier models (GPT-4, Grok, Gemini reportedly MoE) This statement explains *why the paper remained relevant*, but exact benchmark values must be taken from the primary tables in [1]. They are intentionally not invented or copied without experimental context here.
...
- [Architecture deep dive](/architectures/mixtral-8x7b) — inspect the corresponding model family and tensor flow.
- [Mistral 7B](/papers/mistral-7b) — adjacent reading from the same library section.
...
[1] Jiang, Sablayrolles, Roux, Mensch, Savary, Bamford, et al.. **Mixtral of Experts**. 2024.
