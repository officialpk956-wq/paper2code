# Mastering Chess and Shogi by Self-Play with AlphaZero
...
Published in 2017 by Silver, Hubert, Schrittwieser, Antonoglou, Lai, Guez, Lanctot, et al., **Mastering Chess and Shogi by Self-Play with AlphaZero** belongs to the REINFORCEMENT LEARNING part of the Paper2Code library. Single algorithm mastered Chess, Shogi, and Go from scratch using only self-play — no human data. Tabula rasa learning via self-play is sufficient for superhuman performance. Generalized AlphaGo. [1]
...
- **Tabula rasa self-play:** identify its input, output, learned parameters, and the assumption it adds to the baseline.
- **single algorithm for multiple games:** identify its input, output, learned parameters, and the assumption it adds to the baseline.
- **PUCT selection:** identify its input, output, learned parameters, and the assumption it adds to the baseline.

Its associated architecture or procedure is **AlphaZero**. The contribution should be explained as a change to a baseline computation: what new state is introduced, where information flows differently, and which objective trains the change. Single algorithm mastered Chess, Shogi, and Go from scratch using only self-play — no human data. Tabula rasa learning via self-play is sufficient for superhuman performance. Generalized AlphaGo.
...
The paper's main claim is summarized in the registry as: Single algorithm mastered Chess, Shogi, and Go from scratch using only self-play — no human data. Tabula rasa learning via self-play is sufficient for superhuman performance. Generalized AlphaGo. The key experiments should therefore isolate **causality**, not merely display a large final score.
...
The registry records the practical significance as: Stockfish supplemented with AlphaZero-style ideas; general game-playing research This statement explains *why the paper remained relevant*, but exact benchmark values must be taken from the primary tables in [1]. They are intentionally not invented or copied without experimental context here.
...
- [Architecture deep dive](/architectures/alphazero) — inspect the corresponding model family and tensor flow.
...
[1] Silver, Hubert, Schrittwieser, Antonoglou, Lai, Guez, Lanctot, et al.. **Mastering Chess and Shogi by Self-Play with AlphaZero**. 2017.
