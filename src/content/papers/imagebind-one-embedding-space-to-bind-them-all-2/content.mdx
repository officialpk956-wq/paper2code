...
Published in 2023 by Girdhar, El-Nouby, Liu, Singh, Alwala, Joulin, Misra (Meta AI), **ImageBind: One Embedding Space to Bind Them All** belongs to the SYSTEMS part of the Paper2Code library. Aligns six modalities — images, text, audio, depth, thermal, IMU data — in a shared embedding space using only image-paired data for each modality. Audio-text alignment is never trained directly, yet emerges from both modalities being aligned to images. Enables zero-shot cross-modal retrieval (find images from audio, find audio from text) without paired training data. [1]
...
- **Emergent cross-modal alignment:** identify its input, output, learned parameters, and the assumption it adds to the baseline.
- **image as the binding pivot:** identify its input, output, learned parameters, and the assumption it adds to the baseline.
- **modality expansion without full pairwise supervision:** identify its input, output, learned parameters, and the assumption it adds to the baseline.

Its associated architecture or procedure is **ImageBind (six-modality binding via image pivoting)**. The contribution should be explained as a change to a baseline computation: what new state is introduced, where information flows differently, and which objective trains the change. Aligns six modalities — images, text, audio, depth, thermal, IMU data — in a shared embedding space using only image-paired data for each modality. Audio-text alignment is never trained directly, yet emerges from both modalities being aligned to images. Enables zero-shot cross-modal retrieval (find images from audio, find audio from text) without paired training data.
...
The paper's main claim is summarized in the registry as: Aligns six modalities — images, text, audio, depth, thermal, IMU data — in a shared embedding space using only image-paired data for each modality. Audio-text alignment is never trained directly, yet emerges from both modalities being aligned to images. Enables zero-shot cross-modal retrieval (find images from audio, find audio from text) without paired training data. The key experiments should therefore isolate **causality**, not merely display a large final score.
...
The registry records the practical significance as: Used in Meta's augmented reality research. Foundational for joint audio-visual understanding. Enabled new retrieval paradigms (search videos by audio description). This statement explains *why the paper remained relevant*, but exact benchmark values must be taken from the primary tables in [1]. They are intentionally not invented or copied without experimental context here.
...
- [Architecture deep dive](/architectures/imagebind) — inspect the corresponding model family and tensor flow.
...
[1] Girdhar, El-Nouby, Liu, Singh, Alwala, Joulin, Misra (Meta AI). **ImageBind: One Embedding Space to Bind Them All**. 2023.
