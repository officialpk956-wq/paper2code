...
Published in 2016 by Abadi, Barham, Chen, Chen, Davis, Dean, Devin, Ghemawat, Irving, et al., **TensorFlow: A System for Large-Scale Machine Learning** belongs to the SYSTEMS part of the Paper2Code library. First scalable, production-ready deep learning framework. Computation graph abstraction. TPU support. Deployed deep learning at Google's scale. [1]
...
- **Computation graph:** identify its input, output, learned parameters, and the assumption it adds to the baseline.
- **deferred execution:** identify its input, output, learned parameters, and the assumption it adds to the baseline.
- **TPU support:** identify its input, output, learned parameters, and the assumption it adds to the baseline.
- **TFRecord:** identify its input, output, learned parameters, and the assumption it adds to the baseline.

Its associated architecture or procedure is **TensorFlow computation graph**. The contribution should be explained as a change to a baseline computation: what new state is introduced, where information flows differently, and which objective trains the change. First scalable, production-ready deep learning framework. Computation graph abstraction. TPU support. Deployed deep learning at Google's scale.
...
The paper's main claim is summarized in the registry as: First scalable, production-ready deep learning framework. Computation graph abstraction. TPU support. Deployed deep learning at Google's scale. The key experiments should therefore isolate **causality**, not merely display a large final score.
...
The registry records the practical significance as: Powers Google search, translation, every major Google product This statement explains *why the paper remained relevant*, but exact benchmark values must be taken from the primary tables in [1]. They are intentionally not invented or copied without experimental context here.
...
Powers Google search, translation, every major Google product To translate that impact into a production decision, measure the complete path rather than an isolated kernel or offline model. Include preprocessing, retrieval or batching, host-device transfer, post-processing, cache behavior, and tail latency.
...
- [Efficient Memory Management for Large Language Model Serving with PagedAttention (vLLM)](/papers/efficient-memory-management-for-large-language-model-serving-with-pagedattention-vllm) — adjacent reading from the same library section.
...
[1] Abadi, Barham, Chen, Chen, Davis, Dean, Devin, Ghemawat, Irving, et al.. **TensorFlow: A System for Large-Scale Machine Learning**. 2016.
