...
Published in 2023 by Google DeepMind, **Gemini: A Family of Highly Capable Multimodal Models** belongs to the MULTIMODAL part of the Paper2Code library. First natively multimodal model at frontier scale — trained jointly on text, image, audio, video. Gemini Ultra surpassed GPT-4 on MMLU. Defined Google's multimodal AI direction. [1]
...
- **Native multimodal training:** identify its input, output, learned parameters, and the assumption it adds to the baseline.
- **efficient attention for long context:** identify its input, output, learned parameters, and the assumption it adds to the baseline.
- **cross-modal reasoning:** identify its input, output, learned parameters, and the assumption it adds to the baseline.

Its associated architecture or procedure is **Gemini (natively multimodal Transformer)**. The contribution should be explained as a change to a baseline computation: what new state is introduced, where information flows differently, and which objective trains the change. First natively multimodal model at frontier scale — trained jointly on text, image, audio, video. Gemini Ultra surpassed GPT-4 on MMLU. Defined Google's multimodal AI direction.
...
The paper's main claim is summarized in the registry as: First natively multimodal model at frontier scale — trained jointly on text, image, audio, video. Gemini Ultra surpassed GPT-4 on MMLU. Defined Google's multimodal AI direction. The key experiments should therefore isolate **causality**, not merely display a large final score.
...
Google Bard/Gemini products, Google Workspace AI To translate that impact into a production decision, measure the complete path rather than an isolated kernel or offline model. Include preprocessing, retrieval or batching, host-device transfer, post-processing, cache behavior, and tail latency.
...
- [PaLI: A Jointly-Scaled Multilingual Language-Image Model](/papers/pali-a-jointly-scaled-multilingual-language-image-model) — adjacent reading from the same library section.
- [Align Before You Fuse: A Unified Multimodal Understanding and Generation Model (BLIP)](/papers/align-before-you-fuse-a-unified-multimodal-understanding-and-generation-model-blip) — adjacent reading from the same library section.
...
[1] Google DeepMind. **Gemini: A Family of Highly Capable Multimodal Models**. 2023.
