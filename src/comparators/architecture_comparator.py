...
def summarize_spatial_behavior(graph: ArchitectureGraph) -> Dict[str, str]:
    ...
    has_skip_connections = any(
        node.semantic_params.get("skip_connection") == "yes"
        for node in graph.nodes
    )
    
    # Check for token-based processing (ViT indicator)
    has_tokens = any(
        "tokens" in node.semantic_params
        for node in graph.nodes
    )
...
def summarize_scaling_behavior(graph: ArchitectureGraph) -> Dict[str, str]:
    ...
    has_quadratic_attention = any(
        node.semantic_params.get("attention") == "quadratic"
        for node in graph.nodes
    )
...
def compare_graphs(
    ...
    if compute_a["total_high_flops"] > compute_b["total_high_flops"]:
        summary.append(f"{graph_a.name} is more compute-intensive ({compute_a['total_high_flops']} vs {compute_b['total_high_flops']} high-FLOPs nodes)")
    elif compute_a["total_high_flops"] < compute_b["total_high_flops"]:
        summary.append(f"{graph_b.name} is more compute-intensive ({compute_b['total_high_flops']} vs {compute_a['total_high_flops']} high-FLOPs nodes)")
    else:
        summary.append(f"Both architectures have similar compute intensity ({compute_a['total_high_flops']} high-FLOPs nodes each)")
