...
export default function DomainPage() {
  ...
  // Stats counts
  const beginnerCount    = groupedTopics.Beginner.length;
  const intermediateCount = groupedTopics.Intermediate.length;
  const advancedCount    = groupedTopics.Advanced.length;
  const expertCount      = groupedTopics.Expert.length;
      ...
      <FadeIn>
        <div className="border-b border-[#262626] px-8 py-6 bg-[#0A0A0A]">
          ...
          <div className="flex flex-wrap items-center gap-2 mt-4">
            <span className="text-[11px] text-[#525252] font-medium">{domain.topics.length} Topics</span>
            {beginnerCount > 0 && (
              <span className="rounded-full px-2.5 py-0.5 text-[10px] font-semibold" style={{ backgroundColor: '#4ADE8015', color: '#4ADE80' }}>
                {beginnerCount} Beginner
              </span>
            )}
            {intermediateCount > 0 && (
              <span className="rounded-full px-2.5 py-0.5 text-[10px] font-semibold" style={{ backgroundColor: '#FACC1515', color: '#FACC15' }}>
                {intermediateCount} Intermediate
              </span>
            )}
            {advancedCount > 0 && (
              <span className="rounded-full px-2.5 py-0.5 text-[10px] font-semibold" style={{ backgroundColor: '#F8717115', color: '#F87171' }}>
                {advancedCount} Advanced
              </span>
            )}
            {expertCount > 0 && (
              <span className="rounded-full px-2.5 py-0.5 text-[10px] font-semibold" style={{ backgroundColor: '#A78BFA15', color: '#A78BFA' }}>
                {expertCount} Expert
              </span>
            )}
          </div>
