...
function PaperRow({ paper }: { paper: PaperEntry }) {
  ...
  const hasMdx = [
    'alexnet', 'attention-is-all-you-need', 'bahdanau-attention', 'batch-normalization', 'bert', 'chinchilla',
    'clip', 'deep-residual-learning', 'dpo', 'flash-attention', 'gan', 'gpt-2', 'gpt-3', 'gpt', 'instructgpt',
    'latent-diffusion-models', 'llama', 'lora', 'palm', 'segment-anything', 'stable-diffusion', 'switch-transformer',
  ...
  return (
    <motion.div 
      className="border border-[#262626] rounded-xl bg-[#111111] mb-3 overflow-hidden"
      ...
      >
        ...
        <div className="flex-1">
          <div className="flex items-start justify-between">
            {href ? (
              <Link href={href} className="text-[15px] font-bold text-white hover:text-[#A78BFA] transition-colors" onClick={(e) => e.stopPropagation()}>
                {paper.title}
              </Link>
            ) : (
                ...
                <div className="grid grid-cols-2 lg:grid-cols-3 gap-4">
                  {papers.map(p => (
                    <Link key={p.id} href={`/papers/${p.id}`}
                      className="bg-[#111111] border border-[#262626] rounded-xl p-4 cursor-pointer hover:border-[#A78BFA]/30 transition-colors flex flex-col h-full">
