...
const STEPS = [
  {
    ...
    iconColor: '#A78BFA',
    title: '1. Upload PDF',
    desc: 'Drop your arXiv PDF directly into our workspace. We support complex math rendering and multi-column layouts.',
  },
  {
    icon: Cpu,
    iconColor: '#F97316',
    title: '2. AI Extraction',
    desc: 'Our vision models extract the exact neural architectures, hyperparameters, and pseudocode from the paper text.',
  },
  {
    icon: Code2,
    iconColor: '#4ADE80',
    title: '3. Get Runnable Code',
    desc: 'Receive a complete PyTorch implementation. You get a clean class structure, a forward pass, and training loop boilerplates.',
  },
];

const FEATURES = [
  'Extracts explicit PyTorch network graphs',
  'Detects hyperparameter tables automatically',
  'Links code directly to equations in the paper',
];

export default function ExtractCodePage() {
  return (
    <div className="min-h-screen bg-[#0A0A0A] text-white">
      {/* HERO SECTION — no animation, above the fold */}
              ...
              <FadeIn key={step.title} delay={idx * 0.15}>
                <div className="relative bg-[#111] border border-[#262626] rounded-2xl p-8 flex flex-col items-center text-center">
                  <div className="w-16 h-16 bg-[#1A1A1A] rounded-2xl flex items-center justify-center mb-6 border border-[#333] shadow-lg shadow-black/50 z-10">
                    <step.icon style={{ color: step.iconColor }} size={28} />
                  </div>
                  <h3 className="text-lg font-bold mb-3">{step.title}</h3>
                  <p className="text-[14px] text-[#A3A3A3] leading-relaxed">{step.desc}</p>
                </div>
              </FadeIn>
            ))}
          </div>
          ...
          <div className="bg-[#111] border border-[#262626] rounded-2xl overflow-hidden shadow-2xl flex flex-col lg:flex-row">
            <div className="w-full lg:w-1/2 p-12 bg-[#0D0D0D] flex flex-col justify-center border-b lg:border-b-0 lg:border-r border-[#262626]">
              ...
              <ul className="space-y-4">
                {FEATURES.map((f, idx) => (
                  <FadeIn key={f} delay={idx * 0.08}>
                    <li className="flex items-center gap-3 text-[14px] text-[#D4D4D4]">
                      <CheckCircle2 size={18} className="text-[#4ADE80]" /> {f}
                    </li>
                  </FadeIn>
                ))}
              </ul>
