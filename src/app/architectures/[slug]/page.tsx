      ...
      <div className="w-[260px] border-r border-[#262626] bg-[#0A0A0A] flex-shrink-0 flex flex-col h-full">
        ...
        <div className="flex-1 overflow-y-auto p-4 space-y-1">
          {ARCH_SECTIONS.map((section, idx) => (
            <a key={section} href={`#section-${idx + 1}`} className="block text-[12px] text-[#A3A3A3] hover:text-white hover:bg-[#111111] px-3 py-2 rounded-md transition-colors truncate">
              {idx + 1}. {section}
            </a>
          ))}
        </div>
                    ...
                    relatedPapers.length > 0 ? (
                      <div className="flex flex-col gap-3">
                        {relatedPapers.map(p => {
                          ...
                          return (
                            <Link key={p.slug} href={href} className="bg-[#111111] border border-[#262626] p-4 rounded-xl flex items-center justify-between hover:border-[#A78BFA]/50 transition-colors">
