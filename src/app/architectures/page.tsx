...
export default function ArchitecturesPage() {
  ...
  return (
    <div className="min-h-screen bg-[#0A0A0A] text-white">
      {/* HEADER */}
      <div className="border-b border-[#262626] bg-[#111] px-8 py-6">
        <div className="flex justify-between items-center mb-4">
          ...
          <input
            ...
            className="w-[240px] bg-[#141414] border border-[#262626] rounded-lg px-3 py-2 text-[13px] text-white placeholder:text-[#525252] focus:outline-none focus:border-[#FB923C]/40"
          />
        </div>
          ...
          {categories.map(c => (
            <button key={c} type="button" onClick={() => setActive(c)}
              className={'rounded-full px-3 py-1 text-[11px] font-medium border transition-colors ' +
                (c === active
                  ? 'bg-[#FB923C] border-[#FB923C] text-black'
      ...
      <div className="mx-auto max-w-7xl px-8 py-8 grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
        {filtered.map(a => (
          <Link key={a.slug} href={`/architectures/${a.slug}`}
            className="flex flex-col rounded-xl border border-[#262626] bg-[#111] p-4 hover:border-[#FB923C]/30 transition-colors">
            ...
            <div className="mt-3">
              <span className="rounded-md bg-[#141414] border border-[#262626] px-2 py-0.5 text-[10px] text-[#A3A3A3]">
                {a.category}
              </span>
            </div>
          </Link>
