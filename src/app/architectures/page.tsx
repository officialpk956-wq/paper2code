...
export default function ArchitecturesPage() {
  ...
  return (
    <div className="min-h-screen bg-transparent text-white">
      {/* HEADER */}
      <div className="border-b border-[#223429] bg-[#121D16] px-8 py-6">
        <div className="flex justify-between items-center mb-4">
          ...
          <input
            ...
            className="w-[240px] bg-[#16241B] border border-[#223429] rounded-lg px-3 py-2 text-[13px] text-white placeholder:text-[#525252] focus:outline-none focus:border-[#A3E635]/40"
          />
        </div>
            ...
            <button key={c} type="button" onClick={() => setActive(c)}
              className={'rounded-full px-3 py-1 text-[11px] font-medium border transition-colors ' +
                (c === active
                  ...
                  : 'border-[#223429] text-[#A3A3A3] hover:text-white hover:border-[#2E4436]')}>
              {c}
            </button>
          ))}
        </div>
      ...
      <div className="mx-auto max-w-7xl px-8 py-8 grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
        {filtered.map(a => (
          <Link key={a.slug} href={`/architectures/${a.slug}`}
            className="flex flex-col rounded-xl border border-[#223429] bg-[#121D16] p-4 hover:border-[#A3E635]/30 transition-colors">
            ...
            <div className="mt-3">
              <span className="rounded-md bg-[#16241B] border border-[#223429] px-2 py-0.5 text-[10px] text-[#A3A3A3]">
                {a.category}
              </span>
            </div>
          </Link>
