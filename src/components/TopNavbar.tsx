...
export function TopNavbar() {
  ...
  return (
    <header
      ...
      style={{ background: 'rgba(10,10,10,0.95)', borderColor: '#1E1E1E' }}
    >
            ...
            return (
              <Link
                ...
                className={
                  ...
                  (active
                    ? 'text-[#F97316] border-b-2 border-[#F97316] bg-[#F97316]/8'
        ...
        <div className="flex items-center gap-2">
          <div className="hidden items-center gap-2 md:flex">
            {user ? (
              <>
                <div title={user.name}
                  className="flex h-8 w-8 items-center justify-center rounded-full bg-[#F97316] text-xs font-bold text-black">
                  {user.name.charAt(0).toUpperCase()}
                </div>
                <button type="button" onClick={signOut}
                  className="rounded-lg border border-[#262626] bg-[#141414] px-3 py-1.5 text-xs text-[#A3A3A3] transition-colors hover:text-white hover:bg-[#1a1a1a]">
                  Sign Out
                </button>
              </>
            ) : (
              <>
                <button type="button" onClick={() => openAuth('signin')}
                  className="rounded-lg border border-[#262626] bg-[#141414] px-3 py-1.5 text-xs text-[#FAFAFA] transition-colors hover:bg-[#1a1a1a]">
                  Sign In
                </button>
                <button type="button" onClick={() => openAuth('signup')}
                  className="rounded-full bg-[#F97316] px-4 py-1.5 text-xs font-semibold text-black transition-colors hover:bg-[#fb8a3a]">
                  Get Started
                </button>
              </>
            )}
      ...
      {open && (
        <div className="fixed inset-0 z-50 md:hidden">
          ...
          <aside className="absolute left-0 top-0 flex h-full w-72 flex-col bg-[#111] p-4"
                ...
                return (
                  <Link key={link.href} href={link.href} onClick={() => setOpen(false)}
                    className={'rounded-md px-3 py-2.5 text-sm transition-colors ' +
                      (active ? 'bg-[#F97316]/10 text-[#F97316]' : 'text-[#A3A3A3] hover:bg-white/5 hover:text-white')}>
                    {link.label}
                  </Link>
                );
              })}
              ...
              {user ? (
                <>
                  ...
                  <button type="button" onClick={() => { setOpen(false); signOut(); }}
                    className="rounded-lg border border-[#262626] bg-[#141414] px-3 py-2 text-xs text-[#FAFAFA]">
                    Sign Out
                  </button>
                </>
              ) : (
                <>
                  ...
                  <button type="button" onClick={() => { setOpen(false); openAuth('signup'); }}
                    className="rounded-full bg-[#F97316] px-4 py-2 text-xs font-semibold text-black">
                    Get Started
                  </button>
                </>
              )}
