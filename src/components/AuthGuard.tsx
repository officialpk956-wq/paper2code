...
const isGatedRoute = (p: string) => {
  return p.startsWith('/dojo') || 
         p.startsWith('/architectures') || 
         p.startsWith('/extract-code') || 
         p.startsWith('/labs') || 
         p.startsWith('/learn') || 
         p.startsWith('/papers') || 
         p.startsWith('/system-design');
};
...
export function AuthGuard({ children }: { children: React.ReactNode }) {
  const { user, hydrated, signIn } = useAuthModal();
  ...
  const gated = isGatedRoute(pathname || '');

  useEffect(() => {
    if (hydrated && !loggedIn && gated) {
      sessionStorage.setItem('auth_intended_path', pathname);
    }
  }, [hydrated, loggedIn, gated, pathname]);

  const redirectAfterAuth = () => {
    const intended = sessionStorage.getItem('auth_intended_path');
    sessionStorage.removeItem('auth_intended_path');
    if (intended && intended !== pathname) {
  ...
  const submit = async (e: FormEvent) => {
    ...
    try {
      if (tab === 'signup') {
        const u = await signupWithPassword(username, email, password);
        signIn(u);
        redirectAfterAuth();
      } else {
  ...
  if (gated && !loggedIn) {
    return (
      <div className="mx-auto mt-24 w-full max-w-[420px] rounded-2xl border border-[#262626] bg-[#111111] p-8 relative">
        <div className="text-center mb-6">
          <h2 className="text-xl font-bold text-white mb-2">Sign in to continue</h2>
          <p className="text-sm text-[#A3A3A3]">You need an account to access this section.</p>
        </div>
