...
from src.explainers import explain_node as _explain_node, explain_graph as _explain_graph
from src.comparators import explain_architecture_comparison as _explain_arch_comparison
...
class DefaultExplanationAgent(ExplanationAgent):
    ...
    def explain_graph(self, graph: ArchitectureGraph) -> str:
        ...
        # CRITICAL: Read from semantic_params, not params
        repeat_groups: Dict[str, int] = {}
        for node in graph.nodes:
            if not node.semantic_params:
                continue

            group = node.semantic_params.get("_repeat_group")
            total = node.semantic_params.get("_repeat_total", 1)
