...
async def test_explorer_page(page, architecture_id, arch_name):
    ...
    try:
        ...
        if console_messages:
            ...
            print(f"[WARN] Console errors found: {len(console_messages)}")
            for err in console_messages[:3]:
                print(f"       {err}")
        else:
        ...
        if timeline_items:
            print(f"[OK] Timeline renders ({len(timeline_items)} stages)")
        ...
        if tensor_journey:
            ...
            for step in tensor_steps[:5]:  # Check first 5
                ...
                if text and text.strip() != "?" and text.strip() != "":
                    shapes_with_data += 1
                    if shapes_with_data <= 3:
                        print(f"     Shape: {text.strip()}")

            if shapes_with_data > 0:
                print(f"[OK] Tensor shapes display correctly ({shapes_with_data} have data)")
            else:
                print("[FAIL] Tensor shapes missing or showing '?'")
                arch_results["tensor_journey"] = "PARTIAL"

        else:
            print("[FAIL] Tensor Journey not found")
            arch_results["tensor_journey"] = "FAIL"

        # 6. Test FLOPs Display
        flops_elements = await page.query_selector_all(".tensor-step-info")
        if flops_elements:
            print(f"[OK] FLOPs/Params display found ({len(flops_elements)} elements)")
        ...
        if graph_container:
            ...
            if cytoscape_node:
                print("[OK] Graph appears to be rendered (canvas/SVG found)")
                arch_results["graph"] = "PASS"
            else:
                print("[WARN] Graph container exists but may not be fully rendered")
                arch_results["graph"] = "PARTIAL"
                arch_results["observations"].append("Graph container found but rendering unclear")
        else:
            print("[FAIL] Graph container not found")
        ...
        if len(timeline_items) > 1:
            ...
            print("[OK] Clicked second stage")

            # Check if content updated
            visible_stages = await page.query_selector_all(".stage-detail-panel[style*='display: block']")
            if visible_stages:
                print(f"[OK] Stage content updated after click ({len(visible_stages)} visible panels)")
        ...
        if math_button:
            ...
            print("[OK] Math toggle button clicked")

            # Check if math content visible
            math_elements = await page.query_selector_all(".tensor-math:not([style*='display: none'])")
            if math_elements:
                print(f"[OK] Math content visible after toggle ({len(math_elements)} elements)")
                arch_results["toggles"]["math"] = "PASS"
            else:
                print("[WARN] Math content may not be visible")
                arch_results["toggles"]["math"] = "PARTIAL"
        else:
            print("[WARN] Math toggle button not found")
            arch_results["toggles"]["math"] = "PARTIAL"
        ...
        if code_button:
            ...
            await page.wait_for_timeout(300)
            print("[OK] Code toggle button clicked")

            # Check if code content visible
            code_elements = await page.query_selector_all(".tensor-code:not([style*='display: none'])")
            if code_elements:
                print(f"[OK] Code content visible after toggle ({len(code_elements)} elements)")
                arch_results["toggles"]["code"] = "PASS"
            else:
                print("[WARN] Code content may not be visible")
                arch_results["toggles"]["code"] = "PARTIAL"
        else:
            print("[WARN] Code toggle button not found")
            arch_results["toggles"]["code"] = "PARTIAL"
        ...
        try:
            ...
            if modules_response['status'] == 'ok':
                print(f"[OK] API endpoint working (returned {modules_response.get('modules', 0)} modules)")
                arch_results["network"] = "PASS"
            else:
                print(f"[FAIL] API endpoint failed: {modules_response.get('message', 'unknown error')}")
                arch_results["network"] = "FAIL"
        except Exception as e:
            print(f"[FAIL] Network test error: {str(e)}")
            arch_results["network"] = "FAIL"
        ...
        print(f"[FAIL] Error testing {arch_name}: {str(e)}")
...
async def main():
    ...
    for arch_name, arch_data in results["architectures"].items():
        ...
        if arch_data.get("console_errors"):
            print(f"  CONSOLE ERRORS: {len(arch_data['console_errors'])}")
        if arch_data.get("network_issues"):
            print(f"  NETWORK ISSUES: {arch_data['network_issues']}")

    # Save results to file
    with open("C:/papper2code/phase11_browser_results.json", "w") as f:
        json.dump(results, f, indent=2)

    print("\n[OK] Results saved to phase11_browser_results.json")
    print("="*70)
