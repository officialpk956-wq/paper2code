...
def explain_node(node: GraphNode, context: dict = None) -> str:
    """
...
def explain_graph(graph: ArchitectureGraph) -> str:
    ...
    high_flops_nodes = [
        n for n in graph.nodes if n.semantic_params.get("flops") in ["high", "very high"]
    ]
    ...
    try:
        ...
        if flops_events:
            ...
            if critical_layers:
                advice.append(
                    f"Found {len(critical_layers)} layers with critical computational cost."
                )
                for cl in critical_layers:
                    if "attention" in cl["node_type"].lower():
                        advice.append(
                            f"  - Optimization: Use **Flash Attention** or **Linear Attention** for '{cl['node_id']}' to reduce quadratic O(N²) complexity."
                        )
                    elif "conv" in cl["node_type"].lower():
                        advice.append(
                            f"  - Optimization: Consider **Depthwise Separable Convolutions** to reduce the FLOPs/Params ratio for '{cl['node_id']}'."
                        )

            if total_flops > 5000:  # 5 GFLOPs threshold
                advice.append(
                    f"Overall model complexity is high ({total_flops / 1000:.1f} GFLOPs). Consider **Pruning** or **Weight Distillation**."
                )
