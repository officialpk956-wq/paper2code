...
class KnowledgeGraph:
    ...
    def _build_ontology(self):
        ...
        self.graph.add_node("transformer_encoder", type="block", contains=["multiheadattention", "layernorm", "linear"])
        self.graph.add_node("transformer_decoder", type="block", contains=["causal_attention", "cross_attention", "layernorm", "linear"])
        ...
        self.graph.add_edge("transformer_encoder", "transformer_encoder", relation="SEQUENCE_PRESERVED", reason="Encoder stacks preserve sequence lengths")
        self.graph.add_edge("causal_attention", "causal_attention", relation="CAUSAL_PRESERVED", reason="Autoregressive generation mask must be maintained")
        
        # Residual Compatibility Rules
        self.graph.add_edge("residual_add", "residual_add", relation="REQUIRES_EQUAL_DIMS", reason="Residual connections strictly require identically sized tensors")
        
        # Encoder-Decoder Constraints
        self.graph.add_edge("transformer_encoder", "cross_attention", relation="PROVIDES_MEMORY", reason="Encoder provides Key/Value memory for Decoder cross-attention")
        self.graph.add_edge("causal_attention", "cross_attention", relation="REQUIRES_PRIOR", reason="Decoder cross-attention typically follows causal self-attention")

        # 4. Semantic Role Mappings (KAG)
        for role in ["patch_embedding", "token_mixer", "sequence_encoder", "feature_aggregator", "classifier_head", "encoder", "decoder"]:
    ...
    def detect_motifs(self, arch_graph) -> List[str]:
        ...
        if causal_count >= 6 and "encoder" not in "".join(node_types) and "cross_attention" not in "".join(node_types):
            motifs.append("GPT-style Autoregressive Decoder Stack")
            
        # ViT token pipeline
        if "patchembedding" in "".join(node_types) and ("transformerblock" in "".join(node_types) or "encoder" in "".join(node_types)):
    ...
    def verify_topology(self, arch_graph) -> List[str]:
        ...
        if "cross_attention" in node_types and not any("encoder" in t for t in node_types):
            anomalies.append("Anomaly: Cross-Attention detected without an explicit Encoder memory source.")
            
        if "causal_attention" in node_types and "cross_attention" not in node_types and "encoder" in node_types:
            anomalies.append("Anomaly: Causal Attention mixed with Encoder blocks but missing Cross-Attention bridge.")
            ...
            if src_node and tgt_node:
                ...
                if self.graph.has_edge(s_type, t_type):
                    ...
                    if edge_data.get("relation") == "INCOMPATIBLE":
                        anomalies.append(f"Incompatible Transition: '{s_type}' to '{t_type}'. Reason: {edge_data.get('reason')}")
                        
                # Tensor Semantic Verification based on rules
                if s_type == "conv2d" and t_type == "linear":
                    anomalies.append(f"Missing Operation: Connecting '{s_type}' directly to '{t_type}' requires flattening.")
