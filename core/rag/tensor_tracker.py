...
class TensorTracker:
    ...
    def _validate_head_divisibility(self, node_id: str, dim: int, num_heads: int):
        ...
        if num_heads <= 0:
            raise TensorMismatchError(
                f"Multi-head Error at {node_id}: num_heads must be > 0, got {num_heads}"
            )
        if dim % num_heads != 0:
            raise TensorMismatchError(
    ...
    def _resolve_reshape(self, node_id: str, in_shape: tuple, target_shape: list) -> tuple:
        ...
        for d in in_shape[1:]:
            if isinstance(d, int):
                ...
                return tuple(target_shape)  # Dynamic/Symbolic bypass
        ...
        for i, d in enumerate(target_shape):
            if d == -1:
                if wildcard_idx != -1:
                    raise TensorMismatchError(
                        f"Reshape Error at {node_id}: Multiple -1 wildcards in {target_shape}",
                        node_id,
                    )
                wildcard_idx = i
        ...
        if wildcard_idx != -1:
            if in_elements % out_elements != 0:
                raise TensorMismatchError(
                    f"Reshape Error at {node_id}: Cannot reshape {in_shape} to {target_shape}",
                    node_id,
                )
            target_shape[wildcard_idx] = in_elements // out_elements
        elif in_elements != out_elements:
            raise TensorMismatchError(
                f"Reshape Error at {node_id}: Total elements mismatch. In: {in_elements}, Out: {out_elements}",
                node_id,
            )
    ...
    def propagate_shapes(
        self, graph: ArchitectureGraph, initial_shape: tuple | None = None
    ) -> None:
        """
                ...
                if len(input_shapes) > 1:
                    if node.type == "cross_attention":
                        if len(input_shapes) != 2:
                            raise TensorMismatchError(
                                f"Cross-Attention Error: Expected 2 inputs (Q, KV), got {len(input_shapes)}",
                                node.id,
                                self.dependency_chains[node.id],
                            )
                        q_shape, kv_shape = input_shapes[0], input_shapes[1]
                        if q_shape[-1] != kv_shape[-1]:
                            raise TensorMismatchError(
                                f"Cross-Attention Error: Embed dim mismatch. Q: {q_shape[-1]}, KV: {kv_shape[-1]}",
                                node.id,
                                self.dependency_chains[node.id],
                            )
                        ...
                        if len(input_shapes) != 2:
                            raise TensorMismatchError(
                                f"Attention Scores Error: Expected 2 inputs (Q, K), got {len(input_shapes)}",
                                node.id,
                                self.dependency_chains[node.id],
                            )
    ...
    def _compute_output_shape(self, node: GraphNode, in_shape: Any) -> tuple:
        ...
        if node.type == "attention_scores":
            ...
            if len(q_shape) != 4 or len(k_shape) != 4:
                raise TensorMismatchError(
                    f"Attention Scores Error at {node.id}: Expected 4D inputs, got {q_shape} and {k_shape}",
                    node.id,
                )
            B, H, N_q, D_H = q_shape
            _, _, N_k, _ = k_shape
        ...
        if not isinstance(in_shape, tuple) or (
            len(in_shape) > 0 and isinstance(in_shape[0], tuple)
        ):
        ...
        if node.type.lower() == "clstoken":
            if len(in_shape) != 3:
                raise TensorMismatchError(
                    f"CLS Token Error at {node.id}: Expected 3D input (B, N, D), got {in_shape}",
                    node.id,
                )
        ...
        if node.type.lower() in [
            ...
            if len(in_shape) == 2:
                ...
                if in_shape[2] != expected_dim:
                    raise TensorMismatchError(
                        f"Embedding Error at {node.id}: Dimension mismatch. Expected dim {expected_dim}, got {in_shape[2]}",
                        node.id,
                    )
                return in_shape
            else:
                raise TensorMismatchError(
                    f"Embedding Error at {node.id}: Expected 2D or 3D input, got {in_shape}",
                    node.id,
                )
        ...
        if len(in_shape) == 4:
            ...
            if node.type.lower() in ["conv2d", "conv1d", "conv"]:
                out_c = node.params.get(
                    "channels", node.params.get("out_channels", node.params.get("filters", C))
                )
            ...
            if node.type.lower() == "causal_mask":
                ...
                if in_shape[2] != in_shape[3] and node.params.get("strict_square", True):
                    raise TensorMismatchError(
                        f"Causal Mask Error at {node.id}: Expected square attention matrix for masking, got {in_shape[2]}x{in_shape[3]}",
                        node.id,
                    )
                return in_shape
        ...
        if node.type.lower() == "merge_heads":
            ...
            if len(in_shape) != 4:
                raise TensorMismatchError(
                    f"Merge Heads Error at {node.id}: Expected 4D input (B, H, N, D_H), got {in_shape}",
                    node.id,
                )
        ...
        if node.type.lower() == "transpose":
            ...
            if len(dims) != 2:
                raise TensorMismatchError(
                    f"Transpose Error at {node.id}: Expected 2 dimensions to swap.", node.id
                )
            res = list(in_shape)
        ...
        if node.type.lower() == "feedforward":
            out_features = node.params.get(
                "embed_dim", node.params.get("hidden_size", in_shape[-1])
            )
            return (*in_shape[:-1], out_features)
        ...
        if node.type.lower() in [
            ...
            if causal and node.type == "cross_attention":
                raise TensorMismatchError(
                    f"Cross-Attention Error at {node.id}: Cross-attention typically shouldn't be causal autoregressive.",
                    node.id,
                )
            return in_shape
        ...
        if len(in_shape) in [2, 3]:
            if node.type.lower() == "conv2d":
                raise TensorMismatchError(
                    f"Tensor Shape Error: Cannot pass flat tensor {in_shape} into spatial Conv2D layer ({node.id}).",
                    node.id,
                )
