...
class FLOPsEngine:
    ...
    def estimate(
        ...
        r = FLOPsResult(node_id=node_id, node_type=node_type,
    ...
    def _residual_block(self, r: FLOPsResult, in_s, out_s, p):
        ...
        if not isinstance(C_out, int): C_out = C_in
        H = _int(out_s, 2, 56)
        W = _int(out_s, 3, 56)
        K = _int(p, "kernel_size", 3)
    ...
    def _cross_attention(self, r: FLOPsResult, in_s, out_s, p):
        ...
        r.formula      = (
            ...
            f"= {total/1e6:.1f} MFLOPs across {H} heads"
        )
    ...
    def _generate_warnings(self, r: FLOPsResult, ntype: str,
                            in_s: tuple, p: Dict) -> List[str]:
        w = []
        if r.memory_mb > 500:
            w.append(f"High VRAM: {r.memory_mb:.1f} MB activation memory")
        if ntype.lower() in ("multiheadattention", "mhsa", "causal_attention",
                             "cross_attention", "self_attention"):
            N = _int(in_s, 1, 0)
            if isinstance(N, int) and N > 1024:
                w.append(
                    f"Quadratic attention explosion: N={N} → "
                    f"{N*N:,} score elements per head"
                )
        if r.flops_mflops > 2000:
            w.append(f"Compute-heavy: {r.flops_mflops:.0f} MFLOPs — "
