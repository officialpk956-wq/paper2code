...
def estimate_training_cost(
    ...
    arch_profile = ARCH_PROFILES.get(
        architecture,
        ARCH_PROFILES.get("ResNet50", ARCH_PROFILES.get("ResNet", list(ARCH_PROFILES.values())[0])),
    )
    ...
    if mixed_precision:
        assumptions.append(
            "Mixed precision enabled: effective VRAM reduced by factor 1.6 (FP16 weights + FP32 master copy)"
        )
    if gradient_checkpointing:
        assumptions.append(
            "Gradient checkpointing enabled: activation VRAM halved, training time increased by 25%"
        )
