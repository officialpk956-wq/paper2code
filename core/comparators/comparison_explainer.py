...
from core.comparators.architecture_comparator import compare_graphs


def explain_compute_difference(
    summary_a: dict[str, Any], summary_b: dict[str, Any], name_a: str, name_b: str
) -> str:
    """
    ...
    if count_a > count_b:
        if count_b == 0:
            lines.append(
                f"**{name_a}** is significantly more compute-intensive, with {count_a} high-cost operation(s), while **{name_b}** has no major computational bottlenecks."
            )
        else:
            lines.append(
                f"**{name_a}** requires more computation ({count_a} vs {count_b} high-FLOPs nodes)."
            )
    elif count_a < count_b:
        if count_a == 0:
            lines.append(
                f"**{name_b}** is significantly more compute-intensive, with {count_b} high-cost operation(s), while **{name_a}** has no major computational bottlenecks."
            )
        else:
            lines.append(
                f"**{name_b}** requires more computation ({count_b} vs {count_a} high-FLOPs nodes)."
            )
    else:
        if count_a == 0:
            lines.append(
                "Both architectures have minimal computational overhead — no major bottlenecks detected."
            )
        else:
            lines.append(
                f"Both architectures have similar compute intensity ({count_a} high-FLOPs nodes each)."
            )
...
def explain_spatial_difference(
    summary_a: dict[str, str], summary_b: dict[str, str], name_a: str, name_b: str
) -> str:
    """
...
def explain_architecture_comparison(graph_a: ArchitectureGraph, graph_b: ArchitectureGraph) -> str:
    """
    ...
    compute_explanation = explain_compute_difference(
        comparison["graph_a"]["compute"], comparison["graph_b"]["compute"], name_a, name_b
    )

    spatial_explanation = explain_spatial_difference(
        comparison["graph_a"]["spatial"], comparison["graph_b"]["spatial"], name_a, name_b
    )

    scaling_explanation = explain_scaling_difference(
        comparison["graph_a"]["scaling"], comparison["graph_b"]["scaling"], name_a, name_b
    )
