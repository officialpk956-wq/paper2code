...
class ExperimentResult:
    ...
    flops_delta: int
    depth_delta: int
    prediction_score: Optional[float]   # 0.0–1.0
    prediction_feedback: Optional[str]
    created_at: str = field(default_factory=lambda: datetime.datetime.utcnow().isoformat())
...
class HypothesisEngine:
    ...
    MUTATION_KEYWORDS = {
        "increase_depth": ["capacity", "expressivity", "overfitting", "gradient", "vanishing", "depth"],
        ...
        "add_attention":  ["global context", "receptive field", "self-attention", "tokens", "quadratic"],
    ...
    def score_prediction(
        ...
        if text_bonus > 0:
            ...
            if keywords:
                feedback_parts.append(
                    f"Tip: Explain using terms like: {', '.join(keywords[:4])}."
                )
    ...
    def generate_prediction_prompt(self, mutation_type: str, architecture: str) -> Dict[str, Any]:
        ...
        prompts = {
            "increase_depth": {
                ...
                "hints": ["Think about parameter count", "Consider gradient flow", "What about training time?"],
            ...
            "increase_width": {
                ...
                "hints": ["More parameters means?", "How does memory scale?", "Effect on expressivity?"],
                "expected_effects": {"param_direction": "increase", "flops_direction": "increase"},
            },
            "decrease_width": {
                "question": f"If you halve the channel widths of this {architecture}, what do you predict?",
                "hints": ["Efficiency vs accuracy tradeoff", "Mobile architectures do this", "Think about MobileNet"],
                "expected_effects": {"param_direction": "decrease", "flops_direction": "decrease"},
            },
            "add_residual": {
                "question": f"If you add residual (skip) connections to this {architecture}, what changes?",
                "hints": ["ResNet's key innovation", "Gradient flow in deep networks", "Identity shortcut effect"],
                "expected_effects": {"param_direction": "no_change", "flops_direction": "no_change"},
            },
            "remove_residual": {
                "question": f"If you remove all skip connections from this {architecture}, what do you predict?",
                "hints": ["Effect on training stability", "Gradient vanishing in depth", "Performance on deep networks"],
                "expected_effects": {"param_direction": "no_change", "flops_direction": "no_change"},
            },
            "add_attention": {
                "question": f"If you add a Multi-Head Attention block to this {architecture}, what effects do you predict?",
                "hints": ["Global receptive field", "Quadratic complexity (O(N²))", "Memory usage?"],
                "expected_effects": {"param_direction": "increase", "flops_direction": "increase"},
            },
            "change_patch_size": {
                "question": f"If you change the patch size in this {architecture} from 16 to 8, what happens?",
                "hints": ["Number of tokens = (224/patch_size)²", "Quadratic attention scaling", "Resolution vs compute"],
            ...
            "change_hidden_dim": {
                ...
                "hints": ["Model capacity scales with d²", "Transformer expressivity", "Memory footprint?"],
                "expected_effects": {"param_direction": "increase", "flops_direction": "increase"},
            },
        }

        prompt = prompts.get(mutation_type, {
