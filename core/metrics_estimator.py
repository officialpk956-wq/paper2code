...
def _estimate_node_params(node: GraphNode) -> int:
    ...
    ch = _extract_numeric_value(p.get("channels", p.get("out_channels", p.get("filters", 64))), 64)
    ...
    if t in ("conv2d", "conv1d", "conv"):
        ...
        return hs * hs
    elif t in ("multiheadattention", "transformerblock", "encoder_block", "decoder_block"):
