"""Write test_bug_fixes_h1.py (8 real tests) and test_phase_k.py (4 real tests)."""
import pathlib
...
def test_bug_4(client, db_session):
    \"\"\"GET /api/dojo/exercises returns a list.\"\"\"
...
def test_bug_8(client, db_session):
    \"\"\"GET /api/dojo/problems returns a list.\"\"\"
    resp = client.get("/api/dojo/problems")
    assert resp.status_code in (200, 401)
"""
...
pathlib.Path("tests/test_bug_fixes_h1.py").write_text(h1, encoding="utf-8")
pathlib.Path("tests/test_phase_k.py").write_text(pk, encoding="utf-8")
print("done")
