...
class TokenRepository:
    ...
    def verify_email_token(self, token: str) -> int | None:
        ...
        row = self.db.query(EmailVerificationToken).filter_by(
            token=token, used_at=None).first()
    ...
    def create_password_reset(self, user_id: int) -> str:
        ...
        self.db.query(PasswordResetToken).filter_by(
            user_id=user_id, used_at=None).update({"used_at": datetime.now(timezone.utc)})
        token = secrets.token_urlsafe(48)
    ...
    def verify_reset_token(self, token: str) -> int | None:
        row = self.db.query(PasswordResetToken).filter_by(
            token=token, used_at=None).first()
        if not row or row.expires_at < datetime.now(timezone.utc):
            return None
        row.used_at = datetime.now(timezone.utc)
