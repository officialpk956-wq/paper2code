...
def get_public_profile(user_id: int, db: Session = Depends(get_db)):
    ...
    higher = (db.query(func.count(User.id)).filter(User.points > user.points).scalar()) or 0
    rank = higher + 1
    ...
    earned = [
        {"slug": a.slug, "title": a.title, "description": a.description} for a in achievements
    ]
...
def get_xp_history(
    ...
    total = (
        db.query(func.count(XPEvent.id)).filter(XPEvent.user_id == current_user.id).scalar()
    ) or 0
    events = (
        db.query(XPEvent)
...
def get_learning_path(
    ...
    # Weak topics
    attempts = db.query(AssessmentAttempt).filter_by(learner_id=str(current_user.id)).all()
    ...
    solved = [
        s.problem_id
        for s in db.query(DojoSubmission)
    ...
    probs = (
        db.query(Problem)
        .filter(Problem.is_retired == False, ~Problem.id.in_(solved))
        .limit(30)
        .all()
    )
...
def get_my_papers(
    ...
    offset = (page - 1) * page_size
    total = (
        db.query(func.count(Paper.id)).filter(Paper.uploaded_by == current_user.id).scalar()
    ) or 0
    papers = (
        db.query(Paper)
...
class NotificationPrefsUpdate(BaseModel):
    email_drip_opt_out: bool | None = None
