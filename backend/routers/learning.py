...
def get_recommendations(
    ...
    try:
        ...
        adaptive = adaptive_engine.get_personalized_recommendations(*_fetch_adaptive_data(db, learner_key), _fetch_all_papers_data(db))
        return {
            "user_id": current_user.id,
...
def _get_budget_callback(db):
    def callback(user_id: int):
        ...
        total = db.query(func.sum(UsageLog.prompt_tokens + UsageLog.completion_tokens)).filter(
