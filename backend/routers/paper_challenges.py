...
def get_paper_challenges(
    paper_id: int, current_user: User = Depends(get_current_user), db: Session = Depends(get_db)
):
    ...
    challenges = (
        db.query(PaperChallenge)
        .filter(PaperChallenge.paper_id == paper_id, PaperChallenge.is_published == True)
        .order_by(PaperChallenge.order_idx)
        .all()
    )
    ...
    passed_subs = (
        db.query(PaperPartSubmission.part_id)
        .filter(
...
def get_challenge_part(
    ...
    part = (
        db.query(PaperChallengePart)
        .filter(PaperChallengePart.id == part_id, PaperChallengePart.challenge_id == challenge_id)
        .first()
    )
    if not part:
...
def run_challenge_part(
    ...
    if passed and not prior_pass:
        ...
        award_xp(
            ...
            current_user.id,
            "paper.part.solved",
            f"paper_part-{part.id}",
            amount_override=xp_earned,
        )
...
def get_part_progress(
    ...
    subs = (
        db.query(PaperPartSubmission)
        .filter(
            PaperPartSubmission.user_id == current_user.id, PaperPartSubmission.part_id == part_id
        )
        .order_by(PaperPartSubmission.created_at.desc())
        .all()
    )
    ...
    attempts = [{"id": s.id, "passed": s.passed, "created_at": s.created_at} for s in subs]
