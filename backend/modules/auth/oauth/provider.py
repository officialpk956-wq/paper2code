...
class OAuthUserInfo:
    def __init__(self, provider: str, uid: str, email: str, name: str, avatar_url: Optional[str]):
...
class OAuthProvider:
    async def get_user_info(self, token: str) -> Optional[OAuthUserInfo]:
    ...
    async def get_user_info(self, token: str) -> Optional[OAuthUserInfo]:
        ...
        try:
            async with httpx.AsyncClient() as client:
                ...
                if not email:
                    email_resp = await client.get("https://api.github.com/user/emails", headers=headers, timeout=5.0)
