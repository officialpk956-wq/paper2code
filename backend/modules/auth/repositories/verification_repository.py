...
from backend.modules.auth.models import VerificationToken, ResetToken
...
class VerificationRepository:
    ...
    # Verification Tokens
    def create_verification_token(self, user_id: int, token: str, expires_at: datetime.datetime) -> VerificationToken:
    ...
    def get_verification_token(self, token: str) -> Optional[VerificationToken]:
        hashed = hash_token(token)
    ...
    # Reset Tokens
    def create_reset_token(self, user_id: int, token: str, expires_at: datetime.datetime) -> ResetToken:
        hashed = hash_token(token)
        rt = ResetToken(
            user_id=user_id,
            token_hash=hashed,
