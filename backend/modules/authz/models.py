...
class Organization(Base):
    ...
    subscription_tier = Column(String(50), default="Free", nullable=False)  # Free, Pro, Team, Enterprise
...
class OrganizationMember(Base):
    ...
    organization = relationship("Organization", backref=backref("members", cascade="all, delete-orphan"))
...
class OrganizationInvitation(Base):
    ...
    organization = relationship("Organization", backref=backref("invitations", cascade="all, delete-orphan"))
...
class Team(Base):
    ...
    id              = Column(Integer, primary_key=True, index=True)
    organization_id = Column(Integer, ForeignKey("organizations.id", ondelete="CASCADE"), nullable=False, index=True)
    parent_id       = Column(Integer, ForeignKey("teams.id", ondelete="SET NULL"), nullable=True, index=True)
    name            = Column(String(255), nullable=False)
    slug            = Column(String(255), nullable=False, index=True)
    created_at      = Column(DateTime, default=datetime.datetime.utcnow, nullable=False)

    organization = relationship("Organization", backref=backref("teams", cascade="all, delete-orphan"))
...
class ResourceShare(Base):
    ...
    __table_args__ = (
        UniqueConstraint("resource_type", "resource_id", "shared_with_type", "shared_with_id", name="uq_resource_share"),
    )
