...
from backend.modules.authz.models import OrganizationMember, TeamMember, ResourceShare, ApiKey
from backend.modules.authz.roles import has_role_permission

def check_scoped_api_key(db: Session, key_hash: str, required_scope: str) -> tuple[bool, Optional[User]]:
...
def authorize(
    ...
    if org_id:
        member = db.query(OrganizationMember).filter_by(organization_id=org_id, user_id=user.id).first()
    ...
    if resource_type and resource_id is not None:
        ...
        table_name = f"{resource_type}s" if not resource_type.endswith("y") else f"{resource_type[:-1]}ies"
        ...
        shares = db.query(ResourceShare).filter_by(resource_type=resource_type, resource_id=str(resource_id)).all()
        for share in shares:
            ...
            if required_access in allowed_actions:
                if share.shared_with_type == "public":
                    ...
                    org_member = db.query(OrganizationMember).filter_by(
                        organization_id=int(share.shared_with_id),
                        user_id=user.id
                    ).first()
                    if org_member:
                        return True
                    ...
                    team_member = db.query(TeamMember).filter_by(
                        team_id=int(share.shared_with_id),
                        user_id=user.id
                    ).first()
                    if team_member:
                        return True
