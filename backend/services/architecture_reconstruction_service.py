...
def _extract_signals(title: str, abstract: str, sections: list[dict]) -> dict[str, int | None]:
...
def _resnet_template(
    ...
    components = [
        Component(
            ...
            "conv",
            [1, 64, 112, 112],
            {"in_channels": 3, "out_channels": 64, "kernel_size": 7, "stride": 2, "channels": 64},
        ),
        Component(
            "pool1",
        ...
        Component(
            ...
            [1, out_ch[0], 56, 56],
            {
                ...
                "kernel_size": 3,
                "has_residual": True,
                "repeat_count": stage_d[0],
                "flops_type": "residualblock",
            },
        ),
        Component(
            "stage2",
            f"Stage 2 ×{stage_d[1]} ({variant})",
            "conv",
            [1, out_ch[1], 28, 28],
            {
                "channels": out_ch[1],
                ...
                "kernel_size": 3,
                "has_residual": True,
                "repeat_count": stage_d[1],
                "flops_type": "residualblock",
            },
        ),
        Component(
            "stage3",
            f"Stage 3 ×{stage_d[2]} ({variant})",
            "conv",
            [1, out_ch[2], 14, 14],
            {
                "channels": out_ch[2],
                ...
                "kernel_size": 3,
                "has_residual": True,
                "repeat_count": stage_d[2],
                "flops_type": "residualblock",
            },
        ),
        Component(
            "stage4",
            f"Stage 4 ×{stage_d[3]} ({variant})",
            "conv",
            [1, out_ch[3], 7, 7],
            {
                "channels": out_ch[3],
                ...
                "kernel_size": 3,
                "has_residual": True,
                "repeat_count": stage_d[3],
                "flops_type": "residualblock",
            },
        ),
        Component("avgpool", "Global Avg Pool", "pooling", [1, out_ch[3], 1, 1], {}),
        Component(
            "head",
            f"Classifier (→{n_classes})",
            "head",
            [1, n_classes],
...
def _vit_template(
    ...
    components = [
        Component(
            "patch_embed",
            f"Patch Embedding ({patch_size}×{patch_size})",
            "embedding",
        ...
        Component(
            "encoder",
            f"Transformer Encoder ×{num_layers}",
            "attention",
            [1, seq_len, embed_dim],
            {
                "embed_dim": embed_dim,
        ...
        Component("norm", "Layer Norm", "normalization", [1, seq_len, embed_dim], {}),
        Component(
            "head",
            f"MLP Head (→{n_classes})",
            "head",
            [1, n_classes],
...
def _unet_template(
    ...
    components = [
        ...
        Component(
            "bottleneck",
            f"Bottleneck (×2 Conv, {c5}ch)",
            "conv",
            [1, c5, 16, 16],
            {"in_channels": c4, "out_channels": c5, "channels": c5, "kernel_size": 3},
        ),
        Component("up4", "Upsample ×2", "upsample", [1, c5, 32, 32], {}),
        Component(
            "dec4",
            f"Decoder 4 (×2 Conv, {c4}ch)",
            "conv",
            [1, c4, 32, 32],
            {"in_channels": c5 + c4, "out_channels": c4, "channels": c4, "kernel_size": 3},
        ),
        Component("up3", "Upsample ×2", "upsample", [1, c4, 64, 64], {}),
        Component(
            "dec3",
            f"Decoder 3 (×2 Conv, {c3}ch)",
            "conv",
            [1, c3, 64, 64],
            {"in_channels": c4 + c3, "out_channels": c3, "channels": c3, "kernel_size": 3},
        ),
        Component("up2", "Upsample ×2", "upsample", [1, c3, 128, 128], {}),
        Component(
            "dec2",
            f"Decoder 2 (×2 Conv, {c2}ch)",
            "conv",
            [1, c2, 128, 128],
            {"in_channels": c3 + c2, "out_channels": c2, "channels": c2, "kernel_size": 3},
        ),
        Component("up1", "Upsample ×2", "upsample", [1, c2, 256, 256], {}),
        Component(
            "dec1",
            f"Decoder 1 (×2 Conv, {c1}ch)",
            "conv",
            [1, c1, 256, 256],
...
def _transformer_template(
    ...
    components = [
        Component(
            ...
            "embedding",
            [1, seq_len, embed_dim],
            {"embed_dim": embed_dim, "vocab_size": vocab_size, "flops_type": "token_embedding"},
        ),
        Component(
            "pos_embed",
            "Positional Encoding",
            "embedding",
            [1, seq_len, embed_dim],
            {"embed_dim": embed_dim, "vocab_size": seq_len, "flops_type": "positionalembedding"},
        ),
        Component(
            "enc_stack",
            f"Encoder Stack ×{num_layers}",
            "attention",
            [1, seq_len, embed_dim],
            {
                "embed_dim": embed_dim,
...
def _gan_template(
    ...
    components = [
        Component(
            ...
            "embedding",
            [1, latent_dim],
            {"embed_dim": latent_dim, "vocab_size": latent_dim, "flops_type": "token_embedding"},
        ),
        Component(
            "fc",
            "FC + Reshape (4×4×512)",
        ...
        Component("up1", "ConvTranspose ×2 (256ch)", "upsample", [1, 256, 8, 8], {}),
        Component("up2", "ConvTranspose ×2 (128ch)", "upsample", [1, 128, 16, 16], {}),
        Component("up3", "ConvTranspose ×2 (64ch)", "upsample", [1, 64, 32, 32], {}),
        Component(
            "out_conv",
            f"Output Conv Tanh ({img_ch}ch)",
            "head",
...
def _vae_template(
    ...
    components = [
        ...
        Component(
            ...
            {"in_channels": 32, "out_channels": 64, "channels": 64, "kernel_size": 4, "stride": 2},
        ),
        Component(
            "enc_fc",
            "Encoder FC → μ, σ",
        ...
        Component(
            "latent",
            f"Latent Space z ({latent_d}d)",
            "embedding",
            [1, latent_d],
            {"embed_dim": latent_d, "vocab_size": latent_d, "flops_type": "token_embedding"},
        ),
        Component(
            "dec_fc",
            "Decoder FC",
            "mlp",
            [1, 64 * (img_size // 4) ** 2],
            {"embed_dim": latent_d, "ff_dim": 64 * (img_size // 4) ** 2},
        ),
        Component(
            "dec_conv1",
            "Decoder ConvT ×2 (32ch)",
            "upsample",
            [1, 32, img_size // 2, img_size // 2],
            {},
        ),
...
def _diffusion_template(
    ...
    components = [
        ...
        Component(
            ...
            "embedding",
            [1, base_ch * 4],
            {"embed_dim": base_ch * 4, "vocab_size": 1000, "flops_type": "token_embedding"},
        ),
        Component(
            "unet_enc1",
            "U-Net Encoder 1 (64ch)",
            "conv",
            [1, base_ch, img_size, img_size],
        ...
        Component(
            "unet_dec2",
            "U-Net Decoder 2 (128ch)",
            "upsample",
            [1, base_ch * 2, img_size // 2, img_size // 2],
            {},
        ),
        Component(
            "unet_dec1", "U-Net Decoder 1 (64ch)", "upsample", [1, base_ch, img_size, img_size], {}
        ),
        Component(
            "out_conv",
            f"Output Conv (→{img_ch}ch)",
...
def _lstm_template(
    ...
    components = [
        Component(
            ...
            "embedding",
            [1, seq_len, hidden],
            {"embed_dim": hidden, "vocab_size": vocab, "flops_type": "token_embedding"},
        ),
        Component(
            "lstm",
            f"LSTM ×{layers} Layers",
            "mlp",
            [1, seq_len, hidden],
            {"embed_dim": hidden, "ff_dim": hidden * 4, "repeat_count": layers},
        ),
        Component("dropout", "Dropout", "normalization", [1, seq_len, hidden], {}),
        Component(
            "head",
            f"Output FC (→{n_out})",
            "head",
...
def _generic_template(
    ...
    components = [
        Component(
            ...
            "embedding",
            [1, embed_dim],
            {"embed_dim": embed_dim, "vocab_size": embed_dim, "flops_type": "token_embedding"},
        ),
        Component(
            "encoder",
            "Encoder",
...
def reconstruct_architecture(
    ...
    components, connections, input_shape, output_shape = template_fn(signals, n_classes)

    flops_engine = FLOPsEngine()
    tensor_flow = _simulate_tensor_flow(components, connections, input_shape, flops_engine)
