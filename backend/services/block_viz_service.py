...
class _ViTB16(nn.Module):
    ...
    def __init__(self) -> None:
        ...
        self.encoder = nn.Sequential(*[
            TransformerEncoderBlock(d_model=d_model, num_heads=12, ffn_dim=d_model * 4, dropout=0.0)
            for _ in range(12)
        ])
...
_ARCH_MAP: Dict[str, Dict[str, Any]] = {
    ...
    "vit": {
        ...
        "stage_groups": [
            {
                "id": "embedding", "name": "Patch Embedding", "type": "stem",
            ...
            {
                "id": "encoder", "name": "Transformer Encoder (×12)", "type": "backbone",
                "modules": ["encoder"],
    ...
    "transformer": {
        ...
        "stage_groups": [
            {
                "id": "embedding", "name": "Embedding", "type": "stem",
            ...
            {
                "id": "encoder", "name": "Encoder Stack (×6)", "type": "backbone",
...
class BlockVizService:
    ...
    def get_block_detail(self, paper_id: str, block_id: str) -> Dict[str, Any]:
        ...
        fr = self._flops_engine.estimate(
            block_id, type(mod).__name__,
            tuple(s.get("in") or ()), tuple(s.get("out") or ()),
            _module_to_params(mod), _humanize_name(block_id.split(".")[-1], mod),
        )
        if block_flops == 0.0:
        ...
        for sg in cfg["stage_groups"]:
            ...
            for mod_name in mod_names:
                ...
                for block_prefix, block_mod in _expand_to_blocks(top_mod, mod_name):
                    ...
                    if block_flops == 0.0:
                        fr = self._flops_engine.estimate(
                            block_prefix, type(block_mod).__name__,
                            tuple(b_s.get("in") or ()), tuple(b_s.get("out") or ()),
        ...
        for child_name, child_mod in block_mod.named_children():
            ...
            if isinstance(child_mod, nn.Sequential):
                ...
                for sub_name, sub_mod in child_mod.named_children():
                    ...
                    layers.append(self._make_layer_node(sub_id, f"{child_name}.{sub_name}", sub_mod, shapes))
            else:
    ...
    def _build_forward_pass_steps(
        ...
        for name, mod in model.named_modules():
            ...
            fr = self._flops_engine.estimate(
                ...
                tuple(in_s), tuple(out_s),
                _module_to_params(mod), name,
            )
...
def _describe_layer(mod: nn.Module, in_s: List, out_s: List) -> str:
    if isinstance(mod, nn.Conv2d):
        ...
        return (f"{k_str} convolution — {mod.in_channels}→{mod.out_channels} channels, "
                f"output: {out_s}")
    if isinstance(mod, nn.ConvTranspose2d):
