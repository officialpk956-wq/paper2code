...
from backend.models import Paper, PaperModule, Base
import re
...
def build_unet():
    ...
    for i, ch in enumerate([64, 128, 256, 512]):
        layers.append({"id": f"enc_{i}", "type": "Conv2d", "label": f"Encoder Block {i+1} ({ch}ch)", "params": {"channels": ch}})
        if i > 0:
            connections.append({"source": f"pool_{i-1}", "target": f"enc_{i}", "edge_type": "flow"})
        layers.append({"id": f"pool_{i}", "type": "MaxPool2d", "label": f"Downsample {i+1}", "params": {"kernel_size": 2}})
        connections.append({"source": f"enc_{i}", "target": f"pool_{i}", "edge_type": "flow"})
        
    # Bottleneck
    layers.append({"id": "bottleneck", "type": "Conv2d", "label": "Bottleneck (1024ch)", "params": {"channels": 1024}})
    connections.append({"source": "pool_3", "target": "bottleneck", "edge_type": "flow"})
    
    # Decoder
    prev = "bottleneck"
    for i, ch in enumerate([512, 256, 128, 64]):
        layers.append({"id": f"up_{i}", "type": "Upsample", "label": f"Upsample {i+1}", "params": {}})
        connections.append({"source": prev, "target": f"up_{i}", "edge_type": "flow"})
        
        layers.append({"id": f"dec_{i}", "type": "Conv2d", "label": f"Decoder Block {i+1} ({ch}ch)", "params": {"channels": ch}})
    ...
    # Head
    layers.append({"id": "head", "type": "Conv2d", "label": "Output Conv (1x1)", "params": {"channels": 2}})
...
def build_transformer(encoder_layers, decoder_layers):
    defs = [
        ("PatchEmbedding", "Token + Positional Embedding", {"embed_dim": 512})
    ]
    ...
    for i in range(decoder_layers):
        defs.append(("MultiHeadAttention", f"Decoder Layer {i+1} - Masked MHSA", {"d_model": 512}))
        defs.append(("MultiHeadAttention", f"Decoder Layer {i+1} - Cross-Attention", {"d_model": 512}))
...
def build_vit(blocks):
    defs = [
        ("PatchEmbedding", "Patch Embedding + Positional Encoding", {"embed_dim": 768})
    ]
...
ARCHITECTURES = {
    ...
    "AlexNet": {"family": "cnn", "authors": "Krizhevsky et al.", "builder": lambda: build_alexnet()},
    "VGG16": {"family": "cnn", "authors": "Simonyan & Zisserman", "builder": lambda: build_vgg([(2, 64), (2, 128), (3, 256), (3, 512), (3, 512)])},
    "VGG19": {"family": "cnn", "authors": "Simonyan & Zisserman", "builder": lambda: build_vgg([(2, 64), (2, 128), (4, 256), (4, 512), (4, 512)])},
    "GoogLeNet": {"family": "cnn", "authors": "Szegedy et al.", "builder": lambda: build_googlenet()},
    "ResNet18": {"family": "resnet", "authors": "He et al.", "builder": lambda: build_resnet([2, 2, 2, 2])},
    "ResNet34": {"family": "resnet", "authors": "He et al.", "builder": lambda: build_resnet([3, 4, 6, 3])},
    "ResNet50": {"family": "resnet", "authors": "He et al.", "builder": lambda: build_resnet([3, 4, 6, 3], block_type="BottleneckBlock")},
    "DenseNet121": {"family": "cnn", "authors": "Huang et al.", "builder": lambda: build_densenet121()},
    "MobileNetV2": {"family": "cnn", "authors": "Sandler et al.", "builder": lambda: build_mobilenetv2()},
    "EfficientNet-B0": {"family": "cnn", "authors": "Tan & Le", "builder": lambda: build_efficientnet_b0()},
    ...
    "Transformer": {"family": "transformer", "authors": "Vaswani et al.", "builder": lambda: build_transformer(6, 6)},
    "Vision Transformer": {"family": "transformer", "authors": "Dosovitskiy et al.", "builder": lambda: build_vit(12)},
}
...
def process_and_persist():
        ...
        for name, meta in ARCHITECTURES.items():
            ...
            for m in modules:
                ...
                if exp in global_explanations and not exp.startswith("Structural module:"):
                    raise ValueError(f"QUALITY AUDIT FAILED: Duplicate explanation found: {exp[:50]}...")
    ...
    print("\n" + "=" * 70)
    print(f"Pipeline complete. {success_count}/{len(ARCHITECTURES)} verified architectures processed.")
    print("=" * 70)
